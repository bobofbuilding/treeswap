...
- Deploy the implemented opaque ownership-handle service, same-origin SIWE-session edge core, and private wallet gateway with real D1/session wiring, private edge-to-coordinator TLS, separate key custody, durable/distributed abuse controls or one enforced replica, handle/body-log suppression, exclusive use of the reviewed user-activated browser adapter, durable lifecycle reconciliation, two-provider finality evidence, and explicit approval boundaries; the repository core is documented in [`docs/CONTRACT_INTENT_WALLET.md`](docs/CONTRACT_INTENT_WALLET.md)
