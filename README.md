...
TreeSwap displays **1 BIT = 100 sats** only as a non-binding product reference. The BIT contract does not enforce that price. Executable Lightning/BIT amounts and fees come from competing short-lived signed solver RFQs and remain inside the user's signed limits. No BIT/WBTC pool exists today, so it contributes zero price evidence; after a separate observation-only rollout, maturity period, liquidity review, and signed release-policy admission, it may contribute at most one request-sized risk signal and never becomes the settlement route or sole price authority.
...
- Aggregate-only, fail-closed D1 availability, latency, access-audit, purge-backlog, and dual-route escalation controls in [`docs/ACCOUNT_STORAGE_MONITORING.md`](docs/ACCOUNT_STORAGE_MONITORING.md); deployment remains a launch gate
- Two-role encrypted D1 export and fresh isolated restore evidence in [`docs/ACCOUNT_BACKUP_RESTORE_EVIDENCE.md`](docs/ACCOUNT_BACKUP_RESTORE_EVIDENCE.md); no live data or platform mutation is included
- Private scheduled-only account cleanup with bounded D1 deletion and create-only aggregate R2 evidence in [`docs/SCHEDULED_ACCOUNT_MAINTENANCE.md`](docs/SCHEDULED_ACCOUNT_MAINTENANCE.md); deployment remains blocked
...
- Exact escrow-wallet preflight, opaque wallet/session-bound one-use ownership handles, an exclusive same-origin SIWE-session edge composed through the signed hash-only [`wallet-session reader`](docs/WALLET_SESSION_READER.md), one inert-by-default closed-test Sites route over the existing authoritative D1 binding with bounded current/retiring key overlap, a two-role exact-source [`route review boundary`](docs/WALLET_SESSION_ROUTE_REVIEW.md), a separate two-operator signed private [`deployment preflight`](docs/WALLET_SESSION_ROUTE_DEPLOYMENT_PREFLIGHT.md), a three-observer accountable-claims [`deployment postflight`](docs/WALLET_SESSION_ROUTE_DEPLOYMENT_POSTFLIGHT.md), a three-role 24-hour [`independent live-review boundary`](docs/WALLET_SESSION_ROUTE_DEPLOYMENT_LIVE_REVIEW.md), HMAC CSRF and a strict restart-safe private abuse ledger, verified private abuse-ledger backup/restore plus real process-kill, crash-fence and disk-full evidence, one-shot journal-gated EIP-1193 dispatcher, signed non-replayable edge/coordinator claim core, independent browser-side claim verification, atomic digest-only cross-tab no-resend tombstones, idempotent outcome reporting, durable restart recovery, transaction/replacement validation, receipt/event classification, and repository-only two-observation agreement in [`docs/CONTRACT_INTENT_WALLET.md`](docs/CONTRACT_INTENT_WALLET.md)
...
- Deploy the implemented quote reader, reservation service, independent BOLT 11 decoder, both private browser-signing routes, selected-solver endpoint and invoice-material service, capacity readers, dual-route daemon-evidence protocols, and independent quote-delivery paths behind preserving reviewed HTTPS listeners
...
- Deploy the implemented opaque ownership-handle service, same-origin SIWE-session edge, signed hash-only D1 session reader/provider, private restart-safe abuse ledger, pre-session perimeter, and private wallet gateway with the existing authoritative D1 store, a reviewed server-only session route and listener/CDN, pinned authenticated HTTPS, four-way separate session/gateway key custody, handle/session/body-log suppression, exclusive use of the reviewed user-activated browser adapter, durable lifecycle reconciliation, two-provider finality evidence, and explicit approval boundaries. Early testnet must keep the abuse ledger and implemented no-stale-takeover fence on owner-controlled persistent storage visible to the one supported replica and repeat the repository backup/restore, `SIGKILL`, disk-full and retained-fence campaign on that volume. Automatic or multi-host failover remains closed until a separately reviewed distributed fence and distributed abuse policy exist. The repository cores are documented in [`docs/CONTRACT_INTENT_WALLET.md`](docs/CONTRACT_INTENT_WALLET.md), [`docs/WALLET_SESSION_READER.md`](docs/WALLET_SESSION_READER.md), [`docs/DURABLE_WALLET_ABUSE_STORE.md`](docs/DURABLE_WALLET_ABUSE_STORE.md), and [`docs/WALLET_EDGE_VOLUME_RECOVERY.md`](docs/WALLET_EDGE_VOLUME_RECOVERY.md)
