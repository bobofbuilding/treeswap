...
- Deploy the implemented opaque ownership-handle service, same-origin SIWE-session edge, private restart-safe abuse ledger, pre-session perimeter, and private wallet gateway with real D1/session wiring, a reviewed listener/CDN, private edge-to-coordinator TLS, separate key custody, handle/body-log suppression, exclusive use of the reviewed user-activated browser adapter, durable lifecycle reconciliation, two-provider finality evidence, and explicit approval boundaries. Early testnet must keep the abuse ledger and implemented no-stale-takeover fence on owner-controlled persistent storage visible to the one supported replica and repeat the repository backup/restore, `SIGKILL`, disk-full and retained-fence campaign on that volume. Automatic or multi-host failover remains closed until a separately reviewed distributed fence and distributed abuse policy exist. The repository cores are documented in [`docs/CONTRACT_INTENT_WALLET.md`](docs/CONTRACT_INTENT_WALLET.md), [`docs/DURABLE_WALLET_ABUSE_STORE.md`](docs/DURABLE_WALLET_ABUSE_STORE.md), and [`docs/WALLET_EDGE_VOLUME_RECOVERY.md`](docs/WALLET_EDGE_VOLUME_RECOVERY.md)
