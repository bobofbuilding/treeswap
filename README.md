...
- Deploy the implemented opaque ownership-handle service and private wallet gateway behind the SIWE edge with server-derived session binding, CSRF/rate controls, separate keys, handle/body-log suppression, exclusive use of the reviewed user-activated browser adapter, single-replica enforcement, durable lifecycle reconciliation, two-provider finality evidence, and explicit approval boundaries; the repository core is documented in [`docs/CONTRACT_INTENT_WALLET.md`](docs/CONTRACT_INTENT_WALLET.md)
