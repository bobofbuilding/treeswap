...
- Deploy the implemented opaque ownership-handle service, same-origin SIWE-session edge, pre-session perimeter, and private wallet gateway with real D1/session wiring, a reviewed listener/CDN, private edge-to-coordinator TLS, separate key custody, handle/body-log suppression, exclusive use of the reviewed user-activated browser adapter, durable lifecycle reconciliation, two-provider finality evidence, and explicit approval boundaries. Early testnet must use the implemented no-stale-takeover single-replica fence on one owner-controlled shared POSIX volume; automatic or multi-host failover remains closed until a reviewed distributed fence and durable abuse control exist. The repository core is documented in [`docs/CONTRACT_INTENT_WALLET.md`](docs/CONTRACT_INTENT_WALLET.md)
