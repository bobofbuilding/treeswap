...
Before evaluation, the gate takes one exact immutable snapshot of the complete risk policy, request, BIT/inventory state, and at most 64 indexed price candidates. It does not repeatedly read caller-owned objects or use a caller-overridable array iterator. A getter, proxy, later mutation, extra field, or oversized price-source allowlist therefore cannot present loose limits during evaluation and stricter values when the reviewed policy digest is calculated.

It also pins both proxy and implementation bytecode hashes. An `Upgraded` event is an alert input, but each quote must re-read state rather than trusting that an event was observed. `buildBitRiskAttestation` v3 accepts only the exact request used by the original same-process evaluation and commits its digest plus the complete snapshot digest, reviewed chain, proxy, implementation, code hashes, decimals, pause flag, observation time, latest and finalized blocks, executable-price median/spread, implied quote price, exact quote/market deviation, inventory fee and reserve result, independent source set, exact source-policy digests and observation windows, the earliest safe expiry, and the complete risk-policy digest. Substituting the direction, net BIT amount, Lightning amount, snapshot, or policy invalidates the attestation.

The attestation expires at the earliest exclusive boundary among BIT-snapshot freshness, every surviving source's signed `validUntil`, and every surviving source's maximum-age window. `assertCurrentBitRiskAttestation` accepts only the original same-process attestation, the exact original request, a clock at or after evaluation and before expiry, and a caller-specified required validity window that does not exceed that boundary. A serialized copy cannot be treated as live evidence; the attestation is never timeless authorization.

The authenticated multipath blind-RFQ builder now requires one original current attestation for each distinct retained offer's exact direction, net BIT amount, and Lightning amount, and requires it to remain valid through the signed offer expiry. It also requires the attestation's complete policy digest to equal the risk-policy digest committed by the reviewed release; a genuine proof made under a weaker or substituted policy is ineligible. Offers without that exact proof are rejected individually before the deterministic solver and delivery-path minimums are applied. The received-set commitment includes each market-risk digest, reviewed policy digest, and expiry, so the user's exact selection signature covers the complete price proof without exposing private invoice fields. Coordinator schema v10 durably stores those three fields in the firm-offer record, refuses active schema-v9-or-earlier migration because the reviewed policy and contract-intent bindings cannot be reconstructed, and includes them in settlement execution-policy v3. The active daemon requires the firm offer, settlement, and same-process release context to agree on the nonzero reviewed policy digest and rejects missing, substituted, shortened, expired, or offer-incomplete market authority after restart.
