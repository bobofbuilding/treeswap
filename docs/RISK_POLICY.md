...
A future BIT/WBTC Uniswap v3 pool may contribute one request-sized source through the [BIT/WBTC market-reference boundary](./BIT_WBTC_MARKET_REFERENCE.md). It requires the exact policy-pinned BIT proxy and implementation runtime with an unpaused `BIT` / 18-decimal state, a finalized TWAP, active and wide-range liquidity floors, exact-direction executable probe, two-provider agreement, and a separately pinned WBTC/BTC peg conversion. Any BIT upgrade or unsafe token-state change invalidates the source. The pool and conversion feed cannot satisfy the three-source requirement by themselves, and the source is unavailable until the pool exists and completes its rollout gate.
