...
A future BIT/WBTC Uniswap v3 pool may contribute one request-sized source through the [BIT/WBTC market-reference boundary](./BIT_WBTC_MARKET_REFERENCE.md). It requires the exact policy-pinned BIT proxy and implementation runtime with an unpaused `BIT` / 18-decimal state, the canonical WBTC runtime with an unpaused `WBTC` / 8-decimal state, a finalized TWAP, active and wide-range liquidity floors, exact-direction executable probe, two-provider agreement, and a separately pinned WBTC/BTC peg conversion. Any BIT upgrade or unsafe BIT/WBTC token-state change invalidates the source. The pool and conversion feed cannot satisfy the three-source requirement by themselves, and the source is unavailable until the pool exists and completes its rollout gate.
...
The local monitor now requires fresh, short-lived EIP-712 observations from exact release- and gate-bound collectors across the BIT, price, finality, provider-quorum, Lightning, capacity, reconciliation, and audit domains. Its actual-gate campaign closes quotes, submits a guardian halt, verifies the gate closed, and emits the alert afterward. Signatures authenticate the configured collector claim but do not prove honest or independent operation. This proves the bounded repository path, not a continuously deployed monitor or public alert integration. See [Safety monitoring boundary](./MONITORING.md).
