...
The authenticated multipath blind-RFQ builder now requires one original current attestation for each distinct retained offer's exact direction, net BIT amount, and Lightning amount, and requires it to remain valid through the signed offer expiry. It also requires the attestation's complete policy digest to equal the risk-policy digest committed by the reviewed release; a genuine proof made under a weaker or substituted policy is ineligible. Offers without that exact proof are rejected individually before the deterministic solver and delivery-path minimums are applied. The received-set commitment includes each market-risk digest, reviewed policy digest, and expiry, so the user's exact selection signature covers the complete price proof without exposing private invoice fields. Coordinator schema v9 durably stores those three fields in the firm-offer record, refuses active v8 migration because the reviewed policy binding cannot be reconstructed, and includes them in settlement execution-policy v3. The active daemon requires the firm offer, settlement, and same-process release context to agree on the nonzero reviewed policy digest and rejects missing, substituted, shortened, expired, or offer-incomplete market authority after restart.
