...
The authenticated multipath blind-RFQ builder now requires one original current attestation for each distinct retained offer's exact direction, net BIT amount, and Lightning amount, and requires it to remain valid through the signed offer expiry. Offers without that proof are rejected individually before the deterministic solver and delivery-path minimums are applied. The received-set commitment includes each market-risk digest and expiry, so the user's exact selection signature covers the price proof without exposing private invoice fields. Coordinator schema v8 durably stores those two fields in the firm-offer record, refuses active v7 migration because the evidence cannot be reconstructed, and includes them in settlement execution-policy v2. The active daemon rejects missing, zero, shortened, expired, or offer-incomplete market authority after restart.
