...
The ownership service and gateway must be original factory products, share the exact deployment abort signal, match production/test mode, and have a one-to-one binding. Aggregate status exposes counts only and explicitly omits handles, request and contract-intent digests, quote or invoice material, wallets, and session digests. Local both-direction tests cover copied provenance, wrong wallet/session, accessor input, simultaneous claim, duplicate take, expiry, restart, status privacy, and durable-database exclusion of the handle.

This core does not verify a cookie, derive the session digest, implement CSRF or rate limiting, prove browser intent, make a network request, or persist an ownership handle. The future edge must derive the digest from its already verified server-side SIWE session and accept only the browser handle—not a caller-supplied request digest or preflight.
...
1. deploy the implemented ownership service and private gateway behind a SIWE-authenticated, CSRF-protected, rate-limited edge that derives the session digest server-side and accepts only the issued opaque handle; compose the implemented browser adapter as the only provider path under reviewed CSP/build controls, retain no handle/claim/body logs, and prove that a process, network, reload, or multi-tab retry cannot become another wallet request;
