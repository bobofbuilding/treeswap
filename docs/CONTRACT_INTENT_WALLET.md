...
All ownership handles, CSRF values, raw session tokens, session digests, gateway claim tokens, and active outcome state remain memory-only and are explicitly absent from aggregate status and the wallet SQLite journal. The separate abuse ledger stores only a domain-separated one-way commitment to the session digest plus bounded time/count fields; it stores none of those values or any wallet, request, quote, or invoice. Restart or lifecycle abort clears ephemeral state; the abuse count and clock high-water mark survive, and any durable gateway attempt remains no-resend reconciliation. Both-direction tests cover duplicate and wrong cookies, invalid chain/lifetime/timestamps, duplicate session rows, Origin/Fetch Metadata failures, wrong and overlong CSRF windows, wrong session, concurrent claim, exact and conflicting outcome replay, persistent per-session exhaustion, second-edge/direct ownership or ledger rejection, status/database privacy, and clock rollback.
...
This is a conservative single-replica control, not a distributed consensus fence. Deployment must mount the runtime and abuse-ledger directories from owner-controlled persistent storage with the required POSIX semantics, forbid automated stale-lock deletion, and alert for retained crash fences or a halted/full abuse ledger. Separate hosts, independent volumes, serverless replicas, or automatic failover remain unsupported until a separately reviewed distributed fence and distributed abuse policy exist. Because the repository still has no listener or trustworthy client-IP before the session query, the deployed CDN/listener must add its own coarser unauthenticated connection and request controls outside this process. Deployment must also wire the exact D1/session capability, prevent body and token logging across proxies/tracing/error reporting, protect and rotate the edge requester key separately from the coordinator response key, and repeat the implemented process-kill, disk-full, backup/restore and crash-fence campaigns on the real volume together with reload and multi-tab evidence.
...
1. deploy the implemented ownership service, same-origin SIWE-session edge, private durable abuse ledger, exclusive pre-session perimeter, conservative single-replica fence, and private gateway with real session storage, a reviewed outer CDN/listener, and private TLS composition; retain separate keys and no handle/claim/body logs, use owner-controlled persistent storage for the abuse ledger and one shared POSIX fence volume with no automatic stale takeover, compose the implemented browser adapter as the only provider path under reviewed CSP/build controls, repeat the repository `SIGKILL`, disk-full, backup/restore and retained-fence campaign on that volume, and prove that a process, network, reload, multi-tab retry, or edge restart cannot reset the rate limit or become another wallet request;
