...
1. deploy the implemented private gateway behind a SIWE-authenticated, CSRF-protected, rate-limited edge that accepts only an original server-staged intent ownership handle; compose the implemented browser adapter as the only provider path under reviewed CSP/build controls, retain no claim/body logs, and prove that a process, network, reload, or multi-tab retry cannot become another wallet request;
