...
`createContractIntentWalletSiweEdge` is a Node-only route core for the browser-facing hop. It accepts the exact `__Host-treeswap_session` cookie format already issued by TreeSwap, hashes it before one fixed read-only `auth_sessions` query, and accepts exactly one canonical active row. The row must contain the same token hash, a lowercase nonzero Ethereum wallet, chain ID 1, canonical UTC timestamps, no future creation time, and a lifetime of at most 24 hours. The raw cookie is retained only in short-lived module-private memory. An HMAC keyed by that cookie derives the opaque session digest; the browser cannot supply the wallet or digest.

The edge requires exact same-origin `POST` requests, an allowlisted canonical origin, `Sec-Fetch-Site: same-origin`, `Sec-Fetch-Mode: cors`, `Sec-Fetch-Dest: empty`, no `Sec-Fetch-User` or `Authorization`, `Cache-Control: no-store`, identity encoding, strict bounded JSON, unambiguous framing, a fixed five-second total body-read deadline with cancellation, and generic non-cacheable errors. Separate HMAC CSRF values bind the verified session to either the ownership handle or gateway claim and to the exact expiry. A fixed in-memory limit allows eight requests per verified session per 60 seconds, caps both rate entries and active claims at 128, and halts on clock rollback.
...
All ownership handles, CSRF values, raw session tokens, session digests, gateway claim tokens, and active outcome state remain memory-only and are explicitly absent from aggregate status and the wallet SQLite journal. Restart or lifecycle abort clears them; any durable gateway attempt remains no-resend reconciliation. Both-direction tests cover duplicate and wrong cookies, invalid chain/lifetime/timestamps, duplicate session rows, Origin/Fetch Metadata failures, wrong and overlong CSRF windows, wrong session, concurrent claim, exact and conflicting outcome replay, per-session exhaustion, second-edge/direct ownership rejection, status/database privacy, and clock rollback.

The edge now grants one exclusive provenance-bound lease to the perimeter described below. Once claimed, a retained direct edge reference or a second perimeter cannot issue, claim, inspect, or stop the edge. The edge itself still creates no listener or application route, and its current composition calls the private gateway core in the same process rather than proving a separate authenticated TLS hop.
...
This is a conservative single-replica control, not a distributed consensus fence. Deployment must mount the runtime directory from the same owner-controlled storage with POSIX-atomic directory creation for every possible replica, forbid automated stale-lock deletion, and alert for retained crash fences. Separate hosts, independent volumes, serverless replicas, or automatic failover remain unsupported until a separately reviewed distributed fence and durable authenticated-session abuse control exist. Because the repository still has no listener or trustworthy client-IP before the session query, the deployed CDN/listener must add its own coarser unauthenticated connection and request controls outside this process. Deployment must also wire the exact D1/session capability, prevent body and token logging across proxies/tracing/error reporting, protect and rotate the edge requester key separately from the coordinator response key, and retain real browser, process-kill, disk-full, backup/restore, reload, and multi-tab evidence.
...
1. deploy the implemented ownership service, same-origin SIWE-session edge, exclusive pre-session perimeter, conservative single-replica fence, and private gateway with real session storage, a reviewed outer CDN/listener, and private TLS composition; retain separate keys and no handle/claim/body logs, use one shared owner-controlled POSIX fence volume with no automatic stale takeover, compose the implemented browser adapter as the only provider path under reviewed CSP/build controls, and prove that a process, network, reload, or multi-tab retry cannot become another wallet request;
