...
The edge now grants one exclusive provenance-bound lease to the perimeter described below. Once claimed, a retained direct edge reference or a second perimeter cannot issue, claim, inspect, or stop the edge. The edge itself still creates no listener or application route. The D1 provider is not wired into a Sites route, and the current test composition calls both the provider and private gateway cores in process rather than proving either deployed authenticated HTTPS hop.
...
This is a conservative single-replica control, not a distributed consensus fence. Deployment must mount the runtime and abuse-ledger directories from owner-controlled persistent storage with the required POSIX semantics, forbid automated stale-lock deletion, and alert for retained crash fences or a halted/full abuse ledger. Separate hosts, independent volumes, serverless replicas, or automatic failover remain unsupported until a separately reviewed distributed fence and distributed abuse policy exist. Because the repository still has no listener or trustworthy client-IP before the session read, the deployed CDN/listener must add its own coarser unauthenticated connection and request controls outside this process. Deployment must wire the provider to the existing authoritative D1 binding through one reviewed server-only route, suppress request/response bodies and token material across proxies, tracing, analytics, and error reporting, keep the two session-reader keys separate from both wallet-gateway keys, and repeat the implemented process-kill, disk-full, backup/restore and crash-fence campaigns on the real volume together with D1 outage, key-rotation, reload, and multi-tab evidence.
