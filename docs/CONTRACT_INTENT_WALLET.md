...
This module still creates no listener or application route. Its fixed authenticated-session rate state and claim fence are single-process, and its current composition calls the private gateway core in the same process rather than proving a separate authenticated TLS hop. Because the core has no trustworthy client-IP or perimeter identity before session lookup, deployment must add a coarse unauthenticated request/body limit at the CDN or listener before D1. It must also wire the exact D1/session capability, prevent body and token logging across proxies/tracing/error reporting, protect and rotate the edge requester key separately from the coordinator response key, prove one replica or introduce a separately reviewed shared fence and durable authenticated-session abuse control, and retain real browser, process-kill, disk-full, backup/restore, reload, and multi-tab evidence.
...
1. deploy the implemented ownership service, same-origin SIWE-session edge core, and private gateway with real session storage and private TLS composition; retain separate keys and no handle/claim/body logs, enforce one replica or a separately reviewed shared fence and durable abuse control, compose the implemented browser adapter as the only provider path under reviewed CSP/build controls, and prove that a process, network, reload, or multi-tab retry cannot become another wallet request;
