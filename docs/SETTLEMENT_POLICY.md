...
Status: deterministic policy and boundary-test harness, local two-direction execution-client reorg evidence, a live rapid-block LND cutoff campaign, one integrated local EVM/LND deadline campaign, and the credentialed pinned-live-BIT-fork repeat pass. Public-testnet timing through independently operated providers remains required before funding.
...
The ordinary campaign uses mock BIT and simulated local EVM finality. Its credentialed repeat on the pinned live BIT fork now passes. Genuine public-testnet finality through independent providers, production-like mempool congestion, and independently operated infrastructure remain testnet launch gates.
...
`test:live-bit-cross-chain-deadlines` is the credentialed repeat of that exact sequence against the pinned BIT proxy fork. It requires clean published `main`, exact block/proxy/implementation/code-hash/slot/token-configuration provenance, live LND, the same deadline policy, and a distinct evidence schema that cannot be produced by mock mode. Clean published source `9ba7d89a8b555f749c2a0f9a450effff340d6833` passed both directions; the independently revalidated outer evidence digest is `0xb9711ca69dcdad37f41de2ca2bd321534f0404bebd8d6facf89ad2f9f8cad70c` and the nested deadline digest is `0xe2c2efc01ae1f8e04f9765e21e1069ff9f071e87b5fea3e313e3d5816bd10f8e`. This closes the pinned-fork timing sub-gate only: public-testnet finality, independent providers and solvers, production infrastructure, and funding authorization remain absent. See [Pinned live-BIT cross-chain deadline evidence](./LIVE_BIT_CROSS_CHAIN_EVIDENCE.md).
