...
- [ ] Operate atomic persistent RFQ quotas, cancellation sequence, capacity epochs, solver reliability, idempotent Lightning requests, and one-shot payment authorization.
  ...
  - [x] The exact pinned coordinator image passes schema-v4 parity, verified non-overwriting backup/fresh-path restore, corruption and unknown-schema refusal, SIGKILL/WAL recovery, v2 migration, fail-closed v3 capability migration, solver-capability, authenticated-endpoint, and capacity-reader verification, plus a real bounded-filesystem `SQLITE_FULL` rollback. The current 37-test pinned-runtime job passed on published commit `67655f859ec70c191501d073e75cba808ce06def` and [hosted CI](https://github.com/bobofbuilding/treeswap/actions/runs/32323948108).
  ...
  - [ ] Connect the complete solver daemon; deploy the authenticated capability endpoint and reader protocols with independent operators; deploy the persistence service, encrypted retained backups, alerts, and deployed-volume recovery drills; reproduce the EVM evidence on controlled forks and public testnet.
