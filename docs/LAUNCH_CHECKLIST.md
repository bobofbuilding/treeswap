...
- [ ] Operate atomic persistent RFQ quotas, cancellation sequence, capacity epochs, solver reliability, idempotent Lightning requests, and one-shot payment authorization.
  ...
  - [x] The exact pinned coordinator image passes schema-v4 parity, verified non-overwriting backup/fresh-path restore, corruption and unknown-schema refusal, SIGKILL/WAL recovery, v2 migration, fail-closed v3 capability migration, solver-capability and authenticated-endpoint verification, and a real bounded-filesystem `SQLITE_FULL` rollback. The current 36-test pinned-runtime job passed on published commit `f474e9c577f9c4e70183275f693ce89216e24032` and [hosted CI](https://github.com/bobofbuilding/treeswap/actions/runs/32318279120).
  ...
  - [ ] Connect the complete solver daemon, deploy the authenticated capability endpoint and concrete capacity readers, deploy the persistence service, encrypted retained backups, alerts, and deployed-volume recovery drills; reproduce the EVM evidence on controlled forks and public testnet.
