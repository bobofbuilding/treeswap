...
- [ ] Operate atomic persistent RFQ quotas, cancellation sequence, capacity epochs, solver reliability, idempotent Lightning requests, and one-shot payment authorization.
  ...
  - [x] The exact pinned coordinator image passes schema-v4 parity, verified non-overwriting backup/fresh-path restore, corruption and unknown-schema refusal, SIGKILL/WAL recovery, v2 migration, fail-closed v3 capability migration, solver-capability verification, and a real bounded-filesystem `SQLITE_FULL` rollback. The current 27-test pinned-runtime job passed on published commit `794b9f3bedb21dd6fe39dae0d7a10a5e94289899` and [hosted CI](https://github.com/bobofbuilding/treeswap/actions/runs/32312491429).
  ...
  - [ ] Connect the complete solver daemon, deployed authenticated capability transport, and concrete capacity readers; deploy the persistence service, encrypted retained backups, alerts, and deployed-volume recovery drills; reproduce the EVM evidence on controlled forks and public testnet.
