...
- [ ] Operate atomic persistent RFQ quotas, cancellation sequence, capacity epochs, solver reliability, idempotent Lightning requests, and one-shot payment authorization.
  ...
  - [ ] Connect the planner to reviewed private-packet/secret rehydration plus the existing finality/dispatch modules; deploy the authenticated capability endpoint and reader protocols with independent operators; deploy persistence, encrypted retained backups, alerts, and deployed-volume recovery drills; reproduce the EVM evidence on controlled forks and public testnet.
