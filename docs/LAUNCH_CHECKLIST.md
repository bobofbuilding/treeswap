...
- [ ] Run an isolated Bitcoin/LND regtest adapter: standard and hold invoices, exact BOLT 11 decode, delayed/fast blocks, accepted-HTLC cutoff, timeout, LND restart, force close, credential rotation/revocation, and negative macaroon permissions.
  ...
  - [x] Published checkpoint `e6ebaa72f6b746c38e29b350322d880b8a1d9f28` passed all 24 local campaigns and [hosted CI](https://github.com/bobofbuilding/treeswap/actions/runs/32304969494); independently rebuilt local-only evidence digest `sha256:5f58472ed569fb40ee1abbe9e29c0ee2e1e0c11ee19cf9c7d43bfa6cfcc2bd8b`.
  - [x] The full 3,600-second no-progress interval passed with 3,604 monotonic seconds, 119 continuous observations, restart persistence after 1,813 seconds, deterministic rejection, and zero target-payment dispatch. Stateless initialization, full force-close recovery, genuine unsynced catch-up, daily-cap rollover, directional exhaustion/rebalancing, real TLS and overlap credential rotation/revocation, no-route failure, compressed-threshold stale-header rejection, and both exact/same-hash duplicate paths also pass with one-or-zero dispatch as required.
...
- [ ] Operate atomic persistent RFQ quotas, cancellation sequence, capacity epochs, solver reliability, idempotent Lightning requests, and one-shot payment authorization.
  ...
  - [x] The exact pinned coordinator image passes schema-v3 parity, verified non-overwriting backup/fresh-path restore, corruption and unknown-schema refusal, SIGKILL/WAL recovery, v2 migration, and a real bounded-filesystem `SQLITE_FULL` rollback on published commit `dbc9f1daa205549a0af559bc024c40b347ca8ecd` and [hosted CI](https://github.com/bobofbuilding/treeswap/actions/runs/32311057995).
  - [ ] Connect the complete solver daemon and authenticated capability verifier; deploy the persistence service, encrypted retained backups, alerts, and deployed-volume recovery drills; reproduce the EVM evidence on controlled forks and public testnet.
