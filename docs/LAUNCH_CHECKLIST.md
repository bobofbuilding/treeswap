...
- [ ] Run an isolated Bitcoin/LND regtest adapter: standard and hold invoices, exact BOLT 11 decode, delayed/fast blocks, accepted-HTLC cutoff, timeout, LND restart, force close, credential rotation/revocation, and negative macaroon permissions.
  ...
  - [x] Published checkpoint `1662da743736692bfef0681586cf8e989ae5e190` passed all 15 local campaigns and hosted CI; local-only evidence digest `sha256:707e75701389c0c46c64be7a59503b1eff477a7ee2a18492d831be04516b684d`.
  - [ ] Complete the remaining production-duration delay, force-close, genuinely unsynced-node catch-up, real TLS/credential rotation, and directional-capacity fault matrix. Genuine no-route failure, compressed-threshold stale-header rejection, and both exact/same-hash duplicate paths now pass with one-or-zero dispatch as required.
