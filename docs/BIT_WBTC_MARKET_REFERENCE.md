...
The pool-derived result is one `bit-wbtc-twap-probe` signal. The general risk gate requires its original module-private provenance to match the exact current direction, net BIT amount, and Lightning amount, then binds that request into risk-attestation v3. Every eligible price signal also binds its validity, source-policy digest, evidence digest, venue ID, control-domain ID, and executable depth. The attestation expires at the earliest BIT-snapshot or signed-source freshness boundary. The blind-RFQ builder accepts an offer only when the original in-memory attestation covers its exact economic request through the complete signed quote window and its policy digest equals the reviewed release's risk-policy digest. Coordinator schema v10 persists the selected risk digest, reviewed policy digest, and expiry for restart-safe enforcement. The source-policy digest must be explicitly allowlisted by that same signed release risk policy. Duplicate sources, venues, or control domains do not count toward the minimum source total.
