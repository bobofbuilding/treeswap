...
- the exact direction and both asset amounts are covered by the executable probe; Lightning → BIT is an exact-output BIT buy and BIT → Lightning is an exact-input BIT sale, so neither a one-leg shallow quote nor a reversed quoter result is accepted;
...
5. exact WBTC token, WBTC/BTC feed proxy, underlying aggregator, code hashes, heartbeat/freshness, and depeg response;
