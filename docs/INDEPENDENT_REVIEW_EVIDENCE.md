...
Release-candidate schema v6 is the only accepted candidate artifact. Candidate v1/template v1 are rejected because they accepted unsigned review hashes; candidate v2/template v2 lacked the separate provenance-bound operational-readiness package; candidate v3 lacked the exact public adoption-policy boundary; candidate v4 lacked the separate provenance-bound final [qualification review](./QUALIFICATION_REVIEW_EVIDENCE.md); and candidate v5 omitted the exact monitor upstream-record, operational safety-monitor-policy, and isolated gate-confirmer-binding commitments.
