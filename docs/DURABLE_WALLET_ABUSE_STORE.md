...
unsafe paths and permissions, symlinks, policy tampering, and exclusion of the
raw session digest from the database. The both-direction wallet path uses a real
private file and checks that session tokens, wallets, and request digests are
absent from it.
...
owner-controlled volume, backup/restore and disk-loss drills, alerting for
ledger halt or capacity exhaustion, reviewed retention and access policy, the
existing no-stale-takeover replica fence, and an independently reviewed
