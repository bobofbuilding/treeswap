...
| TS-H07 | Least-privilege isolated regtest adapters, signed actions, durable replay journal, overlap credential rotation/revocation, lost-response coordinator recovery, and secret-free audit implemented | Real TLS rotation, production-duration timing, deployed isolation/monitoring, independent review |
...
**Status:** Exact RPC manifests, expiring credential/TLS isolation, overlap credential rotation and live old-root revocation, TLS-pin mismatch, hold-invoice terminal faults, accepted-state restart, live fee/amount/in-flight caps, channel-offline recovery, signed intent-bound authorization, durable replay journal, payer- and invoice-side coordinator lost-response recovery, and secret-free audits pass isolated regtest; real TLS rotation and production-duration timing remain deployment gates
