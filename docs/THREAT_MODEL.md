...
| TS-M09 | Immutable escrows, constrained gate, sealed registry, strict deployment policy, finalized topology observer, provider/reviewer-signed manifest promotion, three-role signed service-isolation evidence, five-role signed release authorization, same-process two-party-reconciled live activation, and provenance-bound restart recovery implemented | Deployed role-separated services and multisigs, scoped credentials and hardware owners, independently operated providers/reviewers/runtime attesters, persistent activation/recovery coordinator, continuous event monitoring, and external alert delivery |
...
Restart recovery no longer depends on an in-memory funding context surviving an incident. A separate recovery-only activation reopens the exact raw release and approval evidence, rereads current canonical code, proxy implementation, wallet ownership, registry membership, token identity, and reconciled escrow balances through the candidate-bound provider quorum, and accepts closed/emergency/paused or below-reserve state only because it can create no new exposure and can never dispatch Lightning. Schema v7 permanently binds each accepted settlement—before reservation or action—to one unique firm offer, that offer's historical solver capability, the reviewed release, and the exact daemon-evidence policy. A fresh rotated capability for the same solver may recover the old record, but cannot rewrite its authority. Copies, cross-release replay, method substitution, late binding, duplicate selected offers, unsafe legacy migration, and recovery attempts at a Lightning-dispatch step fail closed. Per-action EVM or terminal work still requires live leadership and fresh dual-signed evidence. The packaged coordinator does not yet run this loop, so deployed restart and incident drills remain mandatory.
