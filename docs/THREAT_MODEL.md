...
**Status:** Resolved for EOA account metadata and fail-closed deployment capability; live durable-store operations and independent identity/privacy review remain launch gates
...
- Cross-origin session mutation, expired session use, notification access from a different wallet, and an absent or partial durable schema. **Complete in policy and worker-boundary tests; deployed store access, atomicity, monitoring, and purge evidence remain launch gates.**
