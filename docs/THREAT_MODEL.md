...
Local qualification also no longer trusts pre-existing Docker state. A failed or interrupted force-close run can leave valid but uneconomic 330-sat LND anchor sweeps or incomplete pending-close state in persistent volumes, causing a later run either to fail for the wrong history or to inherit evidence it did not create. The sealed runner destroys only the named disposable `treeswap-regtest` volumes before the first campaign and again on every exit; either destruction failure invalidates the run. This makes local evidence hermetic, but it does not prove production node longevity. Deployed operators still need separate pending-close and sweep-queue alerts, escalation for time-sensitive HTLCs and overdue non-anchor outputs, and a reviewed policy for uneconomic confirmed anchors.
