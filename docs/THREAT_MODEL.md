...
`lib/blind-rfq.mjs` now requires the exact authenticated selection, the same module-private capability proof used during collection, and the exact active coordinator records before private disclosure. Schema v5 reserves both scarce resources atomically: gross BIT plus inbound Lightning for Lightning → BIT, or outbound Lightning plus maximum routing headroom for BIT → Lightning. Every capacity read reconciles the stored counters against the active offer ledger. It then binds one exact private-request and executable-offer digest pair and exact finalization time; only byte-identical retry succeeds. Copied provenance, fake or method-replaced stores, stale capacity, cancellation, expiry, same-ID or partial-ledger mutation, a second invoice, and connection races fail closed. Older schemas with active one-sided commitments refuse migration.
