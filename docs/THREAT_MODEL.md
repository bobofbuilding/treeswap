...
**Status:** Exact RPC manifests, expiring credential/TLS isolation, live root-key revocation, hold-invoice terminal faults, accepted-state restart, signed intent-bound authorization, directional value caps, durable replay journal, coordinator lost-response recovery, and secret-free audits pass isolated regtest; overlap rotation, invoice-side ambiguous-response, and chain/channel failure drills remain deployment gates
