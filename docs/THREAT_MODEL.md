...
For BIT → Lightning, a successful signed tracking lookup now returns the exact hash-bound preimage only to the in-memory coordinator caller, including after a later coordinator restart. Missing or mismatched preimages fail closed, while non-success states expose none. The live lost-response campaign proves the invoice and both raw/textual preimage forms are absent from SQLite, WAL, and shared memory. A durable-state daemon planner separately enforces restart/reconciliation priority and direction-specific payment/claim order without gaining RPC, signer, credential, or finality authority. Full unattended execution remains disabled until reviewed private-packet/secret and signed-deadline rehydration are connected to that planner.
