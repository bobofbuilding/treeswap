...
Operational release inputs no longer enter through opaque operator-supplied support and loss hashes. A separate five-role operational-readiness verifier requires exact backup, alert, incident, monitoring, privacy-retention, provider, reconciliation, solver, support, loss-allocation, and qualification artifacts; the complete immutable incident-drill set; at least two alert channels; one primary plus two retained observers per drill; bounded timestamps; and five distinct EIP-712 signers across at least two organization commitments. Release-candidate v3 re-verifies that package, matches its Lightning and incident roles to the release policy, matches its monitor to the signed upstream roster, forbids overlap with deployment or independent-review authorities, and reconciles campaign alerts, artifacts, liabilities, and drill digests exactly. The signatures and commitments still do not prove the drills occurred, alerts were delivered, backups are recoverable, support is staffed, or organizations are genuinely separate; retained external evidence and witnessed drills remain launch gates. See [Operational readiness evidence](./OPERATIONAL_READINESS_EVIDENCE.md).
