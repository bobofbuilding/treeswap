...
**Status:** Exact RPC manifests, expiring credential/TLS isolation, live root-key revocation, TLS-pin mismatch, hold-invoice terminal faults, accepted-state restart, live fee/amount/in-flight caps, channel-offline recovery, signed intent-bound authorization, durable replay journal, coordinator lost-response recovery, and secret-free audits pass isolated regtest; real rotation, invoice-side ambiguous-response, and remaining chain/channel drills remain deployment gates
