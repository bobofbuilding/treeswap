...
Funding cannot be enabled by a configuration boolean. The local release boundary binds the deployment, policies, evidence, reviews, independent-operator counts including monitors, limits, reserves, and excluded features into one expiring record. Controller and guardian ERC-1271 approvals are checked at the same canonical finalized block through a configured provider quorum and against reviewed wallet code hashes; three separate operating/review roles sign the same EIP-712 digest. The production funding path must then rebuild the provenance-bound candidate and provider set in one uninterrupted process, re-verify all five approvals, verify a fresh reconciliation signed by the Lightning operator and security reviewer, and obtain exact provider agreement on the current canonical gate, registry, escrow, BIT implementation, pause state, accounting, reserves, and caps. The resulting capability and runtime snapshot are a module-private pair; copies, manufactured JSON, restart, reconciliation expiry, or provider disagreement cannot authorize. A separately named bootstrap mode is hard-capped at 500 sats per swap and 10,000 sats per day; ordinary operator-testnet funding requires the signed seven-day campaign. Both require loss allocation, support, findings, operating evidence, and five report digests derived from a fresh five-reviewer signed package. This prevents self-asserted release activation but does not prove provider or runtime-attester independence, hardware custody, evidence quality, or reviewer independence; those remain external gates. See [Public-testnet release activation](./PUBLIC_TESTNET_RELEASE_ACTIVATION.md).
...
Operational release inputs no longer enter through opaque operator-supplied support, loss, or service-isolation hashes. A separate three-role service-isolation verifier requires twelve exact service roles, unique trust domains, exact ingress and encrypted transport, least-privilege credential classes, unique credential sets, bounded rotation windows, and committed network/deployment evidence. A separate five-role operational-readiness verifier requires exact backup, alert, incident, monitoring, privacy-retention, provider, reconciliation, service-isolation, solver, support, loss-allocation, and qualification artifacts; the complete immutable incident-drill set; at least two alert channels; one primary plus two retained observers per drill; bounded timestamps; and five distinct EIP-712 signers across at least two organization commitments. Operational-readiness v2 can be prepared only from live isolation-verifier provenance. Release-candidate v3 binds both isolation evidence and participant-set digests into every operational composite, requires exact Lightning/security role continuity, keeps the infrastructure signer separate from operational/deployment/review/release authorities, and matches alerts, artifacts, liabilities, and drill evidence. The signatures and commitments still do not prove deployed separation, scoped secrets, drills, alerts, backups, support, or organizational independence; retained external evidence, direct infrastructure inspection, and witnessed drills remain launch gates. See [Service isolation evidence](./SERVICE_ISOLATION_EVIDENCE.md) and [Operational readiness evidence](./OPERATIONAL_READINESS_EVIDENCE.md).
