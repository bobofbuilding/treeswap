...
**Status:** Canonical/finalized escrow gate, one-shot Lightning revalidation, live rapid-block 24-block cutoff, integrated local two-direction EVM/LND deadline proof, exact EVM claim outbox, two-provider read-only agreement, and local plus pinned live-BIT fork reorg/replacement campaigns implemented; the fail-closed combined pinned live-BIT-fork runner is implemented, while its credentialed execution and public-testnet campaigns through independent providers remain deployment gates
