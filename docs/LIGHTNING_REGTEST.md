...
`regtest:solver-node-proof-smoke` bakes two disposable, time-bounded, exact-URI credentials under separate root-key IDs: Bob may call only `SignMessage`, while Alice may call only `VerifyMessage`. Bob signs four fresh exact `TreeSwap solver capability v1` challenges, Alice must independently recover Bob's exact compressed node public key, and each mutated challenge must be invalid. Cross-role RPCs must fail specifically for authorization. Both credential files and root-key IDs are removed after the campaign. This proves the local LND node-control mechanism; the separately tested endpoint and reader protocols remain undeployed.
