...
`regtest:stale-chain-smoke` reads the pinned LND node's real `wallet_synced`, height, and `best_header_timestamp`, then gives a disposable payer adapter a read-only baseline observation. After a deliberate no-block interval crosses its compressed one-second limit, the adapter must reject an exact signed payment specifically because the same header made no local progress; read-only tracking through the normally configured adapter must prove the payment was never dispatched. This remains deterministic when rapid regtest mining has placed the reported header timestamp ahead of wall time. The normal adapter uses a 3,600-second no-progress/age ceiling, regtest separately allows its synthetic timestamp up to 7,200 seconds ahead, and the same invoice remains decodable there. Three consecutive warm-state runs pass without emitting invoices, hashes, macaroons, or preimages.
