...
Until those gates pass, the implemented client and consumer are repository evidence only. They cannot fund a pool, open the bridge, pay an invoice, broadcast an EVM transaction, or settle a swap. The missing consumer must still construct and verify the actual direction-specific escrow quote, deadlines, signatures, calldata, wallet action, confirmed reservation, and private packet from the exact durable settlement.
