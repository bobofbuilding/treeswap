...
- one attestation per reviewer, ordered by canonical role, containing only `role`, `reviewerId`, `signer`, the signed `attestedAt`, and `signature`. The timestamp must follow that reviewer's report and remain inside its validity window.
