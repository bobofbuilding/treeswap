...
1. `application-security-reviewer`, covering the closed-test lock, signed request/response binding, read-only D1 query, fail-closed behavior, credential overlap, privacy, replay limits, and absence of browser, wallet, Lightning, settlement, or funding authority; and
2. `platform-data-isolation-reviewer`, covering authoritative D1 binding, server-only runtime values, lack of application body logging, cache and disclosure headers, inert initialization, isolate/version limitations, and the external deployment evidence that remains missing.
...
Repository review does not verify Sites configuration. The next repository gate is the two-operator [wallet-session route deployment preflight](./WALLET_SESSION_ROUTE_DEPLOYMENT_PREFLIGHT.md), which binds a short-lived, owner-only, non-production, no-body-capture plan to this original verified review without granting deployment authority. Before even a closed funded test, separate retained post-deployment evidence must still prove private access, generated and separated route keys, non-production D1 use, CDN and application body-log suppression, cache behavior, exact version cutover and retirement, pinned wallet-edge HTTPS, external admission halt monitoring, D1 outage and rotation drills, backup/restore and purge controls, and single-replica abuse-ledger recovery. Those controls then require the broader qualification review, operational evidence, release approvals, and external review already defined by the production checklist.
