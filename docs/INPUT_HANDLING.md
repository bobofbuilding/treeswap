...
Audit records are serialized as bounded JSON objects so a newline cannot forge another record. Production log sinks must preserve the JSON structure and restrict access because escaping does not make sensitive data public-safe.
