...
Every LND node requires exactly two collectors with different IDs, Ed25519 keys, exact-role macaroons, supervisor identities, and failure domains. Each v2 signature binds the opaque node commitment, collector ID, status, observation time, expiry, complete evaluator digest, one timestamp-independent aggregate-state digest, and sorted reason codes. The aggregate-state digest commits to the block height, policy, close/HTLC/limbo/sweep totals, anchor bounds, status, and reason codes without exposing Lightning identifiers. Attestations live for at most 60 seconds; the recommended production lifetime is 30 seconds on a schedule of 10 seconds or less. The verifier accepts exactly one fresh report from each configured public key, requires both state digests to match, and requires their observation times to remain inside the configured clock-skew window. A missing, duplicate, unknown, stale, future, malformed, wrong-node, wrong-key, state-disagreeing, time-skewed, or unsafe report yields one `unsafe` `lightning-node` observation. Legacy v1 attestations fail closed.
...
`collector-pair.json` contains only `{ "attestations": [...], "now": 0 }`; replace `now` with the verifier's current Unix time. The quorum output omits signatures and all raw LND data, retaining only collector/public-key/state/evidence/attestation digests, status, and time. The verifier output must immediately replace the generic monitor's `lightning-node` observation. If collection or delivery produces no pair, invoke the verifier with the incomplete set so absence becomes an explicit halt; never reuse the previous healthy quorum.
...
`npm run regtest:force-close-smoke` creates two distinct exact-role macaroons and Ed25519 identities, runs both hardened one-shot containers against pinned LND REST, verifies a signed same-state healthy quorum, force-closes the only payer channel, and requires both signed collectors plus the adapter to halt before any new dispatch. It advances through the node-reported CSV maturity, requires all non-anchor and pending-close exposure to clear, opens a fresh balanced channel, and requires a new same-state healthy quorum. Service may recover with only anchors that meet every bound. Unit campaigns separately prove that two individually healthy signatures from different block/state snapshots or outside the allowed observation window halt rather than form quorum. Reports and quorum evidence remain aggregate-only and feed the same `lightning-node` safety observation used by the gate monitor.
