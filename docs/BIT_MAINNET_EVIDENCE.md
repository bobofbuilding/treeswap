...
Before making an RPC request, v3 requires the canonical HTTPS TreeSwap origin, a clean `main` checkout, and an exact commit matching the current remote `origin/main`. Each operator must supply a stable nonzero `bytes32` provider-identity commitment and a credential-free display label. The observation has one exact canonical shape, recomputes its own safety result, rejects unknown or endpoint-bearing fields, and binds every read to one canonical finalized block. After the anchored reads, it rechecks the chain ID, finalized head, and exact target block; upstream rotation, same-height finality conflict, finality regression, lost finality, or target replacement rejects the capture.
...
Do not copy this document or a verification summary into a production manifest automatically. BIT manifest derivation requires two authenticated provider observations from the same published commit, both provider signatures, the complete two-role signed review package, both source bundles and compiler artifacts, live role and upgrade-authority evidence, the controlled reorg campaign, and closure of every independent-review finding. The guarded [reviewed BIT deployment manifest](./BIT_REVIEWED_MANIFEST.md) now preserves that exact live provenance in a short-lived, non-authorizing token-deployment handoff. It is not the TreeSwap contract deployment manifest and cannot authorize a testnet or mainnet release.
