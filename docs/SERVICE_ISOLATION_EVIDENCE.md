...
Operational-readiness schema v4 includes one exact `serviceIsolation` artifact plus two explicit gate-confirmer service assignments. Preparing or verifying that five-role package requires the original isolation record, policy, and attestations. The isolation package is re-verified in the same process, its deployment and validity fields must match, and its derived evidence digest must equal the operational artifact. The confirmer assignments must reproduce the exact asset-verifier and EVM-finality-authorizer service identities, trust domains, credentials, network policies, and deployment evidence. A copied JSON verification or invented service binding has no provenance and is rejected.
