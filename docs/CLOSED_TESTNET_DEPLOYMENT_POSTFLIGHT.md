...
This postflight proves exact transaction history plus finalized empty state. [Deployment-manifest promotion](./DEPLOYMENT_PROMOTION.md) v2 now re-verifies the complete postflight and binds its exact record and policy digests. Release authorization v2 requires those postflight and promotion digests in both the signed record and signed policy, and runtime funding snapshots must retain them. Real provider independence, hardware custody, Safe policy, monitor delivery, public-testnet operations, incident drills, and external review remain evidence gates.
