...
Status: one bounded fail-closed monitoring cycle, release-bound signed collector observations, and a local actual-gate halt campaign are implemented. A continuously scheduled deployment, independently operated collectors, redundant guardian transaction delivery, public alert routing, retention, and operator drills remain funding gates.
...
`lib/safety-observation-attestation.mjs` requires an exact, maximum-seven-day policy bound to the release-record digest, chain, deployed gate, freshness limit, and one distinct EIP-712 collector identity and signer for every domain. Each observation signature binds that policy digest, release digest, collector, kind, status, evidence digest, observation time, and expiry. The monitor accepts only the original same-process result of signature verification under the exact configured policy digest. Unsigned JSON, copied verified objects, changed policy, reused collector identities or signers, expired signatures, and freshness settings that differ from policy fail closed.

After authentication, every monitor input deliberately contains only four fields: fixed kind, `healthy` or `unsafe` status, observed time, and a lowercase evidence digest. Missing, duplicate, future, stale, expired, unsafe, malformed, extra-field, unsigned, or wrong-policy input produces fixed reason codes and takes the halt path. The policy digest is included in the evidence-set and alert commitments. Raw invoices, preimages, signatures, keys, RPC URLs, addresses, and remote error text are never copied into an alert.
...
The campaign deploys the actual `TreeSwapOpenGate` with distinct test-only controller and guardian contract callers, opens it through its real 24-hour delay, creates a release- and gate-bound policy with eight distinct test collector signers, verifies their short-lived EIP-712 observations, injects an unsafe BIT observation, closes quote issuance, submits the guardian halt, verifies the gate is closed, and only then records alert delivery. A healthy cycle cannot reopen it. An observation carrying an extra invoice-like field loses its same-process signature provenance, fails closed, and cannot copy that field into the alert.

The deterministic campaign digest is `0xb6f619fe9e51a936f66b6c84a6229a05ad134a5e8e96ed41ca0902220f33aa36`. The contract suite separately proves an emergency halt blocks new reservations/opens while existing claims, refunds, and solver withdrawals remain callable.
...
This is a local one-process harness with test collector keys, a test-only alert sink, and a local guardian signer. A valid signature authenticates the configured collector's claim; it does not prove the collector is honest, online, independent, or observing the correct private upstream system. This is not a continuously scheduled service, independent observation network, public paging provider, production multisig, redundant transaction broadcaster, or incident drill. A process that is not running cannot submit a halt; production therefore also needs supervised redundant instances and a short reviewed gate-attestation lifetime that bounds monitor-outage exposure.
