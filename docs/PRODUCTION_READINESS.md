...
- [ ] Inject delayed and fast Bitcoin blocks, LND restart, lost responses, idempotent retry, force close, unsynced state, TLS pin change, credential rotation, and credential revocation. Accepted-state LND restart, terminal no-route failure, exact and same-hash duplicate rejection, one-dispatch payer and invoice recovery, root-key revocation, live in-flight saturation, channel-offline recovery, TLS-pin mismatch, genuine unsynced-node catch-up, and compressed-threshold stale-header rejection now pass.
  ...
  - [x] A real no-block interval ages LND's best-header timestamp past a compressed limit; the adapter independently requires chain sync, wallet sync, and a fresh header, rejects before dispatch, and read-only tracking proves no payment exists.
  ...
  - [ ] Prove force-close recovery, production-duration block delay, real certificate rotation, and overlap-credential rotation.
...
- [ ] Produce a secret-free evidence bundle from the final published release commit containing versions, configuration hashes, test results, and timestamps—never macaroons, invoices, or preimages.
  ...
  - [x] Published checkpoint `9f931e34f1301802273954cc5393f646e3e5cbeb` passed all 16 local campaigns and hosted CI; its validated local-only digest is `sha256:1443a7f8625d6d6ac92eb4c9f74407fe8042e59882025adbc97782c99ebb537e`.
