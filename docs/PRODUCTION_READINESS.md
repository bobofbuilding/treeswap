...
- [ ] Operate a solver daemon that quotes, reserves, waits for finality, performs the exact Lightning action, relays the preimage, reconciles, and halts on any mismatch.
  ...
  - [ ] Deploy that endpoint protocol with concrete onchain/Lightning capacity readers; connect the complete solver state machine, deploy the reviewed persistence service, and run deployed-volume recovery plus real multi-instance drills.
