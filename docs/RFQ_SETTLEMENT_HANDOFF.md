...
The [contract-intent wallet boundary](./CONTRACT_INTENT_WALLET.md) now supplies the non-dispatching validation core for the exact request, wallet outcome, transaction or same-intent replacement, receipt, direction-specific reservation event, finality, and reorg state. Its separate private SQLite journal durably claims the attempt and records only original core artifacts; restart is reconciliation-only and never authorizes resend. The core calls no wallet or provider, the journal is not yet composed into the persistent coordinator or a fixed dispatcher, and two-observation agreement remains repository evidence rather than independent canonical finality.

Before even capped funded testnet use, the remaining consumer must add the fixed wallet and authenticated provider adapters, compose the journal and complete transaction lifecycle into the durable coordinator, and independently confirm through two genuinely operated providers that the exact durable quote ID and contract intent are canonical and finalized in the reviewed escrow. Only then may it permit the direction-correct Lightning action through the existing active daemon and private-packet controls.
