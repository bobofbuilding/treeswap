...
Status: the exact second user authorization and its durable settlement record are committed atomically in the local coordinator. A same-process consumer derives the exact direction-specific contract typed data, approved deadlines, required EOA signatures, and zero-ETH calldata from that original RFQ provenance. The capability-bound authenticated solver-signing client and durable repository-only provider obtain the selected solver's exact EIP-712 signature with ambiguity-safe replay. A separate wallet core prepares and reviews that exact transaction, durably claims it, preserves and verifies a reported hash or same-intent replacement, and classifies exact reservation receipts and two-observation agreement. A Node-only private gateway core now binds one short-lived SIWE-edge request to that durable claim and permits idempotent recording of only its exact wallet outcome. Coordinator schema v10 independently verifies and durably binds the signed contract intent before any execution-policy binding, reservation, private packet, evidence request, or action can use it. No SIWE edge, browser adapter, gateway listener, independently operated solver listener, or Ethereum provider is deployed, no wallet is called through the gateway, and no transaction is confirmed onchain. A browser authorization response still proves only that the non-dispatching settlement record exists and grants no EVM reservation, Lightning payment, funding, claim, or settlement-dispatch authority. Funded operation remains closed.
...
The [contract-intent wallet boundary](./CONTRACT_INTENT_WALLET.md) now supplies the validation core for the exact request, wallet outcome, transaction or same-intent replacement, receipt, direction-specific reservation event, finality, and reorg state. Its private SQLite journal durably claims the attempt and records only original core artifacts; restart is reconciliation-only and never authorizes resend. A fixed same-process EIP-1193 dispatcher requires explicit confirmation and one original claim before checking context and calling the exact request once. A separate private gateway core authenticates a maximum-30-second edge request, writes that same journal claim before issuing one signed random-token response, and allows exact outcome replay but never claim replay. A lost response or process restart therefore leaves `SEARCH_QUOTE_NO_RESEND`, not another wallet attempt. It has no listener, SIWE store, browser provider, human-gesture proof, or dispatch authority, and two-observation agreement remains repository evidence rather than independent canonical finality.

Before even capped funded testnet use, the remaining consumer must deploy the gateway behind a SIWE-authenticated and CSRF-protected edge, preserve private TLS and key separation, suppress body logs, make the reviewed human-gesture wallet adapter exclusive, add fixed authenticated provider adapters, compose the journal and complete transaction lifecycle into the persistent coordinator, and independently confirm through two genuinely operated providers that the exact durable quote ID and contract intent are canonical and finalized in the reviewed escrow. It must remain one replica until a separately reviewed shared fence exists. Only then may it permit the direction-correct Lightning action through the existing active daemon and private-packet controls.
