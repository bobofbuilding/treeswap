...
Status: the exact second user authorization and its durable settlement record are committed atomically in the local coordinator. A same-process consumer derives the exact direction-specific contract typed data, approved deadlines, required EOA signatures, and zero-ETH calldata from that original RFQ provenance. The capability-bound authenticated solver-signing client and durable repository-only provider obtain the selected solver's exact EIP-712 signature with ambiguity-safe replay. A separate non-dispatching wallet core now prepares and reviews that exact transaction, preserves and verifies a reported hash or same-intent replacement, and classifies exact reservation receipts and two-observation agreement. Coordinator schema v10 independently verifies and durably binds the signed contract intent before any execution-policy binding, reservation, private packet, evidence request, or action can use it. No independently operated solver listener or Ethereum provider is deployed, the core calls no wallet, and no transaction is confirmed onchain. A browser authorization response still proves only that the non-dispatching settlement record exists and grants no EVM reservation, Lightning payment, funding, claim, or settlement-dispatch authority. Funded operation remains closed.
...
The [contract-intent wallet boundary](./CONTRACT_INTENT_WALLET.md) now supplies the validation core for the exact request, wallet outcome, transaction or same-intent replacement, receipt, direction-specific reservation event, finality, and reorg state. Its private SQLite journal durably claims the attempt and records only original core artifacts; restart is reconciliation-only and never authorizes resend. A fixed same-process EIP-1193 dispatcher now requires explicit confirmation and one original claim before checking context and calling the exact request once. It persists a hash, exact rejection, or ambiguity and has no automatic retry. The Node-only journal and browser provider are not yet composed through a deployed authenticated route, and two-observation agreement remains repository evidence rather than independent canonical finality.

Before even capped funded testnet use, the remaining consumer must make the fixed wallet path exclusive through an authenticated browser/coordinator route, add fixed authenticated provider adapters, compose the journal and complete transaction lifecycle into the durable coordinator, and independently confirm through two genuinely operated providers that the exact durable quote ID and contract intent are canonical and finalized in the reviewed escrow. Only then may it permit the direction-correct Lightning action through the existing active daemon and private-packet controls.
