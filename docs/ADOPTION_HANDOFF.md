...
- the SIWE-session edge hashes the cookie locally and uses its exclusive reader lease to send only a signed short-lived token hash; the separately keyed provider performs the implemented fixed D1 query and returns one request-bound signed result without echoing the hash; inactive sessions reject normally, while D1, transport, signature, framing, time, or lifecycle failure halts admission; the edge derives the opaque session digest server-side, checks exact Origin/Fetch Metadata plus HMAC CSRF, and consumes the exclusive private abuse ledger before atomically consuming only an issued ownership handle and signing one wallet claim; the ledger persists its eight-per-60-second limit and clock high-water mark across restart without storing the token, token hash, session digest, wallet, or request; its verified private backup restores that exact state only to a fresh path, while the local `SIGKILL` and full-volume campaigns retain the crash fence and roll back failed consumption; its exclusive perimeter rejects coarse malformed/rate/concurrency traffic before the session read and rechecks the no-stale-takeover replica fence before every admitted request; it cannot recover or replay a lost claim response;
