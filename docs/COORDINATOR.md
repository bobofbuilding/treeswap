...
The payer campaign creates a real 10,000-sat standard regtest invoice, pays it through the signed payer adapter, deliberately discards the successful adapter response, proves the database contains `UNKNOWN`, closes and reopens the store, and recovers `SUCCEEDED` plus the exact bound preimage through a new read-only tracking authorization. It restarts again and recovers the same confirmed proof without changing durable action state. Dispatch count remains `1`; SQLite, WAL, and shared-memory scans contain neither the BOLT 11 string nor raw/textual preimage. Its deterministic evidence digest is `0x3dbfa76c509da075d901d23174f8623bccade72acf2787a7e6e90c7e6c15c584`.
...
2. deploy the implemented capability, endpoint, and reader protocols with independent operators; connect the durable planner to a reviewed private-packet/secret provider and the existing finality/dispatch modules; then qualify the complete daemon on the deployed stable persistence service;
