...
Admission first expires stale requests and releases their exact commitments, then evaluates persisted usage inside `BEGIN IMMEDIATE`. The first accepted RFQ writes the complete normalized admission-policy digest to coordinator metadata; every later admission, reservation, and outcome must use that identical digest, so a request cannot supply looser caps or failure thresholds. A duplicate request or offer ID is idempotent only for the same committed terms. Cancellation sequences only advance. Two local database connections cannot exceed the active-request limit or reserve the same solver capacity concurrently. There is no solver allowlist: only a locally verified capability and exact executable-quote binding can enter firm capacity. Unknown BIT → Lightning solvers receive the configured exposure cap, and an atomic global in-flight ceiling spans every solver identity; established capacity requires the configured number of completed settlements. A filled offer exercises its RFQ and releases every competing commitment in the same transaction, but it cannot increment successful history without an exact `COMPLETED` settlement, the same selected offer, and the same independently verified both-assets proof digest. User abandonment and expiry release capacity without harming solver reliability.
...
The current local matrix passes 246 application/security tests, 91 direct tests in the pinned coordinator runtime plus the real bounded-filesystem rollback campaign, both web builds, and 68 contract tests. Both production dependency trees report zero known vulnerabilities; four moderate development-only Drizzle Kit loader findings remain outside the shipped applications. This is local implementation evidence, not a deployed daemon, independently reviewed private service, public-testnet proof, or funding authorization.
