...
Chain freshness is also durable. Each role requires its own absolute `CHAIN_PROGRESS_PATH` on persistent storage with one adapter process writing it. The exact-schema mode-`0600` record is atomically replaced and both the file and parent directory are synced. A first observation never authorizes new exposure; a later higher LND block height is required. Restart preserves the last-advance clock, while a backward wall-clock observation, height regression, or contradictory header timestamp at the same height latches a conflict until a genuinely higher block is observed. Missing state therefore closes new exposure until the chain advances instead of resetting a stale-node clock.
