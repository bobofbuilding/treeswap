...
`npm run regtest:force-close-smoke` force-closes the payer's only channel, rejects new payment exposure while the close is pending, and independently requires an unsafe aggregate from exact observer-only `PendingChannels` and WalletKit `PendingSweeps` reads. It confirms the commitment, advances through the node-reported CSV maturity, confirms its sweep, and requires all pending-close, HTLC, and non-anchor state to clear. A fresh balanced channel is confirmed before read-only tracking proves zero dispatch and the recovered adapter decodes the same invoice. Only matured, non-immediate 330-sat anchors within the published count, total, age, retry, budget, and deadline bounds may remain healthy. See [Lightning close and sweep monitor](./LIGHTNING_CLOSE_MONITOR.md).
...
`npm run regtest:solver-capacity-smoke` proves both payer and invoice adapters derive fresh direction-specific capacity from live active channels and all pending HTLCs, subtract configured reserves, cap the result at an operator budget, and return only a signed aggregate without channel identifiers. The coordinator request and observer response are separately signed and bound to the exact capability digest, epoch, direction, solver, node, request ID, and short expiry. Unsigned and cross-role requests fail closed. This is authenticated private-state observation, not a cryptographic proof of a Lightning balance.

`npm run regtest:coordinator-smoke` rehydrates a real 10,000-sat standard invoice through an authenticated one-use private packet, lets the bounded daemon independently authorize and dispatch it, deliberately loses the successful response, reopens the coordinator database in `UNKNOWN`, and recovers success through bounded fresh read-only tracking without a second dispatch. A transient `NOT_FOUND` stays unresolved and permits only another read. The smoke provider uses a process-local replay consumer; it proves the response-builder boundary, not durable provider replay after restart. The finality authorization is also simulated, so this is local runtime evidence rather than deployed EVM finality evidence. See [Durable coordinator boundary](./COORDINATOR.md).
...
Before testnet funding, deploy the adapter, capacity observer, and redundant close/sweep collectors under independent service identities, export a secret-free final-release evidence bundle, and independently review the implementation and evidence. Durable stateless initialization, the full production-duration block delay, real TLS certificate/pin rotation with rollback, exact grant manifests, credential timeout, overlap rotation and old-root revocation, hold-invoice terminal faults, accepted-state LND restart, payer- and invoice-side lost-response recovery, rapid-block HTLC cutoff, genuine unsynced-node catch-up, full force-close recovery with the bounded anchor policy, compressed-threshold stale-header rejection, fee/amount/in-flight/daily caps, exact UTC rollover, live directional exhaustion/rebalancing, signed aggregate capacity observation, channel-offline recovery, and TLS-pin mismatch now pass locally.
