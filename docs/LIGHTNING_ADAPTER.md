...
`npm run regtest:coordinator-smoke` rehydrates a real 10,000-sat standard invoice through an authenticated one-use private packet, lets the bounded daemon independently authorize and dispatch it, deliberately loses the successful response, reopens the coordinator database in `UNKNOWN`, and recovers success through bounded fresh read-only tracking without a second dispatch. A transient `NOT_FOUND` stays unresolved and permits only another read. The finality authorization is simulated, so this is local runtime evidence rather than deployed EVM finality evidence. See [Durable coordinator boundary](./COORDINATOR.md).
