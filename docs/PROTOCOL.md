...
The RFQ client follows [`RFQ_POLICY.md`](RFQ_POLICY.md) and [`RFQ_DELIVERY.md`](RFQ_DELIVERY.md): it queries at least two authenticated relay paths and two distinct capability-bound direct solver paths with an unlinkable blind pricing request, locally timestamps their complete signed batches, validates every solver-signed blind offer, and commits the delivery and offer receipts before the user selects one quote. The user signs the selected solver, exact economics, complete received-set commitment, private request/recipient/nonce, invoice commitments, and expiry before the coordinator may reserve either capacity dimension or disclose private fields. After the selected solver supplies the matching full executable quote, the user signs again over the exact payment hash, invoice digest, amounts, recipient, solver, first approval, and durable execution binding. Coordinator schema v9 persists both authorization digests, signed expiries, and the reviewed market-risk policy binding; a solver-finalized quote without the second signature remains non-authorizing, and authorization use fails at its expiry boundary. The client never claims that a relay supplied a globally complete market.
