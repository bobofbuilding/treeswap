...
The RFQ client follows [`RFQ_POLICY.md`](RFQ_POLICY.md), [`RFQ_DELIVERY.md`](RFQ_DELIVERY.md), and [`RFQ_QUOTE_INGRESS.md`](RFQ_QUOTE_INGRESS.md): an authority-free ingress authenticates and rate-limits the exact browser pricing request, then queries at least two authenticated relay paths and two distinct capability-bound direct solver paths with only the unlinkable blind pricing payload. It locally timestamps their complete signed batches, validates every solver-signed blind offer, and commits the delivery and offer receipts before the user selects one opaque choice. The quote-request signature cannot reserve or move funds. The user later signs the selected solver, exact economics, complete received-set commitment, private request/recipient/nonce, invoice commitments, and expiry before the coordinator may reserve either capacity dimension or disclose private fields. After the selected solver supplies the matching full executable quote, the user signs again over the exact payment hash, invoice digest, amounts, recipient, solver, first approval, and durable execution binding. Coordinator schema v10 persists both settlement-authorization digests, signed expiries, the reviewed market-risk policy binding, and a separately verified exact contract-intent record; the production path atomically commits the second authorization and its non-dispatching settlement before acknowledging success. A solver-finalized quote without the second signature remains non-authorizing, the accepted settlement grants no asset action, and authorization use fails at its expiry boundary. See [`RFQ_SETTLEMENT_HANDOFF.md`](RFQ_SETTLEMENT_HANDOFF.md). The client never claims that a relay supplied a globally complete market.
