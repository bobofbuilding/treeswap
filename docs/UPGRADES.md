...
`lib/release-authorization.mjs` enforces this process before any operator-funding capability can exist. The guarded signing ceremony verifies all five public-testnet approvals but deliberately emits no activation provenance. Release v2 rejects mainnet entirely. A future mainnet schema must require a prior qualified public-testnet release, an equivalent mainnet preflight/postflight and promotion ceremony, complete operating evidence, every independent-review digest, exact caps and reserves, and disposition of all findings. Every signed authorization expires; raising a cap or changing an evidence, feature, chain, gate, or deployment digest requires a new record and all five approvals.
