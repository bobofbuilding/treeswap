...
The public-testnet [adoption policy](./ADOPTION_POLICY.md) makes this response machine-checkable: active-liability migration and emergency risk increases must be false; a BIT implementation change halts for review and a new observation; a BIT pause halts until unpaused and reviewed; and any TreeSwap contract change deploys a new immutable release.

`lib/release-authorization.mjs` enforces the signed release, while `lib/capabilities.mjs` enforces the separate same-process live activation. The guarded signing ceremony verifies all five public-testnet approvals but deliberately emits no activation provenance. Any escrow, registry, gate, BIT implementation, code-hash, evidence, feature, chain, deployment, cap, or reserve change invalidates activation and requires the applicable new deployment review plus a new five-role release. Coordinator restart or runtime-reconciliation expiry requires fresh live activation even when the signed release remains valid. Release v2 rejects mainnet entirely. A future mainnet schema must require a prior qualified public-testnet release, an equivalent mainnet preflight/postflight and promotion ceremony, complete operating evidence, every independent-review digest, exact caps and reserves, and disposition of all findings.
