...
function rememberCandidateRuntimeBinding(candidate, manifest) {
  candidateRuntimeBindings.set(candidate, Object.freeze({
    manifest: frozenClone(manifest),
...
function normalizeRecordTemplate(raw) {
  ...
  if (raw.schema !== "treeswap.public-testnet-release-record-template.v3") {
...
export function preparePublicTestnetReleaseCandidate({
  ...
  rememberCandidateRuntimeBinding(result, deploymentPromotionVerification.manifest);
  return result;
}
...
export function preparePublicTestnetBootstrapReleaseCandidate({
  ...
  const approvalDomain = releaseAuthorizationDomain(record);
  const result = Object.freeze({
    schema: "treeswap.prepared-public-testnet-bootstrap-release-candidate.v3",
...
export function buildPublicTestnetReleaseCandidateSummary(candidate) {
  if (!verifiedCandidates.has(candidate)) throw new Error("public-testnet release candidate provenance is invalid");
  return Object.freeze({
    schema: "treeswap.public-testnet-release-candidate-summary.v2",
