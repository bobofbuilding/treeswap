...
const RFQ_DELIVERY_CLIENTS = new WeakMap();
const RFQ_DELIVERY_CLIENT_LEASES = new WeakMap();
const RFQ_DELIVERY_CLIENT_CONFIG_FIELDS = Object.freeze([
  "maximumResponseBytes", "paths", "policy", "requestTtlSeconds", "timeoutMs",
]);
const TEST_RFQ_DELIVERY_CLIENT_CONFIG_FIELDS = Object.freeze([
  ...RFQ_DELIVERY_CLIENT_CONFIG_FIELDS, "nowSeconds", "randomBytesImpl", "requestImpl",
]);
const RFQ_DELIVERY_CLIENT_CALL_FIELDS = Object.freeze([
  "requestDigest", "requestId", "rfq", "signal",
]);
const systemNowSeconds = () => Math.floor(Date.now() / 1_000);
...
function cancellationError() {
  return new RfqDeliveryError("RFQ delivery collection was cancelled", { code: "CANCELLED" });
}
...
async function queryNormalizedPath({
  ...
  const transportSignal = signal ? AbortSignal.any([signal, controller.signal]) : controller.signal;
  if (transportSignal.aborted) throw cancellationError();
  let timer;
  let timedOut = false;
  let abortListener;
  const hardBoundary = new Promise((_, reject) => {
    abortListener = () => reject(new Error("RFQ delivery request was aborted"));
    transportSignal.addEventListener("abort", abortListener, { once: true });
    timer = setTimeout(() => {
      timedOut = true;
      controller.abort();
    }, requestTimeout);
  ...
  try {
    ...
    try {
      ...
      if (signal?.aborted && !timedOut) throw cancellationError();
    ...
    clearTimeout(timer);
    transportSignal.removeEventListener("abort", abortListener);
    controller.abort();
  }
  if (signal?.aborted) throw cancellationError();
...
async function queryConfiguredRfqDelivery(raw, testOnly) {
  const source = dataRecordWithOptionalFields(
    raw,
    testOnly
      ? ["path", "policy", "requestDigest", "requestId", "rfq"]
      : ["path", "policy", "requestDigest", "requestId", "rfq", "signal"],
    testOnly
      ? ["maximumResponseBytes", "nowSeconds", "randomBytesImpl", "requestImpl", "requestTtlSeconds", "signal", "timeoutMs"]
      : ["maximumResponseBytes", "requestTtlSeconds", "timeoutMs"],
  ...
  return queryNormalizedPath({
    ...
    rfq: boundRfq,
    policy: boundPolicy,
    requestImpl: testOnly ? source.requestImpl ?? null : null,
    nowSeconds: testOnly ? source.nowSeconds ?? systemNowSeconds : systemNowSeconds,
    randomBytesImpl: testOnly ? source.randomBytesImpl ?? randomBytes : randomBytes,
    ...
    signal: source.signal === undefined ? null : abortSignal(source.signal, "RFQ delivery query signal"),
  });
}

export function queryVerifiedRfqDelivery(raw) {
  return queryConfiguredRfqDelivery(raw, false);
}

export function queryTestVerifiedRfqDelivery(raw) {
  return queryConfiguredRfqDelivery(raw, true);
}

async function collectNormalizedRfqDeliveries({
  plan,
  boundPolicy,
  boundRequestId,
  boundRequestDigest,
...
async function collectConfiguredRfqDeliveries(raw, testOnly) {
  const source = dataRecordWithOptionalFields(
    raw,
    testOnly
      ? ["paths", "policy", "requestDigest", "requestId", "rfq"]
      : ["paths", "policy", "requestDigest", "requestId", "rfq", "signal"],
    testOnly
      ? ["maximumResponseBytes", "nowSeconds", "randomBytesImpl", "requestImpl", "requestTtlSeconds", "signal", "timeoutMs"]
      : ["maximumResponseBytes", "requestTtlSeconds", "timeoutMs"],
  ...
  return collectNormalizedRfqDeliveries({
    plan,
    boundPolicy,
    boundRequestId,
    boundRequestDigest,
    ...
    timeoutMs: source.timeoutMs ?? 5_000,
    maximumResponseBytes: source.maximumResponseBytes ?? 262_144,
    signal: source.signal === undefined ? null : abortSignal(source.signal, "RFQ delivery collection signal"),
  });
}

export function collectVerifiedRfqDeliveries(raw) {
  return collectConfiguredRfqDeliveries(raw, false);
}

export function collectTestVerifiedRfqDeliveries(raw) {
  return collectConfiguredRfqDeliveries(raw, true);
}

function createRfqClient(source, transportMode) {
  const boundPolicy = normalizedPolicy(source.policy);
  const descriptor = Object.freeze({
    plan: normalizedPlan(source.paths, boundPolicy),
    policy: boundPolicy,
    requestTtlSeconds: (() => {
      const value = integer(source.requestTtlSeconds, "RFQ delivery client request lifetime", boundPolicy.maxResponseTtlSeconds);
      if (value === 0) throw new RangeError("RFQ delivery client request lifetime must be non-zero");
      return value;
    })(),
    timeoutMs: (() => {
      const value = integer(source.timeoutMs, "RFQ delivery client timeout", 30_000);
      if (value === 0) throw new RangeError("RFQ delivery client timeout must be non-zero");
      return value;
    })(),
    maximumResponseBytes: (() => {
      const value = integer(source.maximumResponseBytes, "RFQ delivery client response limit", 1_048_576);
      if (value < 1_024) throw new RangeError("RFQ delivery client response limit is too small");
      return value;
    })(),
    transportMode,
  });
  ...
  const context = {
    ...
    lifecycle,
    descriptor,
    nowSeconds: transportMode === "fixed-public-node-https" ? systemNowSeconds : source.nowSeconds,
    randomBytesImpl: transportMode === "fixed-public-node-https" ? randomBytes : source.randomBytesImpl,
    requestImpl: transportMode === "fixed-public-node-https" ? null : source.requestImpl,
  };
  const collect = (callInput) => {
    if (context.closed) throw new RfqDeliveryError("RFQ delivery client is closed", { code: "CLIENT_CLOSED" });
    const call = exactDataRecord(callInput, RFQ_DELIVERY_CLIENT_CALL_FIELDS, "RFQ delivery client call");
    const callSignal = abortSignal(call.signal, "RFQ delivery client call signal");
    const boundRequestId = bytes32(call.requestId, "requestId", { nonzero: true });
    const boundRequestDigest = bytes32(call.requestDigest, "requestDigest", { nonzero: true });
    const boundRfq = normalizedRfqPayload(call.rfq);
    if (rfqDeliveryPayloadDigest(boundRfq) !== boundRequestDigest) {
      throw new Error("RFQ payload does not match its request digest");
    }
    return collectNormalizedRfqDeliveries({
      plan: descriptor.plan,
      boundPolicy: descriptor.policy,
      boundRequestId,
      boundRequestDigest,
      boundRfq,
      requestImpl: context.requestImpl,
      nowSeconds: context.nowSeconds,
      randomBytesImpl: context.randomBytesImpl,
  ...
  const client = Object.freeze({
    collect(callInput) {
      if (this !== client || RFQ_DELIVERY_CLIENTS.get(this) !== context) {
        throw new TypeError("RFQ delivery client lacks factory provenance");
      }
      if (context.lease) throw new RfqDeliveryError("RFQ delivery client is service-owned", { code: "CLIENT_OWNED" });
      context.unownedInFlight += 1;
      try {
        return Promise.resolve(context.collect(callInput)).finally(() => {
          context.unownedInFlight -= 1;
        });
      } catch (error) {
        context.unownedInFlight -= 1;
  ...
  RFQ_DELIVERY_CLIENTS.set(client, context);
  return client;
}

export function createRfqDeliveryClient(input) {
  const source = exactDataRecord(input, RFQ_DELIVERY_CLIENT_CONFIG_FIELDS, "RFQ delivery client");
  return createRfqClient(source, "fixed-public-node-https");
}

export function createTestRfqDeliveryClient(input) {
  const source = exactDataRecord(
    input,
    TEST_RFQ_DELIVERY_CLIENT_CONFIG_FIELDS,
    "test RFQ delivery client",
  );
  if (typeof source.nowSeconds !== "function" || typeof source.randomBytesImpl !== "function"
      || typeof source.requestImpl !== "function") {
    throw new TypeError("test RFQ delivery client dependencies are required functions");
  }
  return createRfqClient(source, "injected-test");
}

export function isRfqDeliveryClient(value) {
  return Boolean(value && RFQ_DELIVERY_CLIENTS.has(value));
}

export function isProductionRfqDeliveryClient(value) {
  return RFQ_DELIVERY_CLIENTS.get(value)?.descriptor.transportMode === "fixed-public-node-https";
}

export function rfqDeliveryClientTransportMode(value) {
  const mode = RFQ_DELIVERY_CLIENTS.get(value)?.descriptor.transportMode;
  if (!mode) throw new TypeError("RFQ delivery client lacks factory provenance");
  return mode;
}

export function rfqDeliveryClientLifecycleState(value) {
  const context = RFQ_DELIVERY_CLIENTS.get(value);
  if (!context) throw new TypeError("RFQ delivery client lacks factory provenance");
  return context.closed ? "closed" : context.lease ? "owned" : "open";
}

export function claimRfqDeliveryClientOwnership(value) {
  const context = RFQ_DELIVERY_CLIENTS.get(value);
  if (!context) throw new TypeError("RFQ delivery client lacks factory provenance");
  if (context.closed) throw new TypeError("RFQ delivery client is already closed");
  if (context.lease) throw new TypeError("RFQ delivery client is already owned");
  if (context.unownedInFlight !== 0) {
    throw new TypeError("RFQ delivery client has an active unowned request");
  }
  const lease = Object.freeze({
    collect(callInput) {
      if (this !== lease || RFQ_DELIVERY_CLIENT_LEASES.get(this) !== context) {
        throw new TypeError("RFQ delivery client lease lacks factory provenance");
      }
      return context.collect(callInput);
    },
    close() {
      if (this !== lease || RFQ_DELIVERY_CLIENT_LEASES.get(this) !== context) {
        throw new TypeError("RFQ delivery client lease lacks factory provenance");
  ...
  context.lease = lease;
  RFQ_DELIVERY_CLIENT_LEASES.set(lease, context);
  return lease;
}
