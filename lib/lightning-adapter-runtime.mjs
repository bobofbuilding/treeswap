...
export class LightningAdapterRuntime {
  ...
  async #observeService() {
    ...
    const blockFingerprint = `${blockHeight}:${bestHeaderTimestamp}`;
    if (blockFingerprint !== this.#lastBlockFingerprint) {
      this.#lastBlockFingerprint = blockFingerprint;
      this.#lastBlockObservedAt = observedAt;
    }
    const noProgressSeconds = Math.max(0, observedAt - this.#lastBlockObservedAt);
    ...
    const headerAgeSeconds = Math.max(reportedHeaderAgeSeconds, noProgressSeconds);
    return Object.freeze({
      healthy: info.synced_to_chain === true
        && walletSynced
        && headerAgeSeconds <= this.#policy.maxChainHeaderAgeSeconds
