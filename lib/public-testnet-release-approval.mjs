...
const CANDIDATE_KINDS = Object.freeze({
  "treeswap.prepared-public-testnet-bootstrap-release-candidate.v5": Object.freeze({
  ...
  "treeswap.prepared-public-testnet-release-candidate.v5": Object.freeze({
    fields: Object.freeze([
      "adoptionSummary",
      "approval",
      "authorizations",
...
export function inspectPreparedPublicTestnetReleaseCandidate(input) {
  requirePlainObject(input, "prepared release candidate");
  const kind = CANDIDATE_KINDS[input.schema];
  if (!kind) throw new TypeError("prepared release candidate schema is invalid");
  exactKeys(input, kind.fields, "prepared release candidate");
  if (input.status !== kind.status || input.scope !== kind.scope) {
    throw new Error("prepared release candidate status or scope is invalid");
  }
  falseAuthorizations(input.authorizations, "prepared release candidate authorizations");
  exactKeys(input.evidence, kind.evidenceFields, "prepared release candidate evidence");
  for (const field of kind.evidenceFields) {
    digest(input.evidence[field], `prepared release candidate evidence.${field}`);
  }
  if (kind.fundingMode === "operator-testnet") {
    requirePlainObject(input.adoptionSummary, "prepared release candidate adoption summary");
  }
  if (input.record?.fundingMode !== kind.fundingMode) {
    throw new Error("prepared release candidate schema does not match its funding mode");
  }

  const approval = expectedApproval(input.record, input.policy);
  for (const role of ["controller", "guardian"]) {
    if (input.policy?.approvers?.[role]?.signatureKind !== "erc1271") {
  ...
  for (const role of ["lightningOperator", "securityReviewer", "incidentCommander"]) {
    if (input.policy?.approvers?.[role]?.signatureKind !== "eip712") {
      throw new Error(`prepared release candidate ${role} approval must use EIP-712`);
    }
  }
  const recordDigest = digest(input.recordDigest, "prepared release candidate recordDigest");
  const policyDigest = digest(input.policyDigest, "prepared release candidate policyDigest");
  ...
  exactKeys(input.approval, APPROVAL_PAYLOAD_FIELDS, "prepared release candidate approval payload");
  if (canonicalJson(input.approval) !== canonicalJson(approval)) {
...
export function buildPublicTestnetReleaseRoleApprovalPayload({
  candidate: input,
  role,
  now = Math.floor(Date.now() / 1_000),
}) {
  const inspected = inspectPreparedPublicTestnetReleaseCandidate(input);
  if (!RELEASE_APPROVAL_ROLES.includes(role)) throw new TypeError("release approval role is invalid");
  const preparedAt = currentTime(now, "release approval payload preparation time");
...
function normalizeApprovalBundle(input, inspected) {
  exactKeys(input, APPROVAL_BUNDLE_FIELDS, "release approval bundle");
  if (input.schema !== "treeswap.public-testnet-release-approvals.v1") {
    throw new TypeError("release approval bundle schema is invalid");
  }
  if (digest(input.releaseId, "release approval bundle releaseId") !== inspected.message.releaseId
      || digest(input.recordDigest, "release approval bundle recordDigest") !== inspected.recordDigest
      || digest(input.policyDigest, "release approval bundle policyDigest") !== inspected.policyDigest) {
    throw new Error("release approval bundle does not match the prepared candidate");
  }
  if (!Array.isArray(input.approvals) || input.approvals.length !== RELEASE_APPROVAL_ROLES.length) {
    throw new Error("release approval bundle must contain exactly five role approvals");
  }
  const approvals = new Map();
  for (const [index, raw] of input.approvals.entries()) {
...
export function inspectPublicTestnetReleaseApprovalBundle({ candidate, approvalBundle }) {
  const inspected = inspectPreparedPublicTestnetReleaseCandidate(candidate);
  const normalized = normalizeApprovalBundle(approvalBundle, inspected);
  return Object.freeze({
...
export function createPublicTestnetReleaseApprovalProviderSet({
  configuration,
  environment = process.env,
  fetchImpl = globalThis.fetch,
  expectedProviderCount,
  expectedProviderSetDigest,
}) {
  ...
  const expectedDigest = digest(expectedProviderSetDigest, "expected release approval provider-set digest");
...
export async function verifyPublicTestnetReleaseApprovals({
  candidate: input,
  approvalBundle: approvalInput,
  providers,
  now = Math.floor(Date.now() / 1_000),
}) {
  const inspected = inspectPreparedPublicTestnetReleaseCandidate(input);
  const approvalBundle = normalizeApprovalBundle(approvalInput, inspected);
  const verifiedAt = currentTime(now, "release approval verification time");
