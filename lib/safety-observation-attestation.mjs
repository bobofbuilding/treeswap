...
export const SAFETY_MONITOR_POLICY_SCHEMA = "treeswap.safety-monitor-policy.v1";
export const SAFETY_OBSERVATION_SCHEMA = "treeswap.safety-observation-attestation.v1";
...
function normalizePolicy(raw) {
  ...
  if (!Array.isArray(raw.collectors) || raw.collectors.length !== REQUIRED_SAFETY_CHECKS.length) {
    throw new TypeError("safety monitor policy requires exactly one collector per safety domain");
  }
  const collectors = raw.collectors.map((collector, index) => {
    exactKeys(collector, ["collectorId", "kind", "signer"], `safety monitor collector ${index}`);
    if (collector.kind !== REQUIRED_SAFETY_CHECKS[index]) {
      throw new Error("safety monitor collectors must be complete and canonically ordered by kind");
    }
...
function normalizeObservation(raw, policy, policyDigest) {
  ...
  const collector = policy.collectors.find((candidate) => candidate.kind === raw.kind);
  ...
  const normalized = Object.freeze({
    ...
    collectorId: digest(raw.collectorId, "safety observation collector id"),
    kind: raw.kind,
    status: raw.status,
    observedAt,
    validUntil,
...
export function prepareSafetyObservation({
  ...
  const collector = policy.collectors.find((candidate) => candidate.kind === kind);
  if (!collector) throw new TypeError("safety observation kind is invalid");
