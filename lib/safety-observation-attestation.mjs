...
export const SAFETY_MONITOR_POLICY_SCHEMA = "treeswap.safety-monitor-policy.v4";
export const SAFETY_OBSERVATION_SCHEMA = "treeswap.safety-observation-attestation.v4";
export const SAFETY_MONITOR_ACTION_PLAN_SCHEMA = "treeswap.safety-monitor-action-plan.v2";
export const REQUIRED_SAFETY_COLLECTORS_PER_DOMAIN = 2;
export const REQUIRED_GUARDIAN_BROADCASTERS = 2;
export const REQUIRED_GATE_CONFIRMERS = 2;
export const REQUIRED_ALERT_ROUTES = 2;
...
const verifiedActionPlans = new WeakMap();
...
function normalizeRoute(raw, name) {
  exactKeys(raw, ["operatorId", "routeId"], name);
  return Object.freeze({
    routeId: digest(raw.routeId, `${name} route id`),
    operatorId: digest(raw.operatorId, `${name} operator id`),
  });
}

function normalizeRoutes(raw, name, required) {
  if (!Array.isArray(raw) || raw.length !== required) {
    throw new TypeError(`${name} requires exactly ${required} routes`);
  }
  const routes = raw.map((route, index) => normalizeRoute(route, `${name}[${index}]`));
  for (let index = 1; index < routes.length; index += 1) {
    if (routes[index - 1].routeId >= routes[index].routeId) {
      throw new Error(`${name} must be strictly canonically ordered by route id`);
    }
  }
  if (new Set(routes.map((route) => route.operatorId)).size !== routes.length) {
    throw new Error(`${name} must use distinct operator commitments`);
  }
...
function normalizePolicy(raw) {
  ...
  const quoteClosure = normalizeRoute(raw.quoteClosure, "safety monitor quote closure");
  const guardianBroadcasters = normalizeRoutes(
    raw.guardianBroadcasters,
    "safety monitor guardian broadcasters",
    REQUIRED_GUARDIAN_BROADCASTERS,
  );
  const gateConfirmers = normalizeRoutes(
    raw.gateConfirmers,
    "safety monitor gate confirmers",
    REQUIRED_GATE_CONFIRMERS,
  );
  const alertRoutes = normalizeRoutes(
    raw.alertRoutes,
    "safety monitor alert routes",
    REQUIRED_ALERT_ROUTES,
  );
  const actionRouteIds = [
    quoteClosure.routeId,
    ...guardianBroadcasters.map((route) => route.routeId),
    ...gateConfirmers.map((route) => route.routeId),
    ...alertRoutes.map((route) => route.routeId),
  ];
  if (new Set(actionRouteIds).size !== actionRouteIds.length) {
    throw new Error("safety monitor action route identities must be globally distinct");
  }
  const gateControlOperatorIds = [
    ...guardianBroadcasters.map((route) => route.operatorId),
    ...gateConfirmers.map((route) => route.operatorId),
  ];
  if (new Set(gateControlOperatorIds).size !== gateControlOperatorIds.length) {
    throw new Error("guardian broadcasters and gate confirmers must use globally distinct operator commitments");
  }
...
export function normalizeSafetyMonitorPolicy(policy) {
...
function actionRoute(raw, expected, name) {
  exactKeys(raw, ["execute", "operatorId", "routeId"], name);
  if (typeof raw.execute !== "function") throw new TypeError(`${name} execute must be a function`);
  const normalized = normalizeRoute({ routeId: raw.routeId, operatorId: raw.operatorId }, name);
  if (normalized.routeId !== expected.routeId || normalized.operatorId !== expected.operatorId) {
    throw new Error(`${name} does not match the release-bound safety policy`);
  }
  return Object.freeze({ ...normalized, execute: raw.execute });
}

function actionRoutes(raw, expected, name) {
  if (!Array.isArray(raw) || raw.length !== expected.length) {
    throw new TypeError(`${name} count does not match the release-bound safety policy`);
  }
  return Object.freeze(raw.map((route, index) => actionRoute(route, expected[index], `${name}[${index}]`)));
}

export function bindSafetyMonitorActions({
  policy: rawPolicy,
  expectedPolicyDigest,
  now,
  quoteClosure,
  ...
  const boundAt = timestamp(now, "safety monitor action-plan binding time");
  if (boundAt < policy.validFrom || boundAt >= policy.validUntil) {
    throw new Error("safety monitor policy is not active at action-plan binding time");
  }
  const boundQuoteClosure = actionRoute(quoteClosure, policy.quoteClosure, "safety monitor quote closure");
  const boundGuardianBroadcasters = actionRoutes(
    guardianBroadcasters,
    policy.guardianBroadcasters,
    "safety monitor guardian broadcasters",
  );
  const boundGateConfirmers = actionRoutes(
    gateConfirmers,
    policy.gateConfirmers,
    "safety monitor gate confirmers",
  );
  const boundAlertRoutes = actionRoutes(alertRoutes, policy.alertRoutes, "safety monitor alert routes");
  const plan = Object.freeze({
    schema: SAFETY_MONITOR_ACTION_PLAN_SCHEMA,
    policyDigest,
    quoteClosure: policy.quoteClosure,
    guardianBroadcasters: policy.guardianBroadcasters,
    gateConfirmers: policy.gateConfirmers,
    alertRoutes: policy.alertRoutes,
  });
  verifiedActionPlans.set(plan, Object.freeze({
    policyDigest,
    releaseRecordDigest: policy.releaseRecordDigest,
    verifyingContract: policy.verifyingContract.toLowerCase(),
    maximumObservationAgeSeconds: policy.maximumObservationAgeSeconds,
    validFrom: policy.validFrom,
    validUntil: policy.validUntil,
    quoteClosure: boundQuoteClosure,
    guardianBroadcasters: boundGuardianBroadcasters,
    gateConfirmers: boundGateConfirmers,
    alertRoutes: boundAlertRoutes,
  }));
  return plan;
}

export function verifiedSafetyMonitorActionBinding(plan) {
  const binding = verifiedActionPlans.get(plan);
  if (!binding) throw new TypeError("safety monitor action plan lacks same-process policy provenance");
  return binding;
}
