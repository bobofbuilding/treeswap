...
export function normalizeScheduledAccountStorageObserverRequest(raw) {
...
async function buildObserverRequest({ runtime, kind, requestedAt, randomBytes }) {
  ...
  const request = normalizeScheduledAccountStorageObserverRequest({
...
// Cancellation is best effort: a stalled underlying source must never hold up escalation.
  ...
  try {
    return await Promise.race([
      ...
      new Promise((_, reject) => {
        timer = setTimeout(() => reject(new Error("monitor storage operation timed out")), STORAGE_TIMEOUT_MS);
      }),
    ]);
  } finally {
    clearTimeout(timer);
...
async function readBoundedJsonResponse(response, label, signal) {
  ...
  const cancel = () => cancelStream(reader);
...
async function executeScheduledMonitor({ controller, env, clock, randomBytes, collectDatabase, runCycle, log }) {
  ...
  const [databaseResult, accessResult, maintenanceResult] = await Promise.allSettled([
    boundedStorageOperation(() => collectDatabase(runtime.database)),
  ...
  let receipt;
  try {
    // D1/R2 have no cancellation API here. Late results cannot rejoin this invocation,
    // and an ambiguous write is never retried or promoted to a positive receipt.
    receipt = await boundedStorageOperation(
      () => retainEvidence(runtime, schedule, startedAtMs, completedAtMs, cycle),
    );
  } catch {
