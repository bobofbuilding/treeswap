...
const verifiedPostflights = new WeakMap();
...
export function verifyWalletSessionRouteDeploymentPostflight({
  ...
  verifiedPostflights.set(verification, candidate);
  return verification;
}

export function verifyWalletSessionRouteDeploymentPostflightAtSignedBoundary({
  deploymentPreflightVerification,
  evidence,
  attestations,
}) {
  ...
  return verifyWalletSessionRouteDeploymentPostflight({
    deploymentPreflightVerification,
    evidence,
    attestations,
...
export function buildWalletSessionRouteDeploymentLiveReviewEvidence(verification) {
  const candidate = verifiedPostflights.get(verification);
  if (!candidate) {
    throw new Error("wallet session route postflight provenance is invalid");
  }
  return Object.freeze({
    schema: "treeswap.wallet-session-route-deployment-live-review-input.v1",
    status: verification.status,
    scope: "verified-postflight-input-only-independent-live-review-still-required",
    evidenceDigest: verification.evidenceDigest,
    recordDigest: verification.recordDigest,
    attestationDigest: verification.attestationDigest,
    preflightEvidenceDigest: verification.record.preflightEvidenceDigest,
    ...
    attestedFrom: verification.attestedFrom,
    attestedThrough: verification.attestedThrough,
    reviewers: candidate.preflight.reviewers,
    operators: candidate.preflight.operators,
    observers: candidate.evidence.observers,
    reports: candidate.evidence.reports,
    attestedClaims: verification.attestedClaims,
    externalVerification: verification.externalVerification,
    authorizations: verification.authorizations,
  });
