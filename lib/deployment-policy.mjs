...
const ROLE_FIELDS = Object.freeze(["address", "codeHash", "isContract", "ownerAddresses", "owners", "threshold"]);
...
const REGISTRY_FIELDS = Object.freeze(["address", "approvedEscrows", "codeHash", "escrowCount", "sealed"]);
...
export function assertExactDeploymentManifestShape(manifest) {
  ...
  for (const name of ["controller", "guardian", "feeCollector"]) {
    exactKeys(manifest[name], ROLE_FIELDS, `deployment manifest ${name}`);
  }
...
function uint256(value, name, reasons) {
  const raw = String(value ?? "");
  if (!DECIMAL.test(raw)) {
    reasons.push(`${name} is not a canonical uint256 decimal`);
    return null;
  }
  const parsed = BigInt(raw);
  if (parsed > UINT256_MAX) {
    reasons.push(`${name} exceeds uint256`);
    return null;
  }
  return parsed;
}
...
export function validateDeploymentManifest(manifest, policy) {
  ...
  const vaultAvailable = uint256(accounting.vaultTotalAvailableWei, "accounting.vaultTotalAvailableWei", reasons);
  const vaultLocked = uint256(accounting.vaultTotalLockedWei, "accounting.vaultTotalLockedWei", reasons);
  const vaultAccounted = uint256(accounting.vaultAccountedBalanceWei, "accounting.vaultAccountedBalanceWei", reasons);
  const vaultBitBalance = uint256(accounting.vaultBitBalanceWei, "accounting.vaultBitBalanceWei", reasons);
  const userLocked = uint256(accounting.userEscrowTotalLockedWei, "accounting.userEscrowTotalLockedWei", reasons);
  const userBitBalance = uint256(accounting.userEscrowBitBalanceWei, "accounting.userEscrowBitBalanceWei", reasons);
  if ([vaultAvailable, vaultLocked, vaultAccounted].every((value) => value !== null)) {
    add(reasons, vaultAvailable + vaultLocked !== vaultAccounted, "vault accounting does not reconcile");
  }
  if (vaultAccounted !== null && vaultBitBalance !== null) {
    add(reasons, vaultAccounted !== vaultBitBalance, "vault BIT balance does not match accounted inventory");
  }
  if (userLocked !== null && userBitBalance !== null) {
    add(reasons, userLocked !== userBitBalance, "user-escrow BIT balance does not match locked liabilities");
  }
  add(
    reasons,
    [vaultAvailable, vaultLocked, vaultAccounted, vaultBitBalance, userLocked, userBitBalance]
      .some((value) => value !== null && value !== 0n),
    "deployment promotion requires zero BIT inventory and liabilities",
  );
