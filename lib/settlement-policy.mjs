export const TREE_SWAP_SETTLEMENT_POLICY_V1 = Object.freeze({
  version: "1",
...
const POLICY_FIELDS = Object.freeze(Object.keys(TREE_SWAP_SETTLEMENT_POLICY_V1));
...
export function validatedTreeSwapSettlementPolicy(value) {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new TypeError("settlement policy must be an object");
  }
  const prototype = Object.getPrototypeOf(value);
  if (prototype !== Object.prototype && prototype !== null) {
    throw new TypeError("settlement policy must be a plain data object");
  }
  ...
  if (keys.some((key) => typeof key !== "string") || keys.length !== expected.length
    ...
    throw new TypeError("settlement policy fields are not exact");
  }
  for (const field of POLICY_FIELDS) {
    const descriptor = Object.getOwnPropertyDescriptor(value, field);
    if (!descriptor || !Object.hasOwn(descriptor, "value") || descriptor.enumerable !== true) {
      throw new TypeError("settlement policy fields must be enumerable data properties");
    }
    if (descriptor.value !== TREE_SWAP_SETTLEMENT_POLICY_V1[field]) {
      throw new RangeError(`settlement policy ${field} is not the approved v1 value`);
    }
  }
  return TREE_SWAP_SETTLEMENT_POLICY_V1;
}
...
export function issueLightningAuthorization({ actionId, schedule, chain, service, nowSeconds, policy }) {
  if (!/^0x[0-9a-fA-F]{64}$/.test(String(actionId ?? ""))) {
    throw new TypeError("actionId must be bytes32");
  }
  const decision = authorizeLightningAction({ schedule, chain, service, nowSeconds, policy });
  if (!decision.authorized) throw new Error(`Lightning authorization denied: ${decision.reasons.join("; ")}`);
  const now = integer(nowSeconds, "nowSeconds");
  const expiresAt = minimum(
    schedule.lastSafeClaimAt,
    now + integer(policy.maxAuthorizationAgeSeconds, "maxAuthorizationAgeSeconds"),
  );
  if (expiresAt <= now) throw new RangeError("Lightning authorization has no usable lifetime");
  return Object.freeze({
    actionId,
    intentDigest: chain.expectedEscrowDigest,
    escrowBlock: integer(chain.escrowBlock, "chain.escrowBlock"),
    escrowBlockHash: chain.escrowBlockHash,
    finalizedBlock: integer(chain.finalizedBlock, "chain.finalizedBlock"),
    authorizedAt: now,
    expiresAt,
  });
}

export function validateLightningDispatch({ authorization, chain, service, usedActionIds = [], nowSeconds, policy }) {
  const reasons = [];
  const now = integer(nowSeconds, "nowSeconds");
  const latestBlock = integer(chain.latestBlock, "chain.latestBlock");
  const finalizedBlock = integer(chain.finalizedBlock, "chain.finalizedBlock");
  ...
  if (usedActionIds.includes(authorization.actionId)) reasons.push("Lightning authorization was already consumed");
  if (now >= authorization.expiresAt) reasons.push("Lightning authorization expired");
  if (now < authorization.authorizedAt) reasons.push("Lightning authorization time is in the future");
  if (now - authorization.authorizedAt > integer(policy.maxAuthorizationAgeSeconds, "maxAuthorizationAgeSeconds")) {
    reasons.push("Lightning authorization is stale");
  }
  if (escrowBlock !== authorization.escrowBlock) reasons.push("escrow block changed after authorization");
  if (
    !chain.escrowBlockHash || chain.escrowBlockHash !== authorization.escrowBlockHash
      || chain.canonicalBlockHash !== authorization.escrowBlockHash
  ) reasons.push("escrow block reorged after authorization");
  if (finalizedBlock < authorization.finalizedBlock || escrowBlock > finalizedBlock || finalizedBlock > latestBlock) {
    reasons.push("Ethereum finalized state regressed or is inconsistent");
  }
  if (chain.escrowDigest !== authorization.intentDigest || chain.expectedEscrowDigest !== authorization.intentDigest) {
    reasons.push("escrow intent changed after authorization");
  }
  if (service.riskGateEnabled !== true) reasons.push("risk gate closed after authorization");
  if (service.balancesReconciled !== true) reasons.push("balances diverged after authorization");
  if (service.lightningNodeSynced !== true) reasons.push("Lightning node lost sync after authorization");
  if (service.adapterHealthy !== true) reasons.push("Lightning adapter became unhealthy");

  return Object.freeze({ authorized: reasons.length === 0, reasons });
}

export function consumeLightningAuthorization(usedActionIds, authorization, dispatchDecision) {
  if (dispatchDecision?.authorized !== true) throw new Error("cannot consume a denied Lightning authorization");
  if (usedActionIds.includes(authorization.actionId)) throw new Error("Lightning authorization was already consumed");
  return Object.freeze([...usedActionIds, authorization.actionId]);
}
