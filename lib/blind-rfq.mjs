...
function indexedMarketRiskAttestations(attestations) {
  if (!Array.isArray(attestations)) {
    throw new TypeError("blind quote verification requires current market-risk attestations");
  }
  if (attestations.length > MAX_RFQ_DELIVERY_OFFER_CANDIDATES) {
    throw new RangeError("blind market-risk attestation set exceeds the bounded candidate limit");
  }
  const byRequest = new Map();
  for (const attestation of attestations) {
    const requestDigest = String(attestation?.requestDigest ?? "");
...
export function buildMultipathBlindQuoteBook({
  pricing,
  collection,
  ...
  const observedAt = integer(now, "now");
  const marketRiskByRequest = indexedMarketRiskAttestations(marketRiskAttestations);
  ...
  if (capabilityVerifications.length > MAX_RFQ_DELIVERY_OFFER_CANDIDATES) {
    throw new RangeError("blind capability set exceeds the bounded candidate limit");
  }
...
export function authorizeFinalizedBlindQuote({
  ...
  const record = COORDINATOR_METHODS.bindFirmOfferUserAuthorization.call(reservationContext.store, {
