...
export function buildSelectedSolverDisclosure({
  ...
  const expiresAt = integer(Number(request.expiresAt), "request.expiresAt");
  const swapDirection = direction(request);
  if (channel?.authenticated !== true || channel?.encrypted !== true || channel?.peer !== selectedSolver) {
    throw new TypeError("selected solver disclosure requires an authenticated encrypted peer-bound channel");
  }
  if (expiresAt <= observedAt || expiresAt - observedAt > integer(maxDisclosureTtlSeconds, "maxDisclosureTtlSeconds")) {
    throw new RangeError("private disclosure is expired or exceeds its short-lived limit");
  }
  if (String(request.requestId).toLowerCase() === String(pricingId).toLowerCase()) {
    throw new TypeError("public pricing and private settlement identifiers must be unlinkable");
  }
  const solver = getAddress(selectedSolver);
  const packet = {
    requestId: assertBytes32(request.requestId, "requestId"),
    pricingCommitment: keccak256(toUtf8Bytes(`${assertBytes32(pricingId, "pricingId")}:${assertBytes32(selectedOfferId, "selectedOfferId")}`)),
    direction: swapDirection,
    user: getAddress(request.user),
    beneficiary: getAddress(request.beneficiary),
    paymentHash: assertBytes32(request.paymentHash, "paymentHash"),
    invoiceDigest: assertBytes32(request.invoiceDigest, "invoiceDigest"),
    invoice: String(invoice ?? ""),
    selectedSolver: solver,
    selectedOfferId: assertBytes32(selectedOfferId, "selectedOfferId"),
    exactBitOutputWei: unsigned(request.exactBitOutputWei, "exactBitOutputWei").toString(),
    exactLightningOutputSats: unsigned(request.exactLightningOutputSats, "exactLightningOutputSats").toString(),
    maxFeeBps: unsigned(request.maxFeeBps, "maxFeeBps").toString(),
    maxRoutingFeeSats: unsigned(request.maxRoutingFeeSats, "maxRoutingFeeSats").toString(),
    expiresAt,
  };
  if (packet.invoice.length > 4096) throw new RangeError("private invoice is too long");
  if (swapDirection === "bit-to-lightning") {
