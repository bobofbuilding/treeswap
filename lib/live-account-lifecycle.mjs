...
export function inspectSessionCookie(setCookie) {
  const header = String(setCookie ?? "");
  const cookie = extractSessionCookie(header);
  const cookieStart = header.indexOf(cookie);
  const targetHeader = header.slice(cookieStart).split(/,\s*(?=[^;,\s=]+=)/, 1)[0];
  const attributes = targetHeader.split(";").slice(1).map((attribute) => attribute.trim().toLowerCase());
...
export async function runLiveAccountLifecycle({ request, wallet = Wallet.createRandom() }) {
  ...
  try {
    ...
    const initialCookie = extractSessionCookie(initial.setCookie);
    cleanupCookies.add(initialCookie);
    inspectSessionCookie(initial.setCookie);
    ...
    const replacementCookie = extractSessionCookie(replacement.setCookie);
    cleanupCookies.add(replacementCookie);
    inspectSessionCookie(replacement.setCookie);
    ...
    const concurrentCookies = concurrentResponses.map((response) => extractSessionCookie(response.setCookie));
    for (const cookie of concurrentCookies) cleanupCookies.add(cookie);
    for (const response of concurrentResponses) inspectSessionCookie(response.setCookie);
