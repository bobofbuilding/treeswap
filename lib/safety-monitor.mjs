...
function safeHaltResult(value, alertDigest) {
  try {
...
export async function runSafetyMonitorCycle({
  ...
  let guardianBroadcastsSucceeded = 0;
  ...
  if (actionBinding) {
    ...
    guardianBroadcastsSucceeded = guardianResults
      .filter((result) => safeHaltResult(result, evaluation.alertDigest)).length;
    onchainGateHalted = guardianBroadcastsSucceeded > 0;
  ...
  return Object.freeze({
    ...
    guardianBroadcastsSucceeded,
    guardianBroadcastDegraded: guardianBroadcastsSucceeded < guardianBroadcastsAttempted,
