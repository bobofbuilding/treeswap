...
function createEdge(input, injected) {
  ...
  const assertEdge = (edge) => {
    if (edge !== service || EDGES.get(edge) !== context || context.state !== "active") {
  ...
  const service = Object.freeze({
    ...
    async handle(webRequest) {
      if (this !== service || EDGES.get(this) !== context) {
        throw new TypeError("wallet edge request requires the original service");
      }
      try {
        if (context.state !== "active") throw new Error("wallet edge is stopped");
