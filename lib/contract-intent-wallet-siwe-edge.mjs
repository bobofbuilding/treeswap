...
} from "./contract-intent-wallet-abuse-store.mjs";
import {
...
class EdgeFatalError extends Error {}
...
async function readSession(context, request, now) {
  let rawToken;
  try {
    rawToken = cookieToken(request.headers.get("cookie"));
  } catch {
  ...
  let source;
  try {
    const read = await context.sessionReader.read({ tokenHash, observedAt: now });
    source = exactRecord(
      context.sessionReader.consume(read, { tokenHash, observedAt: now }),
      [
        "active",
        "chainId",
        "createdAt",
        ...
        "rawSessionTokenDisclosed",
        "schema",
        "tokenHashDisclosed",
        "walletAddress",
        "walletDispatchAuthority",
      ],
      "wallet edge verified session read",
    );
  } catch (error) {
    context.state = "halted";
    context.activeClaims.clear();
    context.haltedOnSessionReaderFailure = true;
    throw new EdgeFatalError("wallet edge session reader failed closed", {
      cause: error instanceof ContractIntentWalletSessionReaderFatalError ? error : undefined,
    });
  }
  if (!source.active) throw new EdgeAuthenticationError("wallet edge session is unavailable");
  let wallet;
  let createdAt;
  let expiresAt;
  try {
    wallet = address(source.walletAddress, "wallet edge session wallet");
    createdAt = canonicalIso(source.createdAt, "wallet edge session creation time");
    expiresAt = canonicalIso(source.expiresAt, "wallet edge session expiry");
    if (source.schema !== "treeswap.contract-intent-wallet-session-read.v1"
        || source.walletAddress !== wallet || source.chainId !== 1
        || createdAt > now || expiresAt <= now || expiresAt - createdAt > MAXIMUM_SESSION_SECONDS
        || source.rawSessionTokenDisclosed !== false || source.tokenHashDisclosed !== false
        || source.walletDispatchAuthority !== false
        || source.lightningDispatchAuthority !== false || source.fundingAuthorization !== false) {
      throw new Error("wallet session read changed its binding");
    }
  } catch (error) {
    context.state = "halted";
    context.activeClaims.clear();
    context.haltedOnSessionReaderFailure = true;
    throw new EdgeFatalError("wallet edge session reader returned invalid authority", { cause: error });
  }
...
function consumeRate(context, session, now) {
  try {
    context.abuse.consume({
      now,
      sessionDigest: session.sessionDigest,
      sessionExpiresAt: session.expiresAt,
    });
  } catch (error) {
    if (error instanceof ContractIntentWalletAbuseRateLimitError) {
      throw new EdgeRateLimitError("wallet edge request rate is exceeded");
    }
    context.state = "halted";
    context.activeClaims.clear();
    context.haltedOnDurableAbuseStore = true;
    if (error instanceof ContractIntentWalletAbuseClockRollbackError) {
      context.haltedOnClockRollback = true;
    }
    throw new EdgeFatalError("wallet edge durable abuse control failed closed", { cause: error });
  }
}
...
function createEdge(input, injected) {
  ...
  assertContractIntentWalletAbuseStoreLifecycle(source.abuseStore);
  const sessionReaderBinding = assertContractIntentWalletSessionReaderLifecycle(
  ...
  if (gatewayBinding.mode !== expectedMode
      ...
      || sessionReaderBinding.mode !== expectedMode) {
    throw new TypeError("wallet edge, session reader, ownership, and gateway modes must match");
  }
  ...
  if (new Set([
    contractIntentWalletSessionKeyId(source.requesterPrivateKey),
    contractIntentWalletSessionKeyId(source.responsePublicKey),
    sessionReaderBinding.requesterKeyId,
    sessionReaderBinding.responseKeyId,
  ]).size !== 4) {
    throw new TypeError("wallet edge session-reader keys must be separate from gateway keys");
  }
  const context = {
    abuse: null,
    ...
    haltedOnDurableAbuseStore: false,
    haltedOnSessionReaderFailure: false,
  };
  ...
  context.abuse = claimContractIntentWalletAbuseStoreEdge(
    source.abuseStore,
    source.signal,
  );
  context.sessionReader = claimContractIntentWalletSessionReaderEdge(
    source.sessionReader,
    source.signal,
  ).lease;
  const stop = () => {
  ...
  const service = Object.freeze({
    async issue(preflight, webRequest) {
      ...
      try {
        ...
        context.rejected += 1;
        if (error instanceof EdgeFatalError) throw error;
    ...
    status() {
      ...
      const abuse = context.abuse.status();
      const sessionReader = context.sessionReader.status();
      if (context.state === "active" && abuse.state !== "active") {
        context.state = "halted";
        context.haltedOnDurableAbuseStore = true;
      }
      if (context.state === "active" && sessionReader.state !== "active") {
        context.state = "halted";
        context.haltedOnSessionReaderFailure = true;
      }
      return Object.freeze({
        ...
        haltedOnDurableAbuseStore: context.haltedOnDurableAbuseStore,
        haltedOnSessionReaderFailure: context.haltedOnSessionReaderFailure,
        ...
        activeRateWindows: abuse.activeWindows,
        durableRateAcceptedRequests: abuse.acceptedRequests,
        durableRateRejectedRequests: abuse.rateRejectedRequests,
        durableAuthenticatedSessionRateLimit: true,
        remoteAuthenticatedSessionReads: true,
        sessionReaderActiveReads: sessionReader.activeReads,
        sessionReaderInactiveReads: sessionReader.inactiveReads,
        sessionReaderFailedReads: sessionReader.failedReads,
        rawSessionTokensSentToReader: sessionReader.rawSessionTokensSent,
...
export { CONTRACT_INTENT_WALLET_SESSION_QUERY } from "./contract-intent-wallet-session-reader.mjs";
