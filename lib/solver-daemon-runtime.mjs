...
import {
  ...
  fixedPrivatePacketHttpsRequest,
...
const AUTHENTICATED_PRIVATE_PACKET_CLIENTS = new WeakMap();
const SETTLE_INVOICE = "/invoicesrpc.Invoices/SettleInvoice";
const EVM_CLAIM = "evm:claim";
const PRIVATE_PACKET_CLIENT_REQUIRED_FIELDS = Object.freeze([
...
const PRIVATE_PACKET_CLIENT_OPTIONAL_FIELDS = Object.freeze([
  "requestTtlSeconds",
  "timeoutMs",
]);
const TEST_PRIVATE_PACKET_CLIENT_REQUIRED_FIELDS = Object.freeze([
  ...PRIVATE_PACKET_CLIENT_REQUIRED_FIELDS,
  "nowSeconds",
  "randomBytesImpl",
  "requestImpl",
]);
...
function assertEvidenceSettlement(record, settlement, label, { requireReservation = true } = {}) {
  if (record.settlementId !== settlement.settlementId || record.direction !== settlement.direction
      || record.intentDigest !== settlement.contractIntentDigest) {
...
function lightningConfig(value) {
  ...
  if (!value.privateKey || !value.keyId || !value.adapterUrl
      || value.responsePublicKey?.type !== "public"
      || value.responsePublicKey?.asymmetricKeyType !== "ed25519"
      || !value.responseKeyId) {
    throw new Error("Lightning daemon credentials are incomplete");
  }
  return value;
}
...
async function readPaymentProof({ store, settlement, lightning, randomBytesImpl, nowSeconds }) {
  ...
  const proof = await readConfirmedLightningPaymentProof({
    ...
    responsePublicKey: lightning.responsePublicKey,
    responseKeyId: lightning.responseKeyId,
    nowSeconds,
...
function buildAuthenticatedPrivatePacketClient(source, dependencies) {
  const {
    providerOrigin,
    requesterPrivateKey,
    requesterKeyId,
  ...
  const { mode, requestImpl, nowSeconds, randomBytesImpl } = dependencies;
  ...
  const origin = privatePacketProviderOrigin(providerOrigin);
  if (requesterPrivateKey?.type !== "private" || requesterPrivateKey?.asymmetricKeyType !== "ed25519") {
    throw new TypeError("private packet requester key must be a private Ed25519 key handle");
  }
  if (providerPublicKey?.type !== "public" || providerPublicKey?.asymmetricKeyType !== "ed25519") {
    throw new TypeError("private packet provider key must be a public Ed25519 key handle");
  }
  if (createPublicKey(requesterPrivateKey).export({ type: "spki", format: "der" })
    .equals(providerPublicKey.export({ type: "spki", format: "der" }))) {
    throw new TypeError("private packet requester and provider keys must be separate");
  }
  for (const [value, name] of [[requesterKeyId, "requester"], [providerKeyId, "provider"]]) {
    if (!/^[a-z0-9][a-z0-9._-]{0,79}$/.test(String(value ?? ""))) {
      throw new TypeError(`private packet ${name} key identifier is invalid`);
    }
  }
  const safety = integer(minimumEvmSafetySeconds, "private packet minimum EVM safety seconds");
  if (safety === 0 || safety > 86_400) {
    throw new RangeError("private packet minimum EVM safety seconds is outside policy");
  }
  const timeout = integer(timeoutMs, "private packet timeout");
  if (timeout === 0 || timeout > 30_000) throw new RangeError("private packet timeout is outside policy");
  if (typeof requestImpl !== "function" || typeof nowSeconds !== "function" || typeof randomBytesImpl !== "function") {
    throw new TypeError("private packet client dependencies are invalid");
  }
  const client = Object.freeze({
    async read({ settlement, action = null, purpose }) {
      ...
      return fetchVerifiedPrivatePacket({
        ...
        minimumEvmSafetySeconds: safety,
        requestImpl,
        nowSeconds,
        timeoutMs: timeout,
      });
  ...
  AUTHENTICATED_PRIVATE_PACKET_CLIENTS.set(
    client,
    Object.freeze({ mode }),
  );
  return client;
...
export function createAuthenticatedPrivatePacketClient(input) {
  const source = exactDataRecord(
    input,
    PRIVATE_PACKET_CLIENT_REQUIRED_FIELDS,
    PRIVATE_PACKET_CLIENT_OPTIONAL_FIELDS,
    "production private-packet client",
  );
  return buildAuthenticatedPrivatePacketClient(source, Object.freeze({
...
export function createTestAuthenticatedPrivatePacketClient(input) {
  const source = exactDataRecord(
    input,
    TEST_PRIVATE_PACKET_CLIENT_REQUIRED_FIELDS,
    PRIVATE_PACKET_CLIENT_OPTIONAL_FIELDS,
    "test private-packet client",
  );
  return buildAuthenticatedPrivatePacketClient(source, Object.freeze({
    mode: "injected-test",
...
export function isAuthenticatedPrivatePacketClient(value) {
  return Boolean(value && AUTHENTICATED_PRIVATE_PACKET_CLIENTS.has(value));
}

export function authenticatedPrivatePacketClientTransportMode(value) {
  const metadata = value && AUTHENTICATED_PRIVATE_PACKET_CLIENTS.get(value);
  if (!metadata) throw new TypeError("private-packet client lacks same-process factory provenance");
  return metadata.mode;
}

export function isProductionAuthenticatedPrivatePacketClient(value) {
  return AUTHENTICATED_PRIVATE_PACKET_CLIENTS.get(value)?.mode === "fixed-node-https";
}
    ...
    case "AUTHORIZE_AND_DISPATCH_LIGHTNING": {
      ...
      try {
        const dispatched = await dispatchLightningAction({
          ...
          responsePublicKey: config.responsePublicKey,
