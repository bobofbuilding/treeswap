...
export async function executeSolverDaemonStep({
  ...
  switch (step.kind) {
    case "WAIT_FOR_RESERVATION": {
      ...
      let record;
      try {
        ({ record } = verifiedSolverDaemonEvidence(verification, { now, expectedKind: "RESERVATION" }));
        ...
        return halt(store, step, "DAEMON_RESERVATION_EVIDENCE", error.message, now, beforeSideEffect);
      }
      const observed = {
    ...
    case "AUTHORIZE_AND_DISPATCH_LIGHTNING": {
      ...
      try {
        validateDispatchAuthorization(verification, {
          action,
        ...
        return halt(store, step, "DAEMON_AUTH_MISMATCH", error.message, now, beforeSideEffect);
      }
      try {
    ...
    case "VERIFY_REFUND_AND_RECONCILE": {
      ...
      try {
        ...
        return halt(store, step, "DAEMON_ASSET_MISMATCH", error.message, now, beforeSideEffect);
      }
