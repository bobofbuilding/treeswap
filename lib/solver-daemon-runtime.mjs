...
export function createAuthenticatedPrivatePacketClient({
  ...
  AUTHENTICATED_PRIVATE_PACKET_CLIENTS.set(
    client,
    requestImpl === fixedPrivatePacketHttpsRequest ? "fixed-node-https" : "injected-test",
  );
  return client;
}
...
export function authenticatedPrivatePacketClientTransportMode(value) {
  const mode = value && AUTHENTICATED_PRIVATE_PACKET_CLIENTS.get(value);
  if (!mode) throw new TypeError("private-packet client lacks same-process factory provenance");
  return mode;
}
