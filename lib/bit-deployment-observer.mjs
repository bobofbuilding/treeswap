...
import { validatePublishedMainSource } from "./published-source.mjs";

export { TREESWAP_CANONICAL_ORIGIN } from "./published-source.mjs";
...
export const BIT_RPC_MAXIMUM_RESPONSE_BYTES = 1_000_000;
...
const COMPARISON_REPORT_FIELDS = Object.freeze([
  "comparedAt",
  "comparedFields",
  "eligible",
  "evidenceStatus",
...
const COMPARISON_POLICY_FIELDS = Object.freeze([
  "maximumObservationAgeSeconds",
  "maximumObservationClockSkewSeconds",
  "maximumProviderObservationSkewSeconds",
]);

const COMPARISON_OBSERVATION_FIELDS = Object.freeze([
  "observationDigest",
  "providerIdentity",
  "providerLabel",
]);

export const BIT_COMPARISON_COMPARED_FIELDS = Object.freeze([
  "schema",
  "sourceCommit",
  "chainId",
...
export function validateBitComparisonSourceProvenance(comparison, provenance) {
  const sourceCommit = validateBitObservationSourceProvenance(provenance);
  if (!comparison || comparison.schema !== BIT_DEPLOYMENT_COMPARISON_SCHEMA
      || comparison.sourceCommit !== sourceCommit) {
    throw new Error("BIT comparison requires observations from the exact clean commit published on origin/main");
  }
  return sourceCommit;
}
...
export function compareBitDeploymentObservations(left, right, { comparedAt = new Date() } = {}) {
  ...
  const comparedFields = BIT_COMPARISON_COMPARED_FIELDS;
...
export function buildBitDeploymentComparisonReport(left, right, { comparedAt = new Date() } = {}) {
  ...
  return normalizeBitDeploymentComparisonReport({
...
export function normalizeBitDeploymentComparisonReport(raw, name = "BIT provider comparison") {
  exactKeys(raw, COMPARISON_REPORT_FIELDS, name);
  if (raw.schema !== BIT_DEPLOYMENT_COMPARISON_SCHEMA) throw new TypeError(`${name} schema is unsupported`);
  if (raw.evidenceStatus !== "unreviewed-provider-comparison") {
    throw new TypeError(`${name} evidence status is unsupported`);
  }
  if (raw.independenceStatus !== "requires-external-organizational-verification") {
    throw new TypeError(`${name} independence status is unsupported`);
  }
  if (raw.fundingAuthorization !== false) throw new Error(`${name} may not authorize funding`);
  const comparedAt = canonicalIso(raw.comparedAt, `${name}.comparedAt`);
  exactKeys(raw.policy, COMPARISON_POLICY_FIELDS, `${name}.policy`);
  const policy = Object.freeze({
    maximumObservationAgeSeconds: safeInteger(
      raw.policy.maximumObservationAgeSeconds,
      `${name}.policy.maximumObservationAgeSeconds`,
    ),
    maximumObservationClockSkewSeconds: safeInteger(
      raw.policy.maximumObservationClockSkewSeconds,
      `${name}.policy.maximumObservationClockSkewSeconds`,
    ),
    maximumProviderObservationSkewSeconds: safeInteger(
      raw.policy.maximumProviderObservationSkewSeconds,
      `${name}.policy.maximumProviderObservationSkewSeconds`,
    ),
  });
  if (policy.maximumObservationAgeSeconds !== BIT_OBSERVATION_MAXIMUM_AGE_SECONDS
      || policy.maximumObservationClockSkewSeconds !== BIT_OBSERVATION_MAXIMUM_CLOCK_SKEW_SECONDS
      || policy.maximumProviderObservationSkewSeconds !== BIT_OBSERVATION_MAXIMUM_PROVIDER_SKEW_SECONDS) {
    throw new Error(`${name} policy does not match the repository limits`);
  }
  if (!Array.isArray(raw.observations) || raw.observations.length !== 2) {
    throw new TypeError(`${name}.observations must contain exactly two providers`);
  }
  const observations = raw.observations.map((value, index) => {
    exactKeys(value, COMPARISON_OBSERVATION_FIELDS, `${name}.observations[${index}]`);
    return Object.freeze({
      providerIdentity: bytes32(
        value.providerIdentity,
        `${name}.observations[${index}].providerIdentity`,
        { nonzero: true },
      ),
      providerLabel: providerLabel(value.providerLabel, `${name}.observations[${index}].providerLabel`),
      observationDigest: bytes32(
        value.observationDigest,
        `${name}.observations[${index}].observationDigest`,
        { nonzero: true },
      ),
    });
  });
  if (observations[0].providerIdentity.localeCompare(observations[1].providerIdentity) >= 0) {
    throw new Error(`${name}.observations must be strictly ordered by provider identity`);
  }
  if (observations[0].providerLabel.toLowerCase() === observations[1].providerLabel.toLowerCase()) {
    throw new Error(`${name} providers must have distinct labels`);
  }
  if (observations[0].observationDigest === observations[1].observationDigest) {
    throw new Error(`${name} provider observation digests must be distinct`);
  }
  const finalizedBlock = normalizeRecordedBlock(raw.finalizedBlock, `${name}.finalizedBlock`);
  const sourceCommit = String(raw.sourceCommit ?? "");
  if (!GIT_COMMIT.test(sourceCommit)) throw new TypeError(`${name}.sourceCommit must be full lowercase hex`);
  if (!Array.isArray(raw.reasons) || raw.reasons.length > 64
      || raw.reasons.some((reason) => typeof reason !== "string" || reason.length === 0 || reason.length > 256)) {
    throw new TypeError(`${name}.reasons is invalid`);
  }
  const reasons = Object.freeze([...raw.reasons]);
  if (typeof raw.eligible !== "boolean" || raw.eligible !== (reasons.length === 0)) {
    throw new Error(`${name}.eligible does not match its reasons`);
  }
  if (!Array.isArray(raw.comparedFields)
      || JSON.stringify(raw.comparedFields) !== JSON.stringify(BIT_COMPARISON_COMPARED_FIELDS)) {
    throw new Error(`${name}.comparedFields does not match the repository comparison`);
  }
  const normalized = Object.freeze({
    ...
    reasons,
    comparedFields: BIT_COMPARISON_COMPARED_FIELDS,
    independenceStatus: "requires-external-organizational-verification",
    fundingAuthorization: false,
  });
  if (JSON.stringify(canonical(normalized)) !== JSON.stringify(canonical(raw))) {
...
export function bitDeploymentComparisonValueDigest(comparison) {
  return valueDigest(normalizeBitDeploymentComparisonReport(comparison));
}
...
export async function observeBitDeployment({
  ...
  const [closingChainIdValue, closingFinalizedValue, closingBlockValue] = await Promise.all([
    rpcCall("eth_chainId", []),
    rpcCall("eth_getBlockByNumber", ["finalized", false]),
    rpcCall("eth_getBlockByNumber", [exactBlockTag, false]),
  ]);
  const closingChainId = requireHexQuantity(closingChainIdValue, "closing chain ID");
  if (closingChainId !== chainId) throw new Error("RPC chain changed during the BIT observation");
  const closingFinalizedHead = requireBlock(closingFinalizedValue, "closing finalized");
  const closingObservedBlock = requireBlock(closingBlockValue, "closing target");
  if (closingFinalizedHead.number < finalizedHead.number) {
    throw new Error("RPC finalized head regressed during the BIT observation");
  }
  if (closingFinalizedHead.number === finalizedHead.number
      && (closingFinalizedHead.hash !== finalizedHead.hash
        || closingFinalizedHead.timestamp !== finalizedHead.timestamp)) {
    throw new Error("RPC finalized head changed at the same height during the BIT observation");
  }
  if (closingFinalizedHead.number < requestedBlockNumber) {
    throw new Error("BIT target block was no longer finalized after observation");
  }
  if (closingFinalizedHead.number === requestedBlockNumber
      && closingFinalizedHead.hash !== observedBlock.hash) {
    throw new Error("BIT target block no longer matched the finalized head after observation");
  }
  if (closingObservedBlock.number !== observedBlock.number
      || closingObservedBlock.hash !== observedBlock.hash
      || closingObservedBlock.timestamp !== observedBlock.timestamp) {
    throw new Error("BIT target block changed during the anchored observation");
  }
  ...
  const observation = {
    ...
    chainId,
    providerFinalizedHead: {
      number: closingFinalizedHead.number,
      hash: closingFinalizedHead.hash,
      timestamp: new Date(closingFinalizedHead.timestamp * 1_000).toISOString(),
    },
    finalizedBlock: {
...
async function readBoundedRpcResponse(response, method, maximumResponseBytes) {
  if (response.redirected === true) throw new Error(`Ethereum RPC redirected ${method}`);
  const contentType = response.headers?.get?.("content-type");
  if (contentType && !/^application\/json(?:\s*;|$)/i.test(contentType)) {
    throw new Error(`Ethereum RPC returned a non-JSON response for ${method}`);
  }
  const contentLength = response.headers?.get?.("content-length");
  if (contentLength !== null && contentLength !== undefined && contentLength !== "") {
    if (!/^(?:0|[1-9][0-9]*)$/.test(contentLength)
        || BigInt(contentLength) > BigInt(maximumResponseBytes)) {
      throw new Error(`Ethereum RPC response exceeds its size limit for ${method}`);
    }
  }
  ...
  if (response.body?.getReader) {
    ...
    try {
      ...
      throw new Error(`Ethereum RPC returned invalid UTF-8 for ${method}`);
    }
    ...
    throw new Error(`Ethereum RPC returned an unreadable response for ${method}`);
  }
...
export function createJsonRpcClient(
  ...
  { timeoutMs = 10_000, maximumResponseBytes = BIT_RPC_MAXIMUM_RESPONSE_BYTES } = {},
) {
  let url;
  try {
    url = new URL(rpcUrl);
  ...
  if (!Number.isSafeInteger(maximumResponseBytes)
      ...
      || maximumResponseBytes > BIT_RPC_MAXIMUM_RESPONSE_BYTES) {
    throw new RangeError("Ethereum RPC response-size limit is outside policy");
  }
  ...
  return async function rpcCall(method, params) {
    ...
    const payload = await readBoundedRpcResponse(response, method, maximumResponseBytes);
    if (!payload || typeof payload !== "object" || Array.isArray(payload)) {
      throw new Error(`Ethereum RPC returned an invalid envelope for ${method}`);
    }
    ...
    if (hasError === hasResult) throw new Error(`Ethereum RPC returned an invalid result envelope for ${method}`);
    if (hasError) throw new Error(`Ethereum RPC rejected ${method} with code ${payload.error?.code ?? "unknown"}`);
