...
function resultCounts(discovered, eligible, results, attempted, preflightHalted = 0) {
  const counts = {
    ...EMPTY_COUNTS,
    discovered,
    eligible,
    ...
    halted: preflightHalted,
    backlog: Math.max(eligible - attempted, 0),
  };
...
export function createCoordinatorActiveExecutionLifecycle(input) {
  ...
  async function performCycle() {
    ...
    try {
      ...
      const halted = eligible.filter(({ settlement }) => settlement.haltCode !== null);
      const runnable = eligible.filter(({ settlement, policy }) => (
        policy !== null && settlement.haltCode === null
      ));
      const cycleResults = [];
      if (unmatched.length > 0 || halted.length > 0) {
        ...
        const cycleDigest = coordinatorCommitmentDigest({
          schema: "treeswap.coordinator-active-execution-cycle.v2",
          releaseRecordDigest: policyPreparation.releaseRecordDigest,
          policySetDigest: policyPreparation.policySetDigest,
          ...
          haltedSettlementIds: halted.map(({ settlement }) => settlement.settlementId),
          attempted: 0,
          results: [],
        });
      ...
      const ordered = rotateAfterCursor(runnable, cursor);
      ...
      for (const { settlement, policy } of selected) {
        ...
        if (result.outcome === "HALTED" || result.outcome === "GATE_CLOSED") break;
      }
      const counts = resultCounts(
        settlements.length,
        runnable.length,
      ...
      const cycleDigest = coordinatorCommitmentDigest({
        ...
        haltedSettlementIds: cycleResults
          .filter(({ outcome }) => outcome === "HALTED")
          .map(({ settlementId }) => settlementId),
        attempted: cycleResults.length,
        results: cycleResults,
      });
      lastSuccessAt = lastAttemptAt;
      consecutiveFailures = 0;
      currentStatus = statusRecord({
        state: counts.gateClosed === 0 && counts.halted === 0 ? "active" : "degraded",
        startedAt,
        lastAttemptAt,
        lastSuccessAt,
        consecutiveFailures,
