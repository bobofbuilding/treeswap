...
const EXECUTION_ROUTE_FIELDS = Object.freeze([
  "client",
  "policy",
  "quotePolicy",
...
const EXECUTION_ROUTES = new WeakMap();
const LOWER_NONZERO_BYTES32 = /^0x(?!0{64}$)[0-9a-f]{64}$/;

class PendingFinalizationError extends Error {
  constructor(message = "selected-solver finalization remains pending") {
    super(message);
    this.name = "PendingFinalizationError";
  }
}
...
function pendingResponse(policy, includeCors = true) {
  const response = jsonResponse(425, Object.freeze({
...
function reservationTokenKey(value) {
  if (typeof value !== "string" || !LOWER_NONZERO_BYTES32.test(value)) {
    throw new TypeError("RFQ execution ceremony reservation token is invalid");
  }
...
function createExecutionRoute(input, expectedMode) {
  const source = exactDataRecord(input, EXECUTION_ROUTE_FIELDS, "RFQ execution ceremony route input");
  const policy = normalizedPolicy(source.policy);
  if (!isRfqSelectionReservationService(source.selectionReservation)
      || rfqSelectionReservationServiceMode(source.selectionReservation) !== expectedMode) {
    throw new TypeError("RFQ execution ceremony requires a matching reservation service");
  }
  if (!(source.signal instanceof AbortSignal) || source.signal.aborted) {
    throw new TypeError("RFQ execution ceremony requires an active deployment AbortSignal");
  }
  const deploymentSignal = source.signal;
  const finalizationLease = claimRfqSelectedSolverFinalizationOwnership(
    source.selectionReservation,
    Object.freeze({
  ...
  const stop = (forced = false) => {
    if (context.state === "stopped") return;
    if (!forced && pendingFinalizations.size > 0) {
      throw new Error("RFQ execution ceremony cannot stop while finalization is pending");
    }
    context.state = "stopped";
    lifecycle.abort();
    try { finalizationLease.close(); } catch {}
    deploymentSignal.removeEventListener("abort", abortDeployment);
  };
  const abortDeployment = () => stop(true);
  deploymentSignal.addEventListener("abort", abortDeployment, { once: true });

  const route = Object.freeze({
    async handle(webRequest) {
      if (this !== route || EXECUTION_ROUTES.get(this) !== context) {
        throw new TypeError("RFQ execution ceremony route lacks factory provenance");
      }
      context.started += 1;
      const saturated = context.inFlight >= policy.maximumInFlightRequests;
      context.inFlight += 1;
      ...
      const deadline = new Promise((_, reject) => {
        timer = setNodeTimeout(() => {
          controller.abort();
          reject(new PendingFinalizationError("RFQ execution ceremony timed out"));
        }, policy.maximumProcessingMilliseconds);
      });
      let includeCors = false;
      try {
        if (context.state !== "active" || !(webRequest instanceof Request)) {
          throw new Error("RFQ execution ceremony request is invalid");
        }
        if (saturated) throw new Error("RFQ execution ceremony request capacity is exhausted");
        const url = new URL(webRequest.url);
        includeCors = webRequest.headers.get("origin") === policy.clientOrigin;
        if (url.origin !== policy.apiOrigin
            || (url.pathname !== "/v1/selection/finalize"
              && url.pathname !== "/v1/selection/authorize")
            || url.search || url.hash || !includeCors) {
          throw new Error("RFQ execution ceremony target or client origin is invalid");
        }
        if (webRequest.method === "OPTIONS") return preflightResponse(webRequest, policy);
        if (webRequest.method !== "POST") throw new Error("RFQ execution ceremony method is invalid");
        const body = await Promise.race([
          strictRequestJson(webRequest, policy.maximumRequestBytes, controller.signal),
          deadline,
        ]);
        if (controller.signal.aborted) throw new Error("RFQ execution ceremony request was aborted");
        let result;
        if (url.pathname === "/v1/selection/finalize") {
          const command = exactDataRecord(body, FINALIZE_FIELDS, "RFQ execution ceremony finalization");
          const key = reservationTokenKey(command.reservationToken);
          if (pendingFinalizations.has(key)) throw new PendingFinalizationError();
          if (pendingFinalizations.size >= policy.maximumInFlightRequests) {
            throw new Error("RFQ execution ceremony pending-finalization capacity is exhausted");
          }
          const operation = Promise.resolve().then(() => finalizationLease.finalize(command));
          pendingFinalizations.set(key, operation);
          void operation.finally(() => {
            if (pendingFinalizations.get(key) === operation) pendingFinalizations.delete(key);
          }).catch(() => {});
          const interrupted = new Promise((_, reject) => {
            controller.signal.addEventListener("abort", () => {
              reject(new PendingFinalizationError("RFQ execution ceremony request was interrupted"));
            }, { once: true });
          });
          result = await Promise.race([operation, deadline, interrupted]);
          if (result?.schema === "treeswap.selected-solver-execution-signing-payload.v1") {
            context.finalized += 1;
          }
        } else {
          result = finalizationLease.authorize(exactDataRecord(
            body,
            AUTHORIZE_FIELDS,
            "RFQ execution ceremony authorization",
          ));
          context.authorized += 1;
        }
        if (controller.signal.aborted) throw new PendingFinalizationError();
        context.completed += 1;
        return jsonResponse(200, result, policy);
      } catch (error) {
        if (error instanceof PendingFinalizationError || error?.ambiguous === true) {
          context.pending += 1;
          return pendingResponse(policy, includeCors);
        }
        context.failed += 1;
        return rejectedResponse(policy, includeCors);
      } finally {
    ...
    status() {
      ...
      return Object.freeze({
        ...
        requestsCompleted: context.completed,
        requestsRejected: context.failed,
        requestsPending: context.pending,
        requestsInFlight: context.inFlight,
        finalizationsInFlight: pendingFinalizations.size,
        executionSigningPayloadsPrepared: context.finalized,
        executionAuthorizationsCompleted: context.authorized,
        bearerTokensInStatus: false,
        privateTermsInStatus: false,
        networkListener: false,
        signingAuthority: false,
  ...
  EXECUTION_ROUTES.set(route, context);
  return route;
}

export function createRfqExecutionCeremonyRoute(input) {
  return createExecutionRoute(input, "production");
}

export function createTestRfqExecutionCeremonyRoute(input) {
  return createExecutionRoute(input, "injected-test");
}

export function isRfqExecutionCeremonyRoute(value) {
  return Boolean(value && EXECUTION_ROUTES.has(value));
}
