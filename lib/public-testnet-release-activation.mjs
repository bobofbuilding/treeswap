...
export const PUBLIC_TESTNET_RECOVERY_ACTIVATION_INPUT_SCHEMA =
  "treeswap.public-testnet-recovery-activation-inputs.v1";
export const PUBLIC_TESTNET_RECOVERY_ACTIVATION_SUMMARY_SCHEMA =
  "treeswap.public-testnet-recovery-activation-summary.v1";
...
function normalizeRecoveryActivationManifest(input) {
  exactKeys(input, RECOVERY_MANIFEST_FIELDS, "public-testnet recovery activation manifest");
  if (input.schema !== PUBLIC_TESTNET_RECOVERY_ACTIVATION_INPUT_SCHEMA) {
    throw new TypeError("public-testnet recovery activation manifest schema is invalid");
  }
  const candidateKind = hasExactKeys(input.candidateEvidence, PUBLIC_TESTNET_RELEASE_EVIDENCE_PATH_FIELDS)
    ? "campaign-qualified"
    : hasExactKeys(input.candidateEvidence, PUBLIC_TESTNET_BOOTSTRAP_RELEASE_EVIDENCE_PATH_FIELDS)
      ...
      : null;
  if (!candidateKind) {
    throw new TypeError("public-testnet recovery activation candidate evidence fields are not exact");
  }
  const candidateEvidence = Object.create(null);
  for (const [field, value] of Object.entries(input.candidateEvidence ?? {})) {
    candidateEvidence[field] = absolutePath(value, `recovery candidate evidence ${field}`);
  }
  const manifest = Object.freeze({
    schema: input.schema,
    candidateKind,
    candidateEvidence: Object.freeze(candidateEvidence),
    approvalBundle: absolutePath(input.approvalBundle, "recovery release approval bundle"),
    providerConfiguration: absolutePath(input.providerConfiguration, "recovery provider configuration"),
  });
  const allPaths = [
    ...Object.values(manifest.candidateEvidence),
    manifest.approvalBundle,
  ...
  if (new Set(allPaths).size !== allPaths.length) {
    throw new Error("public-testnet recovery activation inputs must use distinct files");
  }
  return manifest;
}
...
export async function activatePublicTestnetRecoveryFromManifest({
  manifestPath,
  environment = process.env,
  fetchImpl = globalThis.fetch,
  now = Math.floor(Date.now() / 1_000),
  ...
  const path = absolutePath(manifestPath, "public-testnet recovery activation manifest");
  const manifest = normalizeRecoveryActivationManifest(await readBoundedJson(
    path,
    "public-testnet recovery activation manifest",
    { maximumBytes: 65_536 },
  ));
  const [candidate, approvalBundle, providerConfiguration] = await Promise.all([
    manifest.candidateKind === "bootstrap"
      ? rebuildPublicTestnetBootstrapReleaseCandidateFromFiles(manifest.candidateEvidence)
      : rebuildPublicTestnetReleaseCandidateFromFiles(manifest.candidateEvidence),
    readBoundedJson(manifest.approvalBundle, "public-testnet recovery approval bundle"),
    readBoundedJson(manifest.providerConfiguration, "public-testnet recovery provider configuration", {
      maximumBytes: 65_536,
    }),
  ]);
  const inspected = inspectPreparedPublicTestnetReleaseCandidate(candidate);
...
export function buildPublicTestnetRecoveryActivationSummary(result) {
  if (!result || typeof result !== "object" || !result.activation || !result.candidate) {
    throw new TypeError("same-process public-testnet recovery activation result is required");
  }
  const { activation, candidate } = result;
  if (activation.status !== "same-process-recovery-only-runtime-verification-active"
      || activation.releaseId !== candidate.record.releaseId) {
    throw new Error("public-testnet recovery activation result is inconsistent");
  }
  return Object.freeze({
    schema: PUBLIC_TESTNET_RECOVERY_ACTIVATION_SUMMARY_SCHEMA,
    status: "same-process-recovery-only-activation-passed",
    scope: "summary-only-no-reusable-context-lightning-dispatch-new-exposure-or-funding-authority",
    releaseId: activation.releaseId,
    validUntil: activation.validUntil,
    recordDigest: candidate.recordDigest,
    policyDigest: candidate.policyDigest,
    ...
    runtime: Object.freeze({
      ...
      balancesReconciled: activation.deployment.balancesReconciled,
    }),
    authorizations: Object.freeze({
      funding: false,
