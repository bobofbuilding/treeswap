...
import {
  assertRfqQuoteIngressReaderOwnershipAvailable,
...
import {
  assertRfqSelectionReservationOwnershipAvailable,
...
const ROUTE_FIELDS = Object.freeze([
  "policy", "quoteReader", "replayStore", "selectionReservation", "signal",
]);
const TEST_ROUTE_FIELDS = Object.freeze([
  "nowSeconds", "policy", "quoteReader", "randomBytesImpl", "replayStore", "selectionReservation", "signal",
]);
...
function createRoute(input, expectedMode, { nowSeconds, randomBytesImpl }) {
  ...
  if (!isRfqSelectionReservationService(source.selectionReservation)
    ...
    throw new TypeError("RFQ quote ingress route requires a matching factory-created selection reservation service");
  }
  ...
  assertRfqQuoteIngressReaderOwnershipAvailable(quoteReader, deploymentSignal);
  assertRfqSelectionReservationOwnershipAvailable(source.selectionReservation, deploymentSignal);
  const quoteReaderLease = claimRfqQuoteIngressReaderOwnership(quoteReader, deploymentSignal);
  const selectionReservationLease = claimRfqSelectionReservationOwnership(
  ...
  const stop = () => {
    ...
    try { selectionReservationLease.close(); } catch {}
  ...
  const handleSelection = async (body, signal) => {
    ...
    try {
      ...
      const handoff = selectionReservationLease.accept(Object.freeze({
...
export function createTestRfqQuoteIngressRoute(input) {
  ...
  return createRoute(Object.freeze({
    ...
    selectionReservation: source.selectionReservation,
