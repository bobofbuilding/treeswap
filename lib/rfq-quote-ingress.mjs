...
const READER_INPUT_FIELDS = Object.freeze(["pricing", "signal"]);
const READER_FACTORY_FIELDS = Object.freeze(["read"]);
...
function createReader(input, mode) {
  const source = exactDataRecord(input, READER_FACTORY_FIELDS, "RFQ quote ingress reader input");
  if (typeof source.read !== "function") throw new TypeError("RFQ quote ingress reader requires a read function");
  const readImpl = source.read;
  const reader = Object.freeze({
    async read(inputValue) {
      if (this !== reader || !READERS.has(this)) throw new TypeError("RFQ quote ingress reader lacks factory provenance");
      const call = exactDataRecord(inputValue, READER_INPUT_FIELDS, "RFQ quote ingress reader call");
      const pricing = publicPricing(call.pricing);
      if (!(call.signal instanceof AbortSignal)) throw new TypeError("RFQ quote ingress reader requires an AbortSignal");
      return readImpl(pricing, { signal: call.signal });
    },
  });
  READERS.set(reader, Object.freeze({ mode }));
  return reader;
}

export function createRfqQuoteIngressReader(input) {
  return createReader(input, "production");
}

export function createTestRfqQuoteIngressReader(input) {
  return createReader(input, "injected-test");
}
...
function createRoute(input, expectedMode, { nowSeconds, randomBytesImpl }) {
  ...
  if (!isRfqQuoteIngressReader(source.quoteReader) || READERS.get(source.quoteReader)?.mode !== expectedMode) {
  ...
  const handleCreate = async (body, signal) => {
    ...
    const session = await quoteReader.read({ pricing, signal });
