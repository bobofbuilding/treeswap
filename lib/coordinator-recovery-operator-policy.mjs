...
import { isAuthenticatedPrivatePacketClient } from "./solver-daemon-runtime.mjs";
...
export function createCoordinatorRecoveryOperatorPolicyPreparer(input) {
  ...
  const policies = Object.freeze(exactArray(source.policies, "recovery operator policies").map((entry, index) => {
    ...
    if (!isSolverCapabilityClient(policySource.capabilityClient)) {
      throw new TypeError("recovery operator policy requires the concrete solver capability client");
    }
