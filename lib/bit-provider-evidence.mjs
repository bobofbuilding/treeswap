...
export function normalizeBitProviderEvidencePolicy(raw) {
...
export function buildBitProviderDeploymentEvidence(verification) {
  ...
  const providerHeads = verification.observations
    .map((value) => Object.freeze({
      providerIdentity: value.providerIdentity,
      observationDigest: verification.record.providerObservations.find(
        (entry) => entry.providerIdentity === value.providerIdentity,
      ).observationDigest,
      observedAt: value.observedAt,
      providerFinalizedHead: value.providerFinalizedHead,
    }))
    .sort((left, right) => left.providerIdentity.localeCompare(right.providerIdentity));
  return Object.freeze({
    schema: "treeswap.bit-provider-deployment-evidence.v1",
    status: verification.status,
    scope: "provider-signed-mainnet-bit-deployment-no-funding-authorization",
    ...
    stateAnchor: observation.stateAnchor,
    proxy: observation.proxy,
    implementation: observation.implementation,
    token: observation.token,
    providerHeads: Object.freeze(providerHeads),
