...
export const SOLVER_DAEMON_EVIDENCE_REPLAY_SCHEMA =
  "treeswap.solver-daemon-evidence-replay.v2";
...
const OBSERVE_TIME_KEYS = Object.freeze(["now"]);
...
function storedClockHighWater(database) {
  const row = database.prepare(`
    SELECT value FROM ${META_TABLE} WHERE key = 'clock_high_water'
  `).get();
  if (!row) throw new Error("solver evidence replay clock high-water mark is missing");
  return canonicalStoredInteger(row.value, "solver evidence replay clock high-water mark");
}

function advanceClockHighWater(database, now) {
  const previous = storedClockHighWater(database);
  if (now < previous) {
    throw new Error("solver evidence replay clock regressed");
  }
  ...
  if (Number(updated.changes) !== 1) {
    throw new Error("solver evidence replay clock high-water update failed");
  }
}
...
function verifyDatabase(database, { requireSchema = true } = {}) {
  ...
  const schemaRows = database.prepare(`SELECT key, value FROM ${META_TABLE} ORDER BY key`).all();
  if (schemaRows.length !== 2
      || schemaRows[0].key !== "clock_high_water"
      || schemaRows[1].key !== "schema"
      || schemaRows[1].value !== SOLVER_DAEMON_EVIDENCE_REPLAY_SCHEMA) {
    throw new Error("solver evidence replay database schema is unsupported");
  }
  canonicalStoredInteger(
    schemaRows[0].value,
    "solver evidence replay clock high-water mark",
  );
}
...
export class SolverDaemonEvidenceReplayStore {
  ...
  observeTime(input) {
    this.#assertOpen();
    const source = exactDataRecord(input, OBSERVE_TIME_KEYS, "solver evidence replay clock observation");
    const now = integer(source.now, "solver evidence replay observation time", 1);
    this.#database.exec("BEGIN IMMEDIATE");
    try {
      verifyDatabase(this.#database);
      advanceClockHighWater(this.#database, now);
  ...
  status(input) {
    ...
    verifyDatabase(this.#database);
    if (observedAt < storedClockHighWater(this.#database)) {
      throw new Error("solver evidence replay clock regressed");
    }
...
export async function createSolverDaemonEvidenceProviderRoute(input) {
  ...
  const handle = async (webRequest) => {
    try {
      const startedAt = integer(nowSeconds(), "solver evidence provider request start time", 1);
      ...
      replayStore.observeTime({ now: signedAt });
  ...
  const route = Object.freeze({
    ...
    status: (...statusArguments) => {
      if (statusArguments.length !== 0) {
        throw new TypeError("solver evidence provider status accepts no caller input");
      }
      const now = integer(nowSeconds(), "solver evidence provider status time", 1);
