...
export const COORDINATOR_RECOVERY_VERIFICATION_SCHEMA =
  "treeswap.coordinator-recovery-verification.v1";
...
function inactiveVerification({ lastAttemptAt, lastSuccessAt, consecutiveFailures }) {
  return Object.freeze({
    schema: COORDINATOR_RECOVERY_VERIFICATION_SCHEMA,
    state: "inactive",
...
function activeVerification({ summary, lastAttemptAt, lastSuccessAt }) {
  return Object.freeze({
    ...
    gateOpen: summary.runtime.gateOpen,
    emergencyHalted: summary.runtime.emergencyHalted,
    bitPaused: summary.runtime.bitPaused,
    balancesReconciled: summary.runtime.balancesReconciled,
    authorizations: AUTHORIZATIONS,
  });
}

export function createCoordinatorRecoveryVerificationSupervisor({
  manifestPath,
  environment = process.env,
  fetchImpl = globalThis.fetch,
  timeoutMs = 10_000,
  activate = activatePublicTestnetRecoveryFromManifest,
  deactivate = deactivatePublicTestnetRecovery,
  isActive = isPublicTestnetRecoveryActive,
} = {}) {
  if (typeof manifestPath !== "string" || manifestPath.length === 0) {
    throw new TypeError("coordinator recovery activation manifest path is required");
  }
  if (typeof activate !== "function" || typeof deactivate !== "function" || typeof isActive !== "function") {
    throw new TypeError("coordinator recovery activation dependencies are invalid");
  }
  if (!Number.isSafeInteger(timeoutMs) || timeoutMs <= 0 || timeoutMs > 30_000) {
    throw new RangeError("coordinator recovery provider timeout is outside policy");
  }
  ...
  function observeTime(now, { countFailure = true } = {}) {
    const observedAt = timestamp(now, "coordinator recovery verification time");
  ...
  async function refresh({ now = Math.floor(Date.now() / 1_000) } = {}) {
    if (stopped) throw new Error("coordinator recovery verification supervisor is stopped");
    if (refreshing) throw new Error("coordinator recovery verification refresh is already running");
    refreshing = true;
    try {
      ...
      try {
        ...
        if (!isActive(result.activation)) {
          throw new Error("coordinator recovery verification result is not an active same-process recovery");
        }
        const nextSummary = buildPublicTestnetRecoveryActivationSummary(result);
        if (nextSummary.validUntil < time.observedAt) {
          deactivate(result.activation);
          throw new Error("coordinator recovery verification result is already expired");
        }
        current = result;
        result = null;
        summary = nextSummary;
        ...
        if (result?.activation) {
          try {
            ...
            // The non-authorizing status remains inactive when an injected dependency is malformed.
          }
        }
        consecutiveFailures += 1;
        return inactiveVerification({ lastAttemptAt, lastSuccessAt, consecutiveFailures });
  ...
  function useActiveActivation(callback, { now = Math.floor(Date.now() / 1_000) } = {}) {
    if (typeof callback !== "function") throw new TypeError("active recovery callback is required");
    const verification = status({ now });
    if (verification.state !== "active" || !current) {
      throw new Error("coordinator recovery verification is not active");
    }
