...
async function resolvePinnedPrivateAddress(hostname, lookupImpl) {
  const literalFamily = isIP(hostname);
  const resolved = literalFamily === 0
    ? await lookupImpl(hostname, { all: true, verbatim: true })
    : [{ address: hostname, family: literalFamily }];
  if (!Array.isArray(resolved) || resolved.length === 0) {
    throw new Error("Lightning capacity observer did not resolve");
  }
  ...
  if (normalized.some((entry) => (entry.family !== 4 && entry.family !== 6)
    ...
    throw new Error("Lightning capacity observer resolved outside the private network");
  }
  return normalized[0];
}
...
export async function fixedLightningCapacityHttpsRequest(observerOrigin, envelope, {
  ...
  const target = await resolvePinnedPrivateAddress(hostname, lookupImpl);
