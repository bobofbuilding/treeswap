...
function snapshotCoordinatorEvidenceData(
  value,
  name,
  state = { depth: 0, counter: { value: 0 } },
) {
  ...
  if (Array.isArray(value)) {
    ...
    for (let index = 0; index < value.length; index += 1) {
      ...
      result.push(snapshotCoordinatorEvidenceData(descriptor.value, `${name}[${index}]`, {
        depth: state.depth + 1,
  ...
  for (const key of keys) {
    if (key === "__proto__") {
      throw new TypeError(`${name} contains a forbidden prototype key`);
    }
    ...
    Object.defineProperty(result, key, {
      ...
      enumerable: true,
      writable: false,
      value: snapshotCoordinatorEvidenceData(descriptor.value, `${name}.${key}`, {
        depth: state.depth + 1,
        counter: state.counter,
      }),
    });
...
function snapshotCoordinatorEvidencePolicy(value, label) {
  ...
  return snapshotCoordinatorEvidenceData(value, `${label} evidence policy`);
}
