...
export async function fixedPrivatePacketHttpsRequest(endpoint, options, {
...
export async function fetchVerifiedPrivatePacket({
  ...
  try {
    ...
    if (response?.redirected === true || response?.status !== 200) {
      ...
      throw new Error("private packet provider rejected the request");
    }
    let body;
    try {
      body = await Promise.race([
        readStrictPrivateJsonResponse(response, {
          label: "private packet response",
