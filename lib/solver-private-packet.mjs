...
const VERIFIED_PRIVATE_PACKET = Symbol("TreeSwap verified private packet");
...
export function buildSignedPrivatePacketResponse({
  requestEnvelope,
  requesterPublicKey,
...
export function isVerifiedPrivatePacketResult(value) {
  return Boolean(value?.[VERIFIED_PRIVATE_PACKET] === true);
}
...
function privateProviderUrl(origin) {
  ...
  if (url.protocol !== "http:" || url.username || url.password || url.search || url.hash
    ...
    throw new Error("private packet provider must use an isolated private HTTP origin");
  }
  return new URL("/v1/private-packet", url);
}

async function boundedJson(response) {
  const type = String(response?.headers?.get?.("content-type") ?? "").toLowerCase();
  if (!type.startsWith("application/json")) throw new Error("private packet response content type is invalid");
  const declared = Number(response.headers.get("content-length") ?? 0);
  if (Number.isFinite(declared) && declared > MAX_RESPONSE_BYTES) throw new Error("private packet response is too large");
