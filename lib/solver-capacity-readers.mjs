...
function normalizeProvider(value, position) {
  exactKeys(value, ["label", "rpcCall"], `${position} BIT RPC provider`);
  ...
  return Object.freeze({ label, rpcCall: value.rpcCall });
}
...
export function createFinalizedBitVaultInventoryReader({
  ...
  if (primary.label === secondary.label || primary.rpcCall === secondary.rpcCall) {
    throw new Error("BIT inventory requires two independently labelled RPC providers");
  }
