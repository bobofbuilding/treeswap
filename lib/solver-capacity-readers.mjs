...
const FINALIZED_BIT_VAULT_INVENTORY_READERS = new WeakSet();
...
export function createFinalizedBitVaultInventoryReader({
  ...
  const reader = async function readVerifiedBitInventory(rawRequest) {
  ...
  FINALIZED_BIT_VAULT_INVENTORY_READERS.add(reader);
  return reader;
}

export function isFinalizedBitVaultInventoryReader(value) {
  return Boolean(value && FINALIZED_BIT_VAULT_INVENTORY_READERS.has(value));
}
