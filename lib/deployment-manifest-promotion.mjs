...
import { buildClosedTestnetDeploymentPostflightSummary } from "./closed-testnet-deployment-postflight.mjs";
...
function validateExactManifestShape(manifest) {
  assertExactDeploymentManifestShape(manifest);
...
function normalizePolicy(raw) {
  ...
  if (raw.schema !== "treeswap.deployment-promotion-policy.v2") throw new TypeError("promotion policy schema is invalid");
  if (raw.environment !== "public-testnet") {
    throw new TypeError("promotion v2 supports only the closed public-testnet deployment ceremony");
  }
  ...
  return Object.freeze({
    ...
    postflightRecordDigest: digest(raw.postflightRecordDigest, "policy.postflightRecordDigest"),
    postflightPolicyDigest: digest(raw.postflightPolicyDigest, "policy.postflightPolicyDigest"),
    minimumProviderCount,
    maximumObservationAgeSeconds,
    maximumPromotionLifetimeSeconds,
...
function normalizeRecord(raw, policy) {
  ...
  if (raw.schema !== "treeswap.deployment-promotion-record.v2") throw new TypeError("promotion record schema is invalid");
  ...
  const record = Object.freeze({
    ...
    postflightRecordDigest: digest(raw.postflightRecordDigest, "record.postflightRecordDigest"),
    postflightPolicyDigest: digest(raw.postflightPolicyDigest, "record.postflightPolicyDigest"),
...
function normalizePromotionInputs({ record, policy, deploymentPolicy, observations, postflightVerification }) {
  ...
  const postflight = buildClosedTestnetDeploymentPostflightSummary(postflightVerification);
  for (const [field, actual, expected] of [
    ["record digest", postflight.recordDigest, normalizedRecord.postflightRecordDigest],
    ["policy digest", postflight.policyDigest, normalizedRecord.postflightPolicyDigest],
    ["deployment policy digest", postflight.deploymentPolicyDigest, normalizedRecord.deploymentPolicyDigest],
    ["manifest digest", postflight.manifestDigest, normalizedRecord.manifestDigest],
    ["environment", postflight.environment, normalizedRecord.environment],
    ["chain", postflight.chainId, normalizedRecord.chainId],
    ["gate", postflight.verifyingContract, normalizedRecord.verifyingContract],
    ["source commit", postflight.sourceCommit, normalizedRecord.reviewedBuildCommit],
    ["finalized block number", postflight.finalizedBlockNumber, normalizedRecord.finalizedBlockNumber],
    ["finalized block hash", postflight.finalizedBlockHash, normalizedRecord.finalizedBlockHash],
  ]) {
    if (actual !== expected) throw new Error(`verified deployment postflight ${field} does not match promotion`);
  }
  if (normalizedRecord.promotedAt < postflight.preparedAt || normalizedRecord.promotedAt > postflight.validUntil) {
    throw new Error("deployment promotion was not prepared inside the verified postflight validity window");
  }
  ...
  if (postflight.approverSetDigest !== hash(normalizedPolicy.approvers)) {
    throw new Error("deployment promotion must retain the exact verified postflight approver set");
  }
  if (postflight.providerSetDigest !== hash(
    normalizedRecord.providerObservations.map((value) => value.providerIdentity),
  )) {
    throw new Error("deployment promotion must retain the exact verified postflight provider set");
  }
...
export function buildDeploymentPromotionApprovalMessage({
  ...
  const normalized = normalizePromotionInputs({
    record,
    policy,
    deploymentPolicy,
    observations,
...
export function verifyDeploymentManifestPromotion({
  ...
  const result = Object.freeze({
    schema: "treeswap.verified-deployment-manifest-promotion.v2",
    ...
    postflight: normalized.postflight,
  });
...
export function buildDeploymentPromotionReleaseEvidence(verification) {
  ...
  return Object.freeze({
    ...
    deploymentPostflight: verification.postflight.recordDigest,
...
export function buildDeploymentPromotionSummary(verification) {
  if (!verifiedPromotions.has(verification)) throw new Error("deployment promotion provenance is invalid");
  return Object.freeze({
    schema: "treeswap.deployment-promotion-summary.v2",
    promotionDigest: verification.recordDigest,
    postflightRecordDigest: verification.postflight.recordDigest,
    postflightPolicyDigest: verification.postflight.policyDigest,
