...
export async function observeDeploymentManifest({
  ...
  const [controller, guardian, feeCollector, gateRuntime, registryRuntime, vault, userEscrow,
    ...
    symbolResult] = await Promise.all([
...
export function compareDeploymentObservations(left, right) {
  ...
  if (reasons.length === 0) {
    add(left.schema !== "treeswap.deployment-observation.v1", "first deployment observation schema is unsupported");
    add(right.schema !== "treeswap.deployment-observation.v1", "second deployment observation schema is unsupported");
