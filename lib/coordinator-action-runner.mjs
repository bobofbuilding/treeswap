...
import { verifyLightningAdapterResponseEnvelope } from "./lightning-adapter-response.mjs";
import {
  lightningAuthorizationEnvelopeDigest,
  signLightningAuthorizationEnvelope,
} from "./lightning-authorization-envelope.mjs";
...
export function privateLightningAdapterOrigin(baseUrl) {
  if (typeof baseUrl !== "string" || baseUrl.length === 0 || baseUrl.length > 2_048) {
    throw new TypeError("adapter URL must be a bounded string");
  }
  const url = new URL(baseUrl);
  if (url.protocol !== "https:" || (url.port && url.port !== "443")) {
    throw new TypeError("adapter URL must use private HTTPS on port 443");
  }
...
function privateAdapterActionUrl(baseUrl, { allowIsolatedHttp = false } = {}) {
  if (typeof baseUrl !== "string" || baseUrl.length === 0 || baseUrl.length > 2_048) {
    throw new TypeError("adapter URL must be a bounded string");
  }
  const url = new URL(baseUrl);
  if (!allowIsolatedHttp) return new URL("/v1/action", `${privateLightningAdapterOrigin(baseUrl)}/`);
  if (url.protocol !== "http:") throw new TypeError("test adapter URL must use isolated HTTP");
  if (url.username || url.password || url.search || url.hash || url.href !== `${url.origin}/`) {
    throw new TypeError("adapter URL must contain only its private origin");
  }
  if (!isPrivateLndHostname(url.hostname)) throw new TypeError("adapter URL must target an explicitly private hostname");
...
export function fixedLightningAdapterHttpsRequest(endpoint, options) {
  const requestUrl = new URL(endpoint);
  const expected = new URL("/v1/action", `${privateLightningAdapterOrigin(requestUrl.origin)}/`);
  if (requestUrl.href !== expected.href || options?.method !== "POST"
      || options?.redirect !== "error" || typeof options?.body !== "string") {
    throw new Error("Lightning adapter fixed HTTPS request is invalid");
  }
...
async function boundedJson(response, maximumBytes = 262_144, signal = null) {
  const type = String(response?.headers?.get?.("content-type") ?? "").split(";", 1)[0].trim().toLowerCase();
  if (type !== "application/json") throw new Error("adapter response content type is invalid");
  const cacheControl = String(response.headers.get("cache-control") ?? "").toLowerCase();
  if (!cacheControl.split(",").some((directive) => directive.trim() === "no-store")) {
    throw new Error("adapter response must disable storage");
  }
  const declaredHeader = response.headers.get("content-length");
  if (declaredHeader !== null && !/^(?:0|[1-9][0-9]*)$/.test(declaredHeader)) {
    throw new Error("adapter response content length is invalid");
  }
  const declared = Number(declaredHeader ?? 0);
  if (declared > maximumBytes) throw new Error("adapter response exceeded its size limit");
...
function authenticateAdapterResponse(envelope, responsePublicKey, responseKeyId) {
...
function validateAdapterError(envelope, {
  authorizationDigest,
  requestId,
  responsePublicKey,
  responseKeyId,
}) {
  const authenticated = authenticateAdapterResponse(envelope, responsePublicKey, responseKeyId);
  const error = exactDataRecord(
    authenticated.body,
    ["ambiguous", "authorizationDigest", "error", "errorCode", "requestId"],
    "adapter error response",
  );
  if (typeof error.ambiguous !== "boolean") throw new TypeError("adapter error ambiguity is invalid");
  if (error.errorCode !== "NOT_FOUND" && error.errorCode !== "REJECTED") {
    throw new TypeError("adapter error code is invalid");
  }
  if (typeof error.error !== "string" || error.error.length === 0 || error.error.length > 240) {
    throw new TypeError("adapter error message is invalid");
  }
  if (bytes32(error.requestId, "adapter error requestId") !== requestId) {
    throw new Error("adapter error request does not match");
  }
  if (bytes32(error.authorizationDigest, "adapter error authorizationDigest") !== authorizationDigest) {
    throw new Error("adapter error authorization does not match");
  }
  return Object.freeze({
    error: Object.freeze({
      ambiguous: error.ambiguous,
      authorizationDigest: error.authorizationDigest,
      errorCode: error.errorCode,
      requestId: error.requestId,
    }),
    responseAuthentication: authenticated.authentication,
  });
}

function validateAdapterResult(action, operation, envelope, authorization) {
  const authenticated = authenticateAdapterResponse(
    envelope,
    authorization.responsePublicKey,
    authorization.responseKeyId,
  );
  ...
  switch (action.method) {
    case "/invoicesrpc.Invoices/AddHoldInvoice": {
      ...
      return Object.freeze({
        result: Object.freeze({ ...normalized }),
        auditBinding,
        responseAuthentication: authenticated.authentication,
      });
    }
    case "/invoicesrpc.Invoices/SettleInvoice": {
      ...
      return Object.freeze({
        result: Object.freeze({ state: "SETTLED" }),
        auditBinding,
        responseAuthentication: authenticated.authentication,
      });
    }
    case "/invoicesrpc.Invoices/CancelInvoice": {
      ...
      return Object.freeze({
        result: Object.freeze({ state: "CANCELED" }),
        auditBinding,
        responseAuthentication: authenticated.authentication,
      });
    }
...
function validateReconciliationResult(action, method, envelope, authorization) {
  const authenticated = authenticateAdapterResponse(
    envelope,
    authorization.responsePublicKey,
    authorization.responseKeyId,
...
function validateNotFoundResponse(response, envelope, authorization) {
  if (response.status !== 502) throw new Error("adapter NOT_FOUND status is invalid");
  const validated = validateAdapterError(envelope, authorization);
  if (validated.error.ambiguous !== false || validated.error.errorCode !== "NOT_FOUND") {
...
export async function dispatchLightningAction({
  ...
  const actionUrl = privateAdapterActionUrl(adapterUrl, {
    allowIsolatedHttp: requestImpl !== fixedLightningAdapterHttpsRequest,
  });
  ...
  const authorizationDigest = lightningAuthorizationEnvelopeDigest(envelope);
  ...
  try {
    ...
    body = await boundedJson(response, 262_144, controller.signal);
  } catch {
  ...
  if (!response.ok) {
    ...
    try {
      validatedError = validateAdapterError(body, {
        authorizationDigest,
        requestId: action.requestId,
        responsePublicKey,
        responseKeyId,
      ...
      // Unauthenticated or request-unbound errors never gain evidentiary meaning.
    }
    const resultCode = validatedError === null
      ? "UNAUTHENTICATED_ADAPTER_ERROR"
      : validatedError.error.ambiguous === true || response.status === 409 || response.status >= 500
        ? "ADAPTER_AMBIGUOUS"
    ...
    const proof = coordinatorCommitmentDigest({
      ...
      resultCode,
      observedAt,
      ...(validatedError === null ? {} : validatedError),
    });
    store.recordActionResult({
      actionId: action.actionId,
      outcome: "ambiguous",
  ...
  let validated;
  try {
    validated = validateAdapterResult(action, normalizedOperation, body, {
      authorizedAt,
      expiresAt,
      responsePublicKey,
      responseKeyId,
...
export async function reconcileLightningAction({
  ...
  if (!response.ok) {
    let notFound = null;
    try {
      notFound = validateNotFoundResponse(response, body, {
        authorizationDigest,
        requestId,
        responsePublicKey,
        responseKeyId,
...
export async function readConfirmedLightningPaymentProof({
  ...
  if (!response.ok) {
    try {
      ...
      throw new CoordinatorDispatchError("confirmed payment proof lookup returned an unauthenticated rejection", {
        ambiguous: false,
        actionState: "CONFIRMED",
      });
    }
