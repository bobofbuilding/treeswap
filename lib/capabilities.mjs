...
async function observeReleaseRuntimeProvider({
  ...
  return bounded(async () => {
    ...
    if (observation.bitPaused || observation.bitDecimals !== manifest.bit.decimals
...
async function observeReleaseRuntime({ providerSet, candidate, manifest, expectedRiskDigest, now, timeoutMs }) {
...
export async function activatePublicTestnetRelease({
  ...
  verifiedRuntimeSnapshots.set(snapshot, Object.freeze({ capabilities, validUntil: activationValidUntil }));
  return Object.freeze({
