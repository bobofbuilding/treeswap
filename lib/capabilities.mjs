...
const SOLVER_DAEMON_POLICY_FIELDS = Object.freeze([
  "approvers",
  "chainId",
  "direction",
  "maxClockSkewSeconds",
...
const ACTIVE_SOLVER_DAEMON_CONTEXT_FIELDS = Object.freeze([
  "capabilities",
  "deployment",
  "evidencePolicy",
  "solverCapabilityVerification",
]);
const RECOVERY_SOLVER_DAEMON_CONTEXT_FIELDS = Object.freeze([
  "deployment",
  "evidencePolicy",
  "solverCapabilityVerification",
]);
...
export function verifiedPublicTestnetRecoveryActivation(activation, {
  now = Math.floor(Date.now() / 1_000),
  requireActive = true,
} = {}) {
  const liveness = activePublicTestnetRecoveryActivations.get(activation);
  ...
  if (!Number.isSafeInteger(now) || now <= 0) {
    throw new TypeError("public-testnet recovery activation verification time is invalid");
  }
  if (requireActive && (liveness.active !== true || now > liveness.validUntil)) {
    throw new Error("public-testnet recovery activation is inactive or stale");
  }
  const runtime = verifiedRecoveryRuntimeSnapshots.get(activation.deployment);
  if (!runtime || runtime.liveness !== liveness) {
    throw new TypeError("public-testnet recovery activation runtime provenance is invalid");
  }
  return Object.freeze({
    releaseId: activation.releaseId,
    releaseRecordDigest: activation.deployment.releaseRecordDigest,
    releasePolicyDigest: activation.deployment.releasePolicyDigest,
    providerConsensusDigest: activation.providerConsensusDigest,
    gateOpen: activation.deployment.gateOpen,
    emergencyHalted: activation.deployment.emergencyHalted,
    bitPaused: activation.deployment.bitPaused,
    runtimeBlockNumber: activation.runtimeBlockNumber,
    runtimeBlockHash: activation.runtimeBlockHash,
    maximumRuntimeObservationAgeSeconds: runtime.maximumRuntimeObservationAgeSeconds,
    validUntil: liveness.validUntil,
  });
}
...
function normalizeSolverDaemonPolicyBinding(input) {
  const raw = exactDataRecord(
    input,
    SOLVER_DAEMON_POLICY_FIELDS,
    [],
    "solver daemon evidence policy",
  );
  const rawApprovers = exactDataRecord(
    raw.approvers,
    RUNTIME_APPROVAL_ROLES,
    [],
    "solver daemon evidence approvers",
  );
  const snapshot = Object.freeze({
    schema: exactString(raw.schema, "solver daemon evidence policy schema"),
    releaseRecordDigest: exactString(
      raw.releaseRecordDigest,
      "solver daemon release record digest",
    ),
    chainId: exactString(raw.chainId, "solver daemon chainId"),
    settlementContract: exactString(
      raw.settlementContract,
      "solver daemon settlement contract",
    ),
    settlementContractCodeHash: exactString(
      raw.settlementContractCodeHash,
      "solver daemon settlement contract code hash",
    ),
    solver: exactString(raw.solver, "solver daemon solver"),
    direction: exactString(raw.direction, "solver daemon direction"),
    approvers: Object.freeze(Object.fromEntries(RUNTIME_APPROVAL_ROLES.map((role) => [
      role,
      exactString(rawApprovers[role], `solver daemon ${role}`),
    ]))),
    maxClockSkewSeconds: raw.maxClockSkewSeconds,
    maxEvidenceAgeSeconds: raw.maxEvidenceAgeSeconds,
    maxEvidenceLifetimeSeconds: raw.maxEvidenceLifetimeSeconds,
  });
...
export function createActiveSolverDaemonContext(input) {
  const source = exactDataRecord(
    input,
    ACTIVE_SOLVER_DAEMON_CONTEXT_FIELDS,
    ["now"],
    "active solver daemon context input",
  );
  const solverCapabilityVerification = source.solverCapabilityVerification;
  const deployment = source.deployment;
  const capabilities = source.capabilities;
  const evidencePolicy = source.evidencePolicy;
  const now = source.now ?? Math.floor(Date.now() / 1_000);
...
export function createRecoverySolverDaemonContext(input) {
  const source = exactDataRecord(
    input,
    RECOVERY_SOLVER_DAEMON_CONTEXT_FIELDS,
    ["now"],
    "recovery solver daemon context input",
  );
  const solverCapabilityVerification = source.solverCapabilityVerification;
  const deployment = source.deployment;
  const evidencePolicy = source.evidencePolicy;
