...
const LND_FAILURE_REASONS = new Set([
  "already-exists",
  "invoice-not-found",
  "not-found",
  "permission-denied",
...
const LND_CLIENTS = new WeakSet();
const LND_CLIENT_FACTORY_TOKEN = Symbol("TreeSwap LND REST client factory");
...
export function lndFailureReason(value, grpcCode = null) {
  const message = typeof value === "string" ? value.toLowerCase() : "";
  if (/(?:permission denied|not authorized)/.test(message)) return "permission-denied";
  if (/(?:unable to locate invoice|there are no existing invoices|invoice not found)/.test(message)) {
    return "invoice-not-found";
  }
  if (/(?:already exists|invoice with payment hash exists)/.test(message)) return "already-exists";
  if (Number(grpcCode) === 5) return "not-found";
  if (Number(grpcCode) === 6) return "already-exists";
  return "unknown";
}
...
export class LndRestError extends Error {
  constructor(message, {
    ...
    const classifiedReason = reason ?? lndFailureReason("", grpcCode);
    this.reason = LND_FAILURE_REASONS.has(classifiedReason) ? classifiedReason : "unknown";
  }
}
...
export class LndRestClient {
  ...
  constructor({
    ...
    if (factoryToken !== LND_CLIENT_FACTORY_TOKEN) {
      throw new TypeError("LND REST clients must be opened through the credential-file factory");
    }
  ...
  static async create({ baseUrl, macaroonPath, tlsCertPath, expectedCertificateFingerprint, requestImpl, maximumResponseBytes }) {
    ...
    return new LndRestClient({
      ...
      requestImpl,
      maximumResponseBytes,
      factoryToken: LND_CLIENT_FACTORY_TOKEN,
    });
  }
          ...
          try {
            ...
            if ((response.statusCode ?? 500) < 200 || (response.statusCode ?? 500) >= 300) {
              ...
              try {
                ...
                const remoteMessage = typeof parsed?.message === "string"
                  ? parsed.message
                  : (typeof parsed?.error === "string" ? parsed.error : parsed?.error?.message);
                reason = lndFailureReason(remoteMessage, grpcCode);
              } catch {}
  ...
  pendingSweeps(timeoutMs) {
    return this.#call("/v2/wallet/sweeps/pending", { timeoutMs });
  }
...
Object.freeze(LndRestClient.prototype);
Object.freeze(LndRestClient);

export function isLndRestClient(value) {
  return Boolean(value && LND_CLIENTS.has(value));
}
