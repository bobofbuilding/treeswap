...
export function unwrapLndStreamFrame(frame, { requestLabel = "stream RPC", errorAmbiguous = true } = {}) {
  if (!frame || typeof frame !== "object" || Array.isArray(frame)) {
    throw new LndRestError(`LND returned a malformed ${requestLabel} frame`, { ambiguous: true });
  }
  ...
  #call(path, {
    ...
    return new Promise((resolve, reject) => {
        ...
        response.on("data", (chunk) => {
          ...
          try {
            ...
            finish(reject, error instanceof LndRestError
              ? error
              : new LndRestError(`LND returned unusable data for ${requestLabel}`, { ambiguous: true }));
          }
        });
        response.on("end", () => {
          if (settled) return;
          try {
            ...
            if (stream) {
              if (!lastStreamValue) {
                finish(reject, new LndRestError(`LND returned no result for ${requestLabel}`, { ambiguous: true }));
                return;
              }
              finish(resolve, lastStreamValue);
              return;
      ...
      const deadline = setTimeout(() => {
        req.destroy(new LndRestError(`LND timed out during ${requestLabel}`, { ambiguous: true }));
      }, timeout);
      req.on("error", (error) => finish(reject, error instanceof LndRestError
        ? error
        : new LndRestError(`LND transport failed during ${requestLabel}`, { ambiguous: true })));
