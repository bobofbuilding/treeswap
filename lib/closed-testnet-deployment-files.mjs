...
export async function readBoundedJson(path, name, { maximumBytes = 1_000_000 } = {}) {
  ...
  try {
    ...
    if (Buffer.byteLength(bytes) > maximumBytes || after.size !== before.size || after.mtimeMs !== before.mtimeMs) {
...
export async function rebuildReviewedClosedTestnetDeploymentArtifacts({ repository, reviewedBuildCommit }) {
  ...
  const published = git(repository, ["rev-parse", "origin/main"]).trim();
  const status = git(repository, ["status", "--porcelain", "--untracked-files=all"]);
  if (head !== reviewedBuildCommit || published !== reviewedBuildCommit || status !== "") {
  ...
  const publishedAfterBuild = git(repository, ["rev-parse", "origin/main"]).trim();
  const statusAfterBuild = git(repository, ["status", "--porcelain", "--untracked-files=all"]);
  if (headAfterBuild !== reviewedBuildCommit || publishedAfterBuild !== reviewedBuildCommit || statusAfterBuild !== "") {
...
export function verifyPublishedArtifactSources({ artifacts, repository, reviewedBuildCommit }) {
  ...
  const ancestorCheck = spawnSync("git", ["merge-base", "--is-ancestor", reviewedBuildCommit, "origin/main"], {
    cwd: repository,
    encoding: "utf8",
    stdio: ["ignore", "pipe", "pipe"],
  });
  if (ancestorCheck.status !== 0) throw new Error("reviewed build commit is not published on origin/main");
