...
export const RELEASE_APPROVAL_ROLES = Object.freeze([...APPROVAL_ROLES]);
...
const ABSOLUTE_BOOTSTRAP_MAXIMUMS = Object.freeze({
...
const ABSOLUTE_QUALIFIED_TESTNET_MAXIMUMS = Object.freeze({
...
function normalizeFundingMode(value, environment) {
  if (!["closed", "operator-testnet-bootstrap", "operator-testnet"].includes(value)) {
    throw new TypeError("fundingMode is invalid");
  }
  if (value !== "closed" && environment !== "public-testnet") {
...
function normalizeRecord(raw) {
  ...
  if (fundingMode !== "closed") {
    if (fundingMode === "operator-testnet" && normalized.evidenceDigests.publicTestnet === ZERO_DIGEST) {
      throw new Error("publicTestnet evidence is required before qualified operator funding");
    }
    for (const [field, value] of Object.entries(normalized.reviewDigests)) {
      if (value === ZERO_DIGEST) throw new Error(`${field} review evidence is required before operator funding`);
    }
    ...
    if (fundingMode === "operator-testnet-bootstrap") {
      for (const [field, maximum] of Object.entries(ABSOLUTE_BOOTSTRAP_MAXIMUMS)) {
        if (BigInt(normalized.limits[field]) > maximum) {
          throw new Error(`${field} exceeds the absolute testnet-bootstrap maximum`);
        }
      }
    }
    if (fundingMode === "operator-testnet") {
      for (const [field, maximum] of Object.entries(ABSOLUTE_QUALIFIED_TESTNET_MAXIMUMS)) {
        if (BigInt(normalized.limits[field]) > maximum) {
          throw new Error(`${field} exceeds the absolute qualified-testnet maximum`);
        }
      }
    }
  }
...
export function verifiedReleaseAuthorizationBinding(verification) {
  ...
  return Object.freeze({
    ...
    riskPolicyDigest: verification.record.evidenceDigests.riskPolicy,
...
export function verifiedReleaseCapabilityBinding(profile) {
  ...
  return Object.freeze({
    ...
    riskPolicyDigest: profile.riskPolicyDigest,
