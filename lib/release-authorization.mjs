...
function normalizeFundingMode(value, environment) {
  if (!["closed", "operator-testnet", "operator-mainnet-beta"].includes(value)) {
  ...
  if (value === "operator-mainnet-beta" && environment !== "capped-mainnet-beta") {
    throw new Error("mainnet funding mode requires the capped-mainnet-beta environment");
  }
  return value;
}
...
function normalizeRecord(raw) {
  ...
  if (raw.schema !== "treeswap.release-record.v1") throw new TypeError("release record schema is invalid");
  if (!["public-testnet", "capped-mainnet-beta"].includes(raw.environment)) {
    throw new TypeError("release environment is invalid");
  }
  ...
  for (const field of ["deploymentManifest", "admissionPolicy", "riskPolicy", "feeSchedule", "testQualification"]) {
  ...
  if (raw.environment === "capped-mainnet-beta" && fundingMode !== "closed") {
    if (normalized.priorReleaseDigest === ZERO_DIGEST) throw new Error("mainnet beta requires a prior release digest");
    for (const [field, value] of Object.entries(normalized.evidenceDigests)) {
      if (value === ZERO_DIGEST) throw new Error(`${field} evidence is required for mainnet beta`);
    }
    for (const [field, value] of Object.entries(normalized.reviewDigests)) {
      if (value === ZERO_DIGEST) throw new Error(`${field} review is required for mainnet beta`);
    }
  }
  return normalized;
}
...
function normalizePolicy(raw) {
  ...
  if (raw.schema !== "treeswap.release-policy.v1") throw new TypeError("release policy schema is invalid");
  if (!["public-testnet", "capped-mainnet-beta"].includes(raw.environment)) {
    throw new TypeError("release policy environment is invalid");
  }
