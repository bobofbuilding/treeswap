...
import {
  ...
  normalizeBitProviderEvidencePolicy,
...
function assertProviderLink(candidate, providerVerification) {
  ...
  assertProviderReviewerSeparation({
    providerApprovers: providerVerification.policy.providerApprovers,
    reviewApprovers: candidate.policy.reviewApprovers,
    artifacts: candidate.artifacts,
  });
  return providerSummary;
}

function assertProviderReviewerSeparation({ providerApprovers, reviewApprovers, artifacts }) {
  const providerSigners = new Set(
    providerApprovers.map((value) => value.signer.toLowerCase()),
  );
  const providerOrganizations = new Set(
    providerApprovers.map((value) => value.organizationId),
  );
  const providerCommitments = new Set(providerApprovers.flatMap((value) => [
...
export function preflightBitReviewCeremony({
  providerPolicy,
  reviewPolicy,
  artifacts,
  findingCounts,
} = {}) {
  const normalizedProviderPolicy = normalizeBitProviderEvidencePolicy(providerPolicy);
  const normalizedReviewPolicy = normalizePolicy(reviewPolicy);
  const normalizedArtifacts = normalizeArtifacts(artifacts);
  const normalizedFindingCounts = normalizeFindingCounts(findingCounts);
  if (normalizedReviewPolicy.sourceCommit !== normalizedProviderPolicy.sourceCommit
      || normalizedReviewPolicy.chainId !== normalizedProviderPolicy.chainId
      || normalizedReviewPolicy.verifyingContract !== normalizedProviderPolicy.verifyingContract) {
    throw new Error("BIT provider and review policies do not match");
  }
  const effectiveMaximumReviewLifetimeSeconds = Math.min(
    normalizedProviderPolicy.maximumEvidenceLifetimeSeconds,
    normalizedReviewPolicy.maximumReviewLifetimeSeconds,
  );
  if (effectiveMaximumReviewLifetimeSeconds < MINIMUM_REVIEW_LIFETIME_SECONDS) {
    throw new Error("BIT policies do not permit the minimum independent-review lifetime");
  }
  assertProviderReviewerSeparation({
    providerApprovers: normalizedProviderPolicy.providerApprovers,
    reviewApprovers: normalizedReviewPolicy.reviewApprovers,
    artifacts: normalizedArtifacts,
  });
  assertSecretFree({
    providerPolicy: normalizedProviderPolicy,
    reviewPolicy: normalizedReviewPolicy,
    artifacts: normalizedArtifacts,
    findingCounts: normalizedFindingCounts,
  });
  const preflight = Object.freeze({
    schema: "treeswap.bit-review-ceremony-preflight.v1",
    status: "static-inputs-valid",
    scope: "static-input-validation-only-no-live-evidence-no-funding-authorization",
    sourceCommit: normalizedProviderPolicy.sourceCommit,
    chainId: normalizedProviderPolicy.chainId,
    verifyingContract: normalizedProviderPolicy.verifyingContract,
    providerPolicyDigest: valueDigest(normalizedProviderPolicy),
    providerSetDigest: valueDigest(normalizedProviderPolicy.providerApprovers),
    reviewPolicyDigest: valueDigest(normalizedReviewPolicy),
    reviewerSetDigest: valueDigest(normalizedReviewPolicy.reviewApprovers),
    artifactSetDigest: valueDigest(normalizedArtifacts),
    findingCountsDigest: valueDigest(normalizedFindingCounts),
    providerCount: normalizedProviderPolicy.providerApprovers.length,
    reviewerCount: normalizedReviewPolicy.reviewApprovers.length,
    reviewArtifactCount: Object.keys(normalizedArtifacts).length,
    effectiveMaximumReviewLifetimeSeconds,
    findingCounts: normalizedFindingCounts,
  ...
  return Object.freeze({
    ...preflight,
    preflightDigest: valueDigest(preflight),
  });
}
...
export function buildBitIndependentReviewManifestEvidence(verification) {
  if (!verifiedBitReviews.has(verification)) throw new Error("BIT independent review provenance is invalid");
  return Object.freeze({
    schema: "treeswap.bit-independent-review-manifest-evidence.v1",
    status: verification.status,
    ...
    findingCounts: verification.findingCounts,
    findingCountsDigest: verification.record.findingCountsDigest,
    reviewRecordDigest: verification.recordDigest,
    reviewPolicyDigest: verification.policyDigest,
    reviewerSetDigest: verification.reviewerSetDigest,
