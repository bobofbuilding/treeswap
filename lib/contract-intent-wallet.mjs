...
export function recordContractIntentWalletOutcome(input) {
  ...
  const postContextUnavailable = source.chainId === null && source.accounts === null;
  if ((source.chainId === null) !== (source.accounts === null)) {
    throw new TypeError("wallet post-request context must be complete or explicitly unavailable");
  }
  const postChainId = postContextUnavailable
    ? null
    : quantity(source.chainId, "wallet post-request chain ID");
  const postAccounts = postContextUnavailable ? null : accounts(source.accounts, null, 0);
  const contextChanged = postContextUnavailable
    || postChainId.toString() !== context.preflight.chainId
...
export function contractIntentWalletJournalArtifact(value) {
  ...
  if (SUBMISSIONS.has(value)) {
    return Object.freeze({
      ...
      postContextUnavailable: value.postContextUnavailable,
