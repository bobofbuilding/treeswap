...
export const MAX_PRICE_SIGNAL_CANDIDATES = 64;
export const BIT_RISK_ATTESTATION_SCHEMA = "treeswap.bit-risk-attestation.v3";

const BIT_SCALE = 10n ** 18n;
const BPS = 10_000n;
const RISK_POLICY_FIELDS = Object.freeze([
  "allowedPriceSourcePolicyDigests",
  "baseFeeBpsBitToLightning",
  "baseFeeBpsLightningToBit",
  "chainId",
  "decimals",
...
const RISK_SNAPSHOT_FIELDS = Object.freeze([
  "availableBitWei",
  ...
  "latestBlock",
  "lightningCapacitySats",
  "observedAt",
  "paused",
  "proxyAddress",
  "proxyCodeHash",
]);
const RISK_REQUEST_FIELDS = Object.freeze(["bitWei", "direction", "lightningSats", "now"]);
const verifiedRiskEvaluations = new WeakMap();
const verifiedRiskAttestations = new WeakMap();
...
function exactSnapshot(value, fields, name) {
  exactKeys(value, fields, name);
  return Object.freeze(Object.fromEntries(fields.map((field) => [field, value[field]])));
}

function snapshotRiskPolicy(policy) {
  const snapshot = exactSnapshot(policy, RISK_POLICY_FIELDS, "risk policy");
  if (!Array.isArray(snapshot.allowedPriceSourcePolicyDigests)
      || snapshot.allowedPriceSourcePolicyDigests.length > MAX_PRICE_SIGNAL_CANDIDATES) {
    throw new RangeError("risk policy price-source allowlist is invalid or unbounded");
  }
  const allowedPriceSourcePolicyDigests = Object.freeze(Array.from(
    { length: snapshot.allowedPriceSourcePolicyDigests.length },
    (_, index) => snapshot.allowedPriceSourcePolicyDigests[index],
  ));
  return Object.freeze({ ...snapshot, allowedPriceSourcePolicyDigests });
}

function snapshotRiskRequest(request) {
  return exactSnapshot(request, RISK_REQUEST_FIELDS, "risk request");
}

function snapshotRiskState(snapshot) {
  return exactSnapshot(snapshot, RISK_SNAPSHOT_FIELDS, "risk snapshot");
}

export function bitRiskRequestDigest(request) {
  return objectDigest(snapshotRiskRequest(request));
}

export function bitRiskPolicyDigest(policy) {
  return objectDigest(snapshotRiskPolicy(policy));
}
...
function exceedsDeviationBps(observed, reference, maximumBps) {
  const difference = observed > reference ? observed - reference : reference - observed;
  return difference * BPS > reference * maximumBps;
}
...
export function evaluateBitRisk({ policy, snapshot, priceSignals, request }) {
  policy = snapshotRiskPolicy(policy);
  snapshot = snapshotRiskState(snapshot);
  request = snapshotRiskRequest(request);
  const reasons = [];
  const now = asBigInt(request.now, "request.now");
  const snapshotAt = asBigInt(snapshot.observedAt, "snapshot.observedAt");
  const maxSnapshotAgeSeconds = asBigInt(policy.maxSnapshotAgeSeconds, "maxSnapshotAgeSeconds");
  const maxPriceAgeSeconds = asBigInt(policy.maxPriceAgeSeconds, "maxPriceAgeSeconds");
  ...
  const quotedPriceMsatPerBit = requestedBitWei > 0n && requestedLightningSats > 0n
    ? requestedLightningSats * 1_000n * BIT_SCALE / requestedBitWei
    : 0n;
  ...
  addReason(reasons, snapshotAt > now || now - snapshotAt > maxSnapshotAgeSeconds, "BIT state snapshot is stale");
  ...
  const candidateCount = Array.isArray(priceSignals) ? priceSignals.length : 0;
  const candidateLimitExceeded = candidateCount > MAX_PRICE_SIGNAL_CANDIDATES;
  addReason(reasons, candidateLimitExceeded, "price signal candidate limit exceeded");
  const rawPriceSignals = candidateLimitExceeded ? [] : Array.from(
    { length: candidateCount },
    (_, index) => priceSignals[index],
  );
  const candidates = [];
  for (const rawSignal of rawPriceSignals) {
    ...
    if (
      ...
      || (kind === "bit-wbtc-twap-probe" && !isVerifiedBitWbtcPoolPriceSignal(rawSignal, {
        ...
        bitWei: requestedBitWei,
        lightningSats: requestedLightningSats,
      }))
    ...
    if (
      observedAt <= now
      && validUntil > now
      && now - observedAt <= maxPriceAgeSeconds
      && priceMsatPerBit > 0n
      && (kind === "bit-wbtc-twap-probe" || executableDepthSats >= requestedLightningSats)
  ...
  for (const candidate of candidates) exactCandidates.set(objectDigest(candidate), candidate);
  const distinctCandidates = [...exactCandidates.values()];
  const identityFields = ["source", "venueId", "controlDomain", "operatorOrganization"];
  const identityCounts = new Map(identityFields.map((field) => [field, new Map()]));
  for (const candidate of distinctCandidates) {
    for (const field of identityFields) {
      const counts = identityCounts.get(field);
      counts.set(candidate[field], (counts.get(candidate[field]) ?? 0) + 1);
    }
  }
  const hasIdentityConflict = distinctCandidates.some((candidate) => identityFields.some(
    (field) => identityCounts.get(field).get(candidate[field]) > 1,
  ));
  addReason(reasons, hasIdentityConflict, "conflicting price signal identity");
  const signals = distinctCandidates
    .filter((candidate) => identityFields.every(
      (field) => identityCounts.get(field).get(candidate[field]) === 1,
    ))
  ...
  let quoteMarketDeviationBps = 0n;
  let validUntil = snapshotAt + maxSnapshotAgeSeconds + 1n;
  if (signals.length > 0) {
    validUntil = signals.reduce((earliest, signal) => {
      const sourceAgeExpiry = signal.observedAt + maxPriceAgeSeconds + 1n;
      const sourceExpiry = signal.validUntil < sourceAgeExpiry ? signal.validUntil : sourceAgeExpiry;
      return sourceExpiry < earliest ? sourceExpiry : earliest;
    }, validUntil);
    ...
    addReason(
      reasons,
      (high - low) * BPS
        > marketPriceMsatPerBit * asBigInt(policy.maxSignalSpreadBps, "maxSignalSpreadBps"),
      "external price sources disagree",
    );
    addReason(
      reasons,
      exceedsDeviationBps(
    ...
    if (requestedBitWei > 0n && requestedLightningSats > 0n) {
      const quotedPriceScaled = requestedLightningSats * 1_000n * BIT_SCALE;
      const marketPriceScaled = requestedBitWei * marketPriceMsatPerBit;
      const difference = quotedPriceScaled > marketPriceScaled
        ? quotedPriceScaled - marketPriceScaled
        : marketPriceScaled - quotedPriceScaled;
      quoteMarketDeviationBps = difference * BPS / marketPriceScaled;
      addReason(
        reasons,
        difference * BPS > marketPriceScaled * asBigInt(policy.maxMarketDeviationBps, "maxMarketDeviationBps"),
        "quote price is outside the market band",
      );
    }
  }
  ...
  const evaluation = Object.freeze({
    ...
    marketPriceMsatPerBit,
    quotedPriceMsatPerBit,
    quoteMarketDeviationBps,
    signalSpreadBps,
    validUntil,
  ...
  verifiedRiskEvaluations.set(evaluation, Object.freeze({
    policyDigest: bitRiskPolicyDigest(policy),
    snapshotDigest: objectDigest(snapshot),
    requestDigest: bitRiskRequestDigest(request),
  }));
  return evaluation;
}
...
export function buildBitRiskAttestation({ policy, snapshot, request, evaluation }) {
  policy = snapshotRiskPolicy(policy);
  snapshot = snapshotRiskState(snapshot);
  request = snapshotRiskRequest(request);
  ...
  const policyDigest = bitRiskPolicyDigest(policy);
  const snapshotDigest = objectDigest(snapshot);
  const requestDigest = bitRiskRequestDigest(request);
  if (provenance.policyDigest !== policyDigest || provenance.snapshotDigest !== snapshotDigest) {
    throw new Error("risk attestation policy or snapshot does not match the verified evaluation");
  }
  if (provenance.requestDigest !== requestDigest) {
    throw new Error("risk attestation request does not match the verified evaluation");
  }
  ...
  const evidence = [...(evaluation.qualifiedPriceEvidence ?? [])]
    .map((entry) => {
      const fields = [
        ...
        asBigInt(entry.observedAt, "price-source evidence observedAt").toString(),
        asBigInt(entry.validUntil, "price-source evidence validUntil").toString(),
      ];
  ...
  const riskDigest = keccak256(coder.encode(
    ...
    [
      "TreeSwap BIT risk attestation v3",
      ...
      asBigInt(evaluation.quotedPriceMsatPerBit, "evaluation.quotedPriceMsatPerBit"),
      asBigInt(evaluation.quoteMarketDeviationBps, "evaluation.quoteMarketDeviationBps"),
      asBigInt(evaluation.signalSpreadBps, "evaluation.signalSpreadBps"),
      asBigInt(evaluation.feeBps, "evaluation.feeBps"),
      asBigInt(evaluation.remainingInventoryBps, "evaluation.remainingInventoryBps"),
      asBigInt(evaluation.validUntil, "evaluation.validUntil"),
      sourceSetDigest,
      policyDigest,
    ],
  ));
  const attestation = Object.freeze({
    schema: BIT_RISK_ATTESTATION_SCHEMA,
    riskDigest,
    requestDigest,
    snapshotDigest,
    sourceSetDigest,
    policyDigest,
    validUntil: asBigInt(evaluation.validUntil, "evaluation.validUntil"),
    sources,
  });
  verifiedRiskAttestations.set(attestation, Object.freeze({
    issuedAt: asBigInt(request.now, "request.now"),
    requestDigest,
    riskDigest,
...
export function assertCurrentBitRiskAttestation({
  attestation,
  request,
  now,
  requiredValidUntil = null,
}) {
  exactKeys(request, ["bitWei", "direction", "lightningSats", "now"], "risk request");
  const provenance = attestation && typeof attestation === "object"
    ? verifiedRiskAttestations.get(attestation)
    : null;
  if (!provenance) throw new TypeError("current risk check requires the original verified risk attestation");
  const observedAt = asBigInt(now, "now");
  const requiredThrough = requiredValidUntil === null
    ? observedAt + 1n
    : asBigInt(requiredValidUntil, "requiredValidUntil");
  if (bitRiskRequestDigest(request) !== provenance.requestDigest || attestation.requestDigest !== provenance.requestDigest) {
    throw new Error("risk attestation request does not match the verified evaluation");
  }
  if (attestation.riskDigest !== provenance.riskDigest || attestation.validUntil !== provenance.validUntil) {
    throw new Error("risk attestation changed after verification");
  }
  if (observedAt < provenance.issuedAt) throw new Error("risk attestation check predates its evaluation");
  if (observedAt >= provenance.validUntil) throw new Error("risk attestation is expired");
  if (requiredThrough <= observedAt) throw new RangeError("required risk validity must extend beyond now");
  if (requiredThrough > provenance.validUntil) {
    throw new Error("risk attestation does not cover the required validity window");
  }
  return attestation;
}
