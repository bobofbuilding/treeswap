...
export function evaluateBitRisk({ policy, snapshot, priceSignals, request }) {
  ...
  const rawPriceSignals = Array.isArray(priceSignals) ? priceSignals : [];
  const candidateLimitExceeded = rawPriceSignals.length > MAX_PRICE_SIGNAL_CANDIDATES;
  addReason(reasons, candidateLimitExceeded, "price signal candidate limit exceeded");
  const candidates = [];
  for (const rawSignal of candidateLimitExceeded ? [] : rawPriceSignals) {
