...
export const WALLET_SESSION_ROUTE_REVIEW_CONTROLS = Object.freeze({
  "application-security-reviewer": Object.freeze([
    ...
    "signed-boundary-reconstruction-preserves-historical-provenance-without-current-freshness",
  ...
  "platform-data-isolation-reviewer": Object.freeze([
    ...
    "live-review-requires-direct-artifact-platform-independence-and-monitoring-claims",
...
const ATTESTATION_FIELDS = Object.freeze(["attestedAt", "reviewerId", "role", "signature", "signer"]);
...
function sourceCommit(value, name) {
  if (typeof value !== "string" || !COMMIT.test(value)) {
    throw new TypeError(`${name} must be a lowercase Git commit`);
  }
  return value;
}
...
function normalizeArtifact(raw) {
  ...
  const fileSource = exactArray(
    source.files,
    WALLET_SESSION_ROUTE_REVIEW_FILES.length,
    "wallet session route review artifact files",
  );
  const files = fileSource.map((rawFile, index) => {
...
function normalizePolicy(raw, artifact, artifactFileDigest) {
  ...
  if (source.sourceBranch !== artifact.sourceBranch || source.sourceCommit !== artifact.sourceCommit
      || sha256Digest(source.artifactFileDigest, "policy artifact-file digest") !== artifactFileDigest) {
  ...
  const approverSource = exactArray(
    source.reviewApprovers,
    WALLET_SESSION_ROUTE_REVIEW_ROLES.length,
    "wallet session route review approvers",
  );
  const reviewApprovers = approverSource.map((rawApprover, index) => {
    const approver = exactRecord(rawApprover, APPROVER_FIELDS, `wallet session route reviewer ${index}`);
    return Object.freeze({
      role: typeof approver.role === "string" ? approver.role : "",
...
function normalizeReports(raw, policy) {
  const source = exactArray(
    raw,
    WALLET_SESSION_ROUTE_REVIEW_ROLES.length,
    "wallet session route review reports",
  );
...
export function buildWalletSessionRouteReviewApprovalMessage({
  artifactFileBytes,
  policy,
  reports,
  role,
  ...
  const signedAt = safeInteger(attestedAt ?? observedAt, "wallet session route review attestation time", {
    positive: true,
  });
  if (signedAt < report.reviewedAt || signedAt > observedAt || signedAt > report.validUntil) {
    throw new Error("wallet session route review attestation time is invalid");
  }
  return Object.freeze({
...
function normalizeAttestations(raw) {
  const source = exactArray(
    raw,
    WALLET_SESSION_ROUTE_REVIEW_ROLES.length,
    "wallet session route review attestations",
  );
...
export function verifyWalletSessionRouteReview({
  ...
  for (let index = 0; index < normalizedAttestations.length; index += 1) {
    ...
    if (attestation.attestedAt < report.reviewedAt
        || attestation.attestedAt > now
        || attestation.attestedAt > report.validUntil) {
      throw new Error("wallet session route review attestation time is invalid");
    }
...
export function verifyWalletSessionRouteReviewAtSignedBoundary({
  artifactFileBytes,
  policy,
  reports,
  attestations,
  ...
  return verifyWalletSessionRouteReview({
    artifactFileBytes,
    policy,
    reports,
    attestations,
