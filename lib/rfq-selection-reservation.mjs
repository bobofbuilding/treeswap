...
import { prepareFinalizedContractIntent } from "./rfq-contract-intent.mjs";
...
const CONTRACT_INTENT_FIELDS = Object.freeze([
  "bitcoinHeight",
  "reservationToken",
  "settlementPolicy",
]);
...
function finalizationAck(record) {
  ...
  return Object.freeze({
    schema: "treeswap.selected-solver-authorization-ack.v2",
    status: "authorized",
    settlementStatus: "accepted",
    settlementId: record.authorized.settlementId,
    settlementRecordDigest: record.authorized.settlementRecordDigest,
...
export function claimRfqSelectedSolverFinalizationOwnership(service, input) {
  ...
  const lease = Object.freeze({
    ...
    prepareContractIntent(inputValue) {
      assertLease(this);
      const value = exactDataRecord(
        inputValue,
        CONTRACT_INTENT_FIELDS,
        "selected-solver contract intent preparation",
      );
      const now = context.observeNow();
      let record = find(value.reservationToken, now);
      if (record.state !== "authorized") {
        throw new Error("selected-solver quote lacks exact user execution authorization");
      }
      const bitcoinHeight = integer(
        value.bitcoinHeight,
        "selected-solver contract intent Bitcoin height",
        1,
      );
      if (record.contractIntent) {
        if (record.contractIntentBitcoinHeight !== bitcoinHeight) {
          throw new Error("contract intent Bitcoin height changed after preparation");
        }
        return record.contractIntent;
      }
      const contractIntent = prepareFinalizedContractIntent({
        bitcoinHeight,
        finalization: record.authorized,
        invoice: record.invoice,
        invoicePolicy: context.invoicePolicy,
        now,
      ...
      record = replace(value.reservationToken, {
        ...record,
        contractIntent,
        contractIntentBitcoinHeight: bitcoinHeight,
      });
      return record.contractIntent;
    },
