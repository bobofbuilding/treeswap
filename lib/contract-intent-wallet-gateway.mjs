...
export function contractIntentWalletGatewayMode(value) {
  const context = GATEWAYS.get(value);
  if (!context) throw new TypeError("wallet gateway lacks factory provenance");
  return context.mode;
}

export function assertContractIntentWalletGatewayLifecycle(value, signal) {
  const context = GATEWAYS.get(value);
  if (!context) throw new TypeError("wallet gateway lacks factory provenance");
  if (!(signal instanceof AbortSignal) || signal !== context.signal || signal.aborted
      || context.state !== "active") {
    throw new TypeError("wallet gateway does not share the active deployment lifecycle");
  }
  return value;
}

export function contractIntentWalletGatewayBinding(value) {
  const context = GATEWAYS.get(value);
  if (!context) throw new TypeError("wallet gateway lacks factory provenance");
  return Object.freeze({
    apiOrigin: context.apiOrigin,
    requesterKeyId: context.expectedRequesterKeyId,
    responseKeyId: context.responseKeyId,
    mode: context.mode,
  });
}
