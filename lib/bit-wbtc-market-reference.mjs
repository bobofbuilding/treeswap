...
export const BIT_WBTC_MARKET_POLICY_SCHEMA = "treeswap.bit-wbtc-market-policy.v1";
export const BIT_WBTC_PROVIDER_OBSERVATION_SCHEMA = "treeswap.bit-wbtc-provider-observation.v1";
export const BIT_WBTC_PRICE_REQUEST_SCHEMA = "treeswap.bit-wbtc-price-request.v1";
...
function validateObservation(observation, policy, request, now) {
  ...
  if (deviationBps(peg, 100_000_000n) > integer(policy.maximumWbtcPegDeviationBps, "policy.maximumWbtcPegDeviationBps")) {
  ...
  if (deviationBps(spotWbtcAtomicPerBit, twapWbtcAtomicPerBit)
  ...
  if (probeBitWei < integer(request.bitWei, "request.bitWei")) throw new Error("BIT/WBTC executable probe is too shallow");
  const probeWbtcAtomicPerBit = probeWbtcAtomic * BIT_SCALE / probeBitWei;
  if (deviationBps(probeWbtcAtomicPerBit, twapWbtcAtomicPerBit)
  ...
  if (executableDepthSats < integer(request.lightningSats, "request.lightningSats")) {
    throw new Error("BIT/WBTC executable probe does not cover the Lightning amount");
  }
...
export function buildBitWbtcPoolPriceSignal({ policy, observations, request }) {
  ...
  const evidence = Object.freeze({
    schema: "treeswap.bit-wbtc-market-reference.v1",
...
export function isVerifiedBitWbtcPoolPriceSignal(value) {
  return Boolean(value && typeof value === "object" && verifiedPoolSignals.has(value));
}
