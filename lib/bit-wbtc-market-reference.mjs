...
const MARKET_REFERENCE_INPUT_FIELDS = Object.freeze(["observations", "policy", "request"]);
const TYPED_DATA_INPUT_FIELDS = Object.freeze(["observation", "policy"]);
...
const POLICY_PROVIDER_FIELDS = Object.freeze(["organization", "providerId", "signer"]);
const PRICE_REQUEST_FIELDS = Object.freeze(["bitWei", "direction", "lightningSats", "now", "schema"]);
...
const SIGNED_OBSERVATION_FIELDS = Object.freeze([...OBSERVATION_FIELDS, "providerSignature"]);
...
function integer(value, name) {
  ...
  if (typeof value !== "string" && typeof value !== "bigint") {
    throw new TypeError(`${name} must be an integer data value`);
  }
  try {
    return BigInt(value);
  } catch {
...
function safeAddress(value, name) {
  if (typeof value !== "string") throw new TypeError(`${name} must be an address string`);
  try {
...
function bytes32(value, name) {
  if (typeof value !== "string") throw new TypeError(`${name} must be nonzero bytes32`);
...
export function sqrtRatioX96AtTick(tick) {
  if (typeof tick !== "number" || !Number.isSafeInteger(tick)) throw new TypeError("tick must be a safe integer data value");
  const parsed = tick;
  if (parsed < MIN_TICK || parsed > MAX_TICK) throw new RangeError("tick is outside Uniswap bounds");
...
function validatePolicy(rawPolicy) {
  const source = exactDataRecord(rawPolicy, POLICY_FIELDS, "BIT/WBTC market policy");
  const rawProviders = exactDataArray(source.providers, "BIT/WBTC policy providers", 5);
  const providers = Object.freeze(rawProviders.map((provider, index) => exactDataRecord(
    provider,
    POLICY_PROVIDER_FIELDS,
    `BIT/WBTC policy provider ${index}`,
  )));
  const policy = Object.freeze({ ...source, providers });
  ...
  if (policy.providers.length < Number(policy.minimumProviders)) {
...
function normalizeObservation(rawObservation, signatureRequired) {
  const fields = signatureRequired ? SIGNED_OBSERVATION_FIELDS : OBSERVATION_FIELDS;
  const source = exactDataRecord(rawObservation, fields, "BIT/WBTC provider observation");
  const probe = exactDataRecord(source.probe, PROBE_FIELDS, "BIT/WBTC executable probe");
  return Object.freeze({ ...source, probe });
}

function observationWithoutSignature(observation) {
  return Object.freeze(Object.fromEntries(OBSERVATION_FIELDS.map((field) => [field, observation[field]])));
}

function preparedObservationTypedData(policy, unsigned) {
...
export function bitWbtcObservationTypedData(input) {
  const source = exactDataRecord(input, TYPED_DATA_INPUT_FIELDS, "BIT/WBTC observation typed-data input");
  const policy = validatePolicy(source.policy);
  const observationKeys = Reflect.ownKeys(source.observation);
  const hasSignature = observationKeys.includes("providerSignature");
  const observation = normalizeObservation(source.observation, hasSignature);
  const unsigned = hasSignature ? observationWithoutSignature(observation) : observation;
  return preparedObservationTypedData(policy, unsigned);
}
...
function validateObservation(observation, policy, request, now) {
  ...
  let recoveredSigner;
  try {
    if (typeof observation.providerSignature !== "string") throw new TypeError("signature must be a string");
    const typedData = preparedObservationTypedData(policy, observationWithoutSignature(observation));
...
export function buildBitWbtcPoolPriceSignal(input) {
  const source = exactDataRecord(input, MARKET_REFERENCE_INPUT_FIELDS, "BIT/WBTC market-reference input");
  const policy = validatePolicy(source.policy);
  const request = exactDataRecord(source.request, PRICE_REQUEST_FIELDS, "BIT/WBTC price request");
  ...
  const rawObservations = exactDataArray(source.observations, "BIT/WBTC provider observations", policy.providers.length);
  if (rawObservations.length < Number(policy.minimumProviders)) {
    throw new Error("BIT/WBTC price requires the policy minimum provider count");
  }
  const observations = Object.freeze(rawObservations.map((observation) => normalizeObservation(observation, true)));
