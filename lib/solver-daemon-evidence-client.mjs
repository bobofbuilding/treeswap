...
import { readStrictPrivateJsonResponse } from "./private-json-response.mjs";
import {
...
const BUILD_REQUEST_REQUIRED_KEYS = Object.freeze([
  "expiresAt",
  "kind",
  "policy",
  "requestId",
...
const BUILD_REQUEST_OPTIONAL_KEYS = Object.freeze([
  "action",
  "packet",
  "packetResponseDigest",
  "terminalState",
]);
const VERIFY_REQUEST_REQUIRED_KEYS = Object.freeze([
  "envelope",
  "expectedRequesterKeyId",
  "now",
  "requesterPublicKey",
]);
const VERIFY_REQUEST_OPTIONAL_KEYS = Object.freeze(["maxClockSkewSeconds"]);
const ROUTE_RESPONSE_KEYS = Object.freeze([
  "approval",
  "expectedRequesterKeyId",
  "now",
  "policy",
...
const ROUTE_RESPONSE_OPTIONAL_KEYS = Object.freeze(["consumeRequest"]);
const CONTROLS_REQUIRED_KEYS = Object.freeze([
  "policy",
  "requesterKeyId",
  "requesterPrivateKey",
  "routes",
]);
const CONTROLS_OPTIONAL_KEYS = Object.freeze([
  "requestTtlSeconds",
  "timeoutMs",
]);
const TEST_CONTROLS_REQUIRED_KEYS = Object.freeze([
  ...CONTROLS_REQUIRED_KEYS,
  "nowSeconds",
  "randomBytesImpl",
  "requestImpl",
]);
const VERIFIED_CONTROL_SETS = new WeakMap();
const VERIFIED_RECOVERY_CONTROL_SETS = new WeakMap();
...
function policyBinding(rawPolicy) {
  const policy = snapshotPlainData(rawPolicy, "solver evidence policy");
...
export function buildSolverDaemonEvidenceRequest(input) {
  const source = exactDataRecord(
    input,
    BUILD_REQUEST_REQUIRED_KEYS,
    BUILD_REQUEST_OPTIONAL_KEYS,
    "solver evidence request builder",
  );
  const requestedKind = source.kind;
  const settlement = snapshotPlainData(source.settlement, "solver evidence request settlement");
  const action = source.action === undefined
    ? null : snapshotPlainData(source.action, "solver evidence request action");
  const packet = source.packet === undefined
    ? null : snapshotPlainData(source.packet, "solver evidence request packet");
  const packetResponseDigest = source.packetResponseDigest ?? null;
  const terminalState = source.terminalState ?? "NONE";
  ...
  const boundPolicy = policyBinding(source.policy);
...
export function verifySolverDaemonEvidenceRequest(input) {
  const source = exactDataRecord(
    input,
    VERIFY_REQUEST_REQUIRED_KEYS,
    VERIFY_REQUEST_OPTIONAL_KEYS,
    "solver evidence request verification",
  );
  const envelope = snapshotPlainData(source.envelope, "solver evidence request envelope");
  ...
  const key = publicEd25519Key(source.requesterPublicKey, "solver evidence requester public key");
  const observedAt = integer(source.now, "solver evidence request verification time");
  const skew = integer(source.maxClockSkewSeconds ?? 5, "solver evidence request clock skew", 60);
  if (payload.requesterKeyId !== keyId(source.expectedRequesterKeyId, "expected solver evidence requesterKeyId")
...
export async function buildSolverDaemonEvidenceRouteResponse(input) {
  const source = exactDataRecord(
    input,
    ROUTE_RESPONSE_KEYS,
    ROUTE_RESPONSE_OPTIONAL_KEYS,
    "solver evidence route response builder",
  );
  const requestEnvelope = snapshotPlainData(
    source.requestEnvelope,
    "solver evidence route request envelope",
  );
  const record = snapshotPlainData(source.record, "solver evidence route record");
  const policy = snapshotPlainData(source.policy, "solver evidence route policy");
  const approval = snapshotPlainData(source.approval, "solver evidence route approval");
  const request = verifySolverDaemonEvidenceRequest({
    envelope: requestEnvelope,
    requesterPublicKey: source.requesterPublicKey,
    expectedRequesterKeyId: source.expectedRequesterKeyId,
    now: source.now,
  });
  ...
  if (typeof source.consumeRequest !== "function") {
    throw new TypeError("solver evidence route requires a durable request replay consumer");
  }
  const consumed = await source.consumeRequest(Object.freeze({
...
export async function fixedSolverDaemonEvidenceHttpsRequest(endpoint, options, {
  ...
  const expected = privateRouteUrl(requestUrl.origin);
  if (requestUrl.href !== expected.href || options?.method !== "POST"
      || options?.redirect !== "error" || typeof options?.body !== "string") {
    throw new Error("solver evidence fixed HTTPS request is invalid");
  }
...
async function fetchRoute({ endpoint, role, requestEnvelope, requestImpl, signal, deadline }) {
  ...
  return normalizeRouteResponse(await Promise.race([
    readStrictPrivateJsonResponse(response, {
      label: "solver evidence response",
...
function buildSolverDaemonEvidenceControls(source, dependencies) {
  const routes = snapshotPlainData(source.routes, "solver evidence routes");
  ...
  const policy = snapshotPlainData(source.policy, "solver evidence policy");
  solverDaemonEvidencePolicyDigest(policy);
  for (const role of APPROVER_ROLES) approvalSigner(policy, role);
  const privateKey = privateEd25519Key(source.requesterPrivateKey, "solver evidence requester private key");
  const activeRequesterKeyId = keyId(source.requesterKeyId, "solver evidence requesterKeyId");
  const { mode: transportMode, requestImpl, nowSeconds, randomBytesImpl } = dependencies;
  ...
  const ttl = integer(source.requestTtlSeconds ?? 15, "solver evidence request lifetime", MAX_REQUEST_LIFETIME_SECONDS);
  if (ttl === 0) throw new RangeError("solver evidence request lifetime must be nonzero");
  const timeout = integer(source.timeoutMs ?? 5_000, "solver evidence route timeout", 30_000);
  ...
  const controls = Object.freeze({
    observeReservation: (inputValue) => {
      const control = exactDataRecord(
        inputValue,
        ["settlement"],
        ["now"],
        "solver evidence reservation control",
      );
    ...
    authorizeLightning: (inputValue) => {
      const control = exactDataRecord(
        inputValue,
        ["action", "packet", "packetResponseDigest", "settlement"],
        ["now"],
        "solver evidence Lightning control",
      );
      return requestEvidence({
        ...
        packetResponseDigest: control.packetResponseDigest,
        terminalState: "NONE",
      });
    },
    authorizeEvmClaim: (inputValue) => {
      const control = exactDataRecord(
        inputValue,
        ["action", "packet", "packetResponseDigest", "settlement"],
        ["now"],
        "solver evidence EVM claim control",
      );
      return requestEvidence({
    ...
    verifyAssets: (inputValue) => {
      const control = exactDataRecord(
        inputValue,
        ["expectedTerminal", "settlement"],
        ["now"],
        "solver evidence terminal control",
      );
      return requestEvidence({
        kind: control.expectedTerminal === "COMPLETED" ? "TERMINAL_COMPLETED"
          : control.expectedTerminal === "REFUNDED" ? "TERMINAL_REFUNDED"
            : (() => { throw new RangeError("solver evidence terminal state is unsupported"); })(),
        settlement: control.settlement,
        terminalState: control.expectedTerminal,
      });
    },
  });
  VERIFIED_CONTROL_SETS.set(controls, Object.freeze({
    policyDigest: solverDaemonEvidencePolicyDigest(policy),
    transportMode,
  }));
  return controls;
}

export function createSolverDaemonEvidenceControls(input) {
  const source = exactDataRecord(
    input,
    CONTROLS_REQUIRED_KEYS,
    CONTROLS_OPTIONAL_KEYS,
    "production solver evidence controls",
  );
  return buildSolverDaemonEvidenceControls(source, Object.freeze({
    mode: "fixed-node-https",
    requestImpl: fixedSolverDaemonEvidenceHttpsRequest,
    nowSeconds: () => Math.floor(Date.now() / 1_000),
    randomBytesImpl: randomBytes,
  }));
}

export function createTestSolverDaemonEvidenceControls(input) {
  const source = exactDataRecord(
    input,
    TEST_CONTROLS_REQUIRED_KEYS,
    CONTROLS_OPTIONAL_KEYS,
    "test solver evidence controls",
  );
  return buildSolverDaemonEvidenceControls(source, Object.freeze({
    mode: "injected-test",
...
export function createSolverDaemonRecoveryEvidenceControls(input) {
  const activeControls = createSolverDaemonEvidenceControls(input);
  const controls = Object.freeze({
    observeReservation: activeControls.observeReservation,
    authorizeEvmClaim: activeControls.authorizeEvmClaim,
    verifyAssets: activeControls.verifyAssets,
  });
  VERIFIED_RECOVERY_CONTROL_SETS.set(controls, VERIFIED_CONTROL_SETS.get(activeControls));
  return controls;
}

export function createTestSolverDaemonRecoveryEvidenceControls(input) {
  const activeControls = createTestSolverDaemonEvidenceControls(input);
  const controls = Object.freeze({
    observeReservation: activeControls.observeReservation,
    authorizeEvmClaim: activeControls.authorizeEvmClaim,
    verifyAssets: activeControls.verifyAssets,
...
export function solverDaemonEvidenceControlsPolicyDigest(value) {
  const metadata = value && VERIFIED_CONTROL_SETS.get(value);
  if (!metadata) throw new TypeError("solver evidence controls lack same-process factory provenance");
  return metadata.policyDigest;
}

export function isSolverDaemonRecoveryEvidenceControls(value) {
  return Boolean(value && VERIFIED_RECOVERY_CONTROL_SETS.has(value));
}

export function solverDaemonRecoveryEvidenceControlsPolicyDigest(value) {
  const metadata = value && VERIFIED_RECOVERY_CONTROL_SETS.get(value);
  if (!metadata) {
    throw new TypeError("solver recovery evidence controls lack same-process factory provenance");
  }
  return metadata.policyDigest;
}

export function solverDaemonEvidenceControlsTransportMode(value) {
  const metadata = value && (
    VERIFIED_CONTROL_SETS.get(value) ?? VERIFIED_RECOVERY_CONTROL_SETS.get(value)
  );
  if (!metadata) {
    throw new TypeError("solver evidence controls lack same-process factory provenance");
  }
  return metadata.transportMode;
}

export function isProductionSolverDaemonEvidenceControls(value) {
  return VERIFIED_CONTROL_SETS.get(value)?.transportMode === "fixed-node-https";
}

export function isProductionSolverDaemonRecoveryEvidenceControls(value) {
  return VERIFIED_RECOVERY_CONTROL_SETS.get(value)?.transportMode === "fixed-node-https";
}
