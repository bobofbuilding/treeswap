...
export function createSolverDaemonEvidenceControls({
  ...
  VERIFIED_CONTROL_SETS.set(controls, solverDaemonEvidencePolicyDigest(policy));
  return controls;
}
...
export function createSolverDaemonRecoveryEvidenceControls(input) {
  ...
  VERIFIED_RECOVERY_CONTROL_SETS.set(
    controls,
    solverDaemonEvidenceControlsPolicyDigest(activeControls),
  );
  return controls;
}
...
export function solverDaemonEvidenceControlsPolicyDigest(value) {
  const digest = value && VERIFIED_CONTROL_SETS.get(value);
  if (!digest) throw new TypeError("solver evidence controls lack same-process factory provenance");
  return digest;
}
...
export function solverDaemonRecoveryEvidenceControlsPolicyDigest(value) {
  const digest = value && VERIFIED_RECOVERY_CONTROL_SETS.get(value);
  if (!digest) {
