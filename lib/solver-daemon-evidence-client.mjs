...
export function createSolverDaemonEvidenceControls(input) {
  ...
  const requestImpl = source.requestImpl ?? fixedPrivateHttpsRequest;
  const nowSeconds = source.nowSeconds ?? (() => Math.floor(Date.now() / 1_000));
  const randomBytesImpl = source.randomBytesImpl ?? randomBytes;
  ...
  VERIFIED_CONTROL_SETS.set(controls, Object.freeze({
    ...
    transportMode: requestImpl === fixedPrivateHttpsRequest ? "fixed-node-https" : "injected-test",
  }));
  return controls;
}
