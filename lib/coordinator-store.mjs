...
function reconciliationDisposition(method, observedState) {
  ...
  const decisions = {
    ...
    "evm:claim": { CLAIMED: "confirmed", LOCKED: "unresolved", REFUNDED: "mismatch", NOT_FOUND: "unresolved" },
  };
...
export class CoordinatorStore {
  ...
  static async open(path, { allowMemory = false } = {}) {
    ...
    database.prepare("INSERT OR IGNORE INTO coordinator_meta(key, value) VALUES ('schema', 'treeswap.coordinator.v1')").run();
  ...
  metrics() {
    ...
    return Object.freeze({ settlementStates, actionStates, reconciliationRequired });
  }
