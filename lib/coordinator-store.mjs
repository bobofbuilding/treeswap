...
import { validateContractIntentBinding } from "./contract-intent-schema.mjs";
...
const COORDINATOR_SCHEMA = "treeswap.coordinator.v10";
const RFQ_FINALIZATION_RESTART_SCHEMA = "treeswap.rfq-finalization-restart.v1";
const RFQ_FINALIZATION_RESTART_CLOCK_KEY = "rfq_finalization_restart_observed_at";
...
function verifyDatabase(database, { full = false, requireSchema = true } = {}) {
  ...
  if (requireSchema) {
    ...
    verifyPersistedContractIntents(database);
  }
...
CREATE TABLE IF NOT EXISTS settlements (
  ...
  contract_quote_id TEXT UNIQUE,
  contract_intent_digest TEXT UNIQUE,
  contract_intent_code_hash TEXT,
  contract_chain_id TEXT,
  contract_from_address TEXT,
  contract_to_address TEXT,
  contract_calldata TEXT,
  contract_calldata_digest TEXT UNIQUE,
  contract_transaction_value TEXT,
  contract_user_address TEXT,
  contract_solver_address TEXT,
  contract_beneficiary_address TEXT,
  contract_bit_amount TEXT,
  contract_fee_bit_amount TEXT,
  contract_nonce TEXT,
  contract_quote_expires_at INTEGER,
  contract_last_safe_claim_at INTEGER,
  contract_refund_after INTEGER,
  contract_solver_signature TEXT,
  contract_user_signature TEXT,
  contract_authorized_at INTEGER,
  contract_intent_record_digest TEXT,
  contract_intent_bound_at INTEGER,
  ...
  CHECK ((contract_intent_digest IS NULL) = (contract_quote_id IS NULL)),
  CHECK ((contract_intent_digest IS NULL) = (contract_intent_code_hash IS NULL)),
  CHECK ((contract_intent_digest IS NULL) = (contract_chain_id IS NULL)),
  CHECK ((contract_intent_digest IS NULL) = (contract_from_address IS NULL)),
  CHECK ((contract_intent_digest IS NULL) = (contract_to_address IS NULL)),
  CHECK ((contract_intent_digest IS NULL) = (contract_calldata IS NULL)),
  CHECK ((contract_intent_digest IS NULL) = (contract_calldata_digest IS NULL)),
  CHECK ((contract_intent_digest IS NULL) = (contract_transaction_value IS NULL)),
  CHECK ((contract_intent_digest IS NULL) = (contract_user_address IS NULL)),
  CHECK ((contract_intent_digest IS NULL) = (contract_solver_address IS NULL)),
  CHECK ((contract_intent_digest IS NULL) = (contract_beneficiary_address IS NULL)),
  CHECK ((contract_intent_digest IS NULL) = (contract_bit_amount IS NULL)),
  CHECK ((contract_intent_digest IS NULL) = (contract_fee_bit_amount IS NULL)),
  CHECK ((contract_intent_digest IS NULL) = (contract_nonce IS NULL)),
  CHECK ((contract_intent_digest IS NULL) = (contract_quote_expires_at IS NULL)),
  CHECK ((contract_intent_digest IS NULL) = (contract_last_safe_claim_at IS NULL)),
  CHECK ((contract_intent_digest IS NULL) = (contract_refund_after IS NULL)),
  CHECK ((contract_intent_digest IS NULL) = (contract_solver_signature IS NULL)),
  CHECK ((contract_intent_digest IS NULL) = (contract_authorized_at IS NULL)),
  CHECK ((contract_intent_digest IS NULL) = (contract_intent_record_digest IS NULL)),
  CHECK ((contract_intent_digest IS NULL) = (contract_intent_bound_at IS NULL)),
  CHECK (contract_transaction_value IS NULL OR contract_transaction_value = '0x0'),
  CHECK (contract_intent_digest IS NULL OR contract_quote_expires_at < contract_last_safe_claim_at),
  CHECK (contract_intent_digest IS NULL OR contract_last_safe_claim_at < contract_refund_after),
  CHECK (contract_intent_digest IS NULL OR contract_authorized_at <= contract_intent_bound_at),
  CHECK (contract_intent_digest IS NULL OR contract_intent_bound_at < contract_quote_expires_at),
...
const PERSISTED_CONTRACT_INTENT_COLUMNS = Object.freeze([
  "contract_quote_id",
  "contract_intent_digest",
  "contract_intent_code_hash",
  "contract_chain_id",
  "contract_from_address",
  "contract_to_address",
  "contract_calldata",
  "contract_calldata_digest",
  "contract_transaction_value",
  "contract_user_address",
  "contract_solver_address",
  "contract_beneficiary_address",
  "contract_bit_amount",
  "contract_fee_bit_amount",
  "contract_nonce",
  "contract_quote_expires_at",
  "contract_last_safe_claim_at",
  "contract_refund_after",
  "contract_solver_signature",
  "contract_user_signature",
  "contract_authorized_at",
  "contract_intent_record_digest",
  "contract_intent_bound_at",
]);

function verifyPersistedContractIntents(database) {
  const rows = database.prepare(`
    SELECT settlement_id, direction, intent_digest, selected_offer_id, payment_hash,
      invoice_digest, amount_sats, ${PERSISTED_CONTRACT_INTENT_COLUMNS.join(", ")}
    FROM settlements
    ORDER BY settlement_id
  `).all();
  for (const row of rows) {
    const hasIntent = row.contract_intent_digest !== null;
    const requiredColumns = PERSISTED_CONTRACT_INTENT_COLUMNS.filter(
      (column) => column !== "contract_user_signature",
    );
    const hasUnexpectedPartialValue = !hasIntent && PERSISTED_CONTRACT_INTENT_COLUMNS.some(
      (column) => row[column] !== null,
    );
    const hasMissingRequiredValue = hasIntent && requiredColumns.some(
      (column) => row[column] === null,
    );
    if (hasUnexpectedPartialValue || hasMissingRequiredValue) {
      throw new Error("coordinator contract intent record is incomplete");
    }
    if (!hasIntent) continue;
    const lightningToBit = row.direction === "lightning-to-bit";
    if ((lightningToBit && row.contract_user_signature === null)
        || (!lightningToBit && row.contract_user_signature !== null)) {
      throw new Error("coordinator contract intent signature set is inconsistent");
    }
    const beneficiaryField = lightningToBit ? "beneficiary" : "solverBeneficiary";
    const nonceField = lightningToBit ? "nonce" : "solverNonce";
    let validated;
    try {
      validated = validateContractIntentBinding({
        settlementId: row.settlement_id,
        direction: row.direction,
        userAuthorizationDigest: row.intent_digest,
        selectedOfferId: row.selected_offer_id,
        settlementContractCodeHash: row.contract_intent_code_hash,
        domain: {
          name: lightningToBit ? "TreeSwap BIT Vault" : "TreeSwap User BIT Escrow",
          version: "1",
          chainId: row.contract_chain_id,
          verifyingContract: row.contract_to_address,
        },
        primaryType: lightningToBit ? "SelectedQuote" : "BitToLightningQuote",
        message: {
          quoteId: row.contract_quote_id,
          user: row.contract_user_address,
          solver: row.contract_solver_address,
          [beneficiaryField]: row.contract_beneficiary_address,
          amount: row.contract_bit_amount,
          fee: row.contract_fee_bit_amount,
          lightningAmountSats: row.amount_sats,
          paymentHash: row.payment_hash,
          invoiceDigest: row.invoice_digest,
          [nonceField]: row.contract_nonce,
          quoteExpiresAt: row.contract_quote_expires_at,
          lastSafeClaimAt: row.contract_last_safe_claim_at,
          refundAfter: row.contract_refund_after,
        },
        solverSignature: row.contract_solver_signature,
        userSignature: row.contract_user_signature,
        transaction: {
          chainId: row.contract_chain_id,
          from: row.contract_from_address,
          to: row.contract_to_address,
          data: row.contract_calldata,
          dataDigest: row.contract_calldata_digest,
          value: row.contract_transaction_value,
        },
        authorizedAt: row.contract_authorized_at,
        expiresAt: row.contract_quote_expires_at,
      });
    } catch (error) {
      throw new Error("coordinator contract intent record failed cryptographic verification", { cause: error });
    }
    const expected = Object.freeze({
      contract_quote_id: validated.quoteId,
      contract_intent_digest: validated.contractIntentDigest,
      contract_intent_code_hash: validated.settlementContractCodeHash,
      contract_chain_id: validated.chainId,
      contract_from_address: validated.fromAddress,
      contract_to_address: validated.toAddress,
      contract_calldata: validated.calldata,
      contract_calldata_digest: validated.calldataDigest,
      contract_transaction_value: validated.transactionValue,
      contract_user_address: validated.userAddress,
      contract_solver_address: validated.solverAddress,
      contract_beneficiary_address: validated.beneficiaryAddress,
      contract_bit_amount: validated.bitAmount,
      contract_fee_bit_amount: validated.feeBitAmount,
      contract_nonce: validated.nonce,
      contract_quote_expires_at: validated.quoteExpiresAt,
      contract_last_safe_claim_at: validated.lastSafeClaimAt,
      contract_refund_after: validated.refundAfter,
      contract_solver_signature: validated.solverSignature,
      contract_user_signature: validated.userSignature,
      contract_authorized_at: validated.authorizedAt,
      contract_intent_record_digest: digest(validated),
    });
    const changedField = Object.entries(expected).find(([column, value]) => row[column] !== value);
    if (changedField
        || row.contract_intent_bound_at < validated.authorizedAt
        || row.contract_intent_bound_at >= validated.expiresAt) {
      throw new Error("coordinator contract intent record does not match its signed escrow transaction");
    }
  }
}
...
function releaseLiabilitySnapshotForDatabase(database) {
  const rows = database.prepare(`
    SELECT settlement_id, direction, contract_intent_digest, contract_intent_record_digest,
      release_record_digest, risk_policy_digest, evidence_policy_digest,
  ...
  `).all().map((row) => Object.freeze({
    ...
    contractIntentRecordDigest: row.contract_intent_record_digest,
  ...
  const unbound = rows.filter((row) => (
    row.contractIntentDigest === null || row.contractIntentRecordDigest === null
      || row.releaseRecordDigest === null
  ));
  const releases = [];
  ...
  for (const releaseRecordDigest of releaseDigests) {
    ...
    releases.push(Object.freeze({
      ...
      liabilitySetDigest: digest({
        schema: "treeswap.coordinator-release-liability-set.v2",
        releaseRecordDigest,
  ...
  const core = Object.freeze({
    schema: "treeswap.coordinator-release-liabilities.v2",
    ...
    unboundLiabilitySetDigest: digest({
      schema: "treeswap.coordinator-unbound-liability-set.v2",
      settlements: unbound,
    }),
...
export function validatedCoordinatorAdmissionPolicy(input) {
  return normalizeAdmissionPolicy(input);
}
...
function normalizeAuthorizedFirmOfferSettlement(input) {
  exactKeys(
    input,
    [
      "authorizationExpiresAt", "authorizedAt", "executionAuthorizationDigest",
      "executionBindingDigest", "offerId", "settlement",
    ],
    "authorized firm offer settlement",
  );
  return Object.freeze({
    authorization: normalizeFirmOfferUserAuthorization({
...
function asSettlement(row) {
  ...
  return Object.freeze({
    ...
    contractQuoteId: row.contract_quote_id,
    contractIntentDigest: row.contract_intent_digest,
    contractIntentCodeHash: row.contract_intent_code_hash,
    contractChainId: row.contract_chain_id,
    contractFromAddress: row.contract_from_address,
    contractToAddress: row.contract_to_address,
    contractCalldata: row.contract_calldata,
    contractCalldataDigest: row.contract_calldata_digest,
    contractTransactionValue: row.contract_transaction_value,
    contractUserAddress: row.contract_user_address,
    contractSolverAddress: row.contract_solver_address,
    contractBeneficiaryAddress: row.contract_beneficiary_address,
    contractBitAmount: row.contract_bit_amount,
    contractFeeBitAmount: row.contract_fee_bit_amount,
    contractNonce: row.contract_nonce,
    contractQuoteExpiresAt: row.contract_quote_expires_at,
    contractLastSafeClaimAt: row.contract_last_safe_claim_at,
    contractRefundAfter: row.contract_refund_after,
    contractSolverSignature: row.contract_solver_signature,
    contractUserSignature: row.contract_user_signature,
    contractAuthorizedAt: row.contract_authorized_at,
    contractIntentRecordDigest: row.contract_intent_record_digest,
    contractIntentBoundAt: row.contract_intent_bound_at,
    paymentHash: row.payment_hash,
    invoiceDigest: row.invoice_digest,
...
export class CoordinatorStore {
  ...
  static async open(path, { allowMemory = false } = {}) {
    ...
    try {
      ...
      if (existingSchemaValue && existingSchemaValue !== COORDINATOR_SCHEMA) {
        const legacyTables = new Set(database.prepare(
          "SELECT name FROM sqlite_schema WHERE type = 'table'",
        ).all().map(({ name }) => name));
        const activeLegacyOffers = legacyTables.has("firm_offer_commitments")
        ...
        const openLegacySettlements = legacyTables.has("settlements")
          ? Number(database.prepare(
      ...
      const contractIntentColumns = Object.freeze([
        ["contract_quote_id", "TEXT"],
        ["contract_intent_digest", "TEXT"],
        ["contract_intent_code_hash", "TEXT"],
        ["contract_chain_id", "TEXT"],
        ["contract_from_address", "TEXT"],
        ["contract_to_address", "TEXT"],
        ["contract_calldata", "TEXT"],
        ["contract_calldata_digest", "TEXT"],
        ["contract_transaction_value", "TEXT"],
        ["contract_user_address", "TEXT"],
        ["contract_solver_address", "TEXT"],
        ["contract_beneficiary_address", "TEXT"],
      ...
      for (const [name, type] of contractIntentColumns) {
        if (!settlementColumns.some((column) => column.name === name)) {
          database.exec(`ALTER TABLE settlements ADD COLUMN ${name} ${type}`);
        }
      }
      database.exec(`
        CREATE UNIQUE INDEX IF NOT EXISTS settlements_contract_quote_id_unique
          ON settlements(contract_quote_id) WHERE contract_quote_id IS NOT NULL;
        CREATE UNIQUE INDEX IF NOT EXISTS settlements_contract_intent_digest_unique
          ON settlements(contract_intent_digest) WHERE contract_intent_digest IS NOT NULL;
        CREATE UNIQUE INDEX IF NOT EXISTS settlements_contract_calldata_digest_unique
          ON settlements(contract_calldata_digest) WHERE contract_calldata_digest IS NOT NULL;
      `);
      if (existingSchemaValue && existingSchemaValue !== COORDINATOR_SCHEMA) {
        database.exec(`
  ...
  #releaseActiveFirmOffers(requestId, outcome, recordedAt) {
    const settlementOwned = Number(this.#db.prepare(`
      SELECT COUNT(*) AS count
      FROM firm_offer_commitments AS offers
      INNER JOIN settlements ON settlements.selected_offer_id = offers.offer_id
      WHERE offers.request_id = ? AND offers.state = 'ACTIVE'
    `).get(requestId).count);
    if (settlementOwned !== 0) {
      throw new Error("settlement-owned firm offer requires settlement reconciliation");
    }
    const rows = this.#db.prepare(`
  ...
  #expireRfqRequests(observedAt, { identityCommitment = null, limit = 1_000 } = {}) {
    ...
    let expired = 0;
    for (const row of rows) {
      const activeOffers = this.#db.prepare(`
        SELECT offers.*
        FROM firm_offer_commitments AS offers
        LEFT JOIN settlements ON settlements.selected_offer_id = offers.offer_id
        WHERE offers.request_id = ? AND offers.state = 'ACTIVE' AND settlements.settlement_id IS NULL
      ...
      const settlementOwned = Number(this.#db.prepare(`
        ...
        FROM firm_offer_commitments AS offers
        INNER JOIN settlements ON settlements.selected_offer_id = offers.offer_id
        WHERE offers.request_id = ?
      `).get(row.request_id).count);
      if (settlementOwned !== 0) continue;
  ...
  #closeExpiredEmptyRfqRequests(observedAt, limit) {
    const rows = this.#db.prepare(`
      SELECT requests.request_id
      FROM rfq_requests AS requests
      WHERE requests.state = 'ACTIVE' AND requests.expires_at <= ?
        AND NOT EXISTS (
          SELECT 1 FROM firm_offer_commitments AS offers
          WHERE offers.request_id = requests.request_id AND offers.state = 'ACTIVE'
        )
        AND NOT EXISTS (
          SELECT 1
          FROM firm_offer_commitments AS offers
          INNER JOIN settlements ON settlements.selected_offer_id = offers.offer_id
          WHERE offers.request_id = requests.request_id
        )
      ORDER BY requests.expires_at, requests.request_id
      LIMIT ?
    `).all(observedAt, limit);
    for (const row of rows) {
      const proof = digest({ code: "RFQ_EXPIRED", observedAt, requestId: row.request_id });
      this.#db.prepare(`
        UPDATE rfq_requests SET state = 'EXPIRED', resolution_digest = ?, resolved_at = ?
  ...
  reconcileRfqFinalizationRestart(input) {
    exactKeys(input, ["limit", "observedAt"], "RFQ finalization restart reconciliation");
    const observedAt = integer(input.observedAt, "observedAt");
    const limit = integer(input.limit, "limit");
    if (limit === 0 || limit > 1_000) {
      throw new RangeError("RFQ finalization restart limit is outside policy");
    }
    return this.#transaction(() => {
      const priorClock = this.#db.prepare(
        "SELECT value FROM coordinator_meta WHERE key = ?",
      ).get(RFQ_FINALIZATION_RESTART_CLOCK_KEY);
      if (priorClock) {
        if (!UINT.test(priorClock.value)
            || BigInt(priorClock.value) > BigInt(Number.MAX_SAFE_INTEGER)) {
          throw new Error("RFQ finalization restart clock record is invalid");
        }
        if (observedAt < Number(priorClock.value)) {
          throw new Error("RFQ finalization restart clock moved backward");
        }
      }
      const liabilityClock = this.#db.prepare(`
        SELECT MAX(value) AS value FROM (
          SELECT MAX(last_observed_at) AS value FROM rfq_identities
          UNION ALL SELECT MAX(updated_at) AS value FROM solver_capacity
          UNION ALL SELECT MAX(COALESCE(resolved_at, authorized_at, finalized_at, reserved_at)) AS value
            FROM firm_offer_commitments
          UNION ALL SELECT MAX(updated_at) AS value FROM settlements
        )
      `).get().value;
      if (liabilityClock !== null) {
        if (!Number.isSafeInteger(liabilityClock) || liabilityClock < 0) {
          throw new Error("RFQ finalization restart liability clock is invalid");
        }
        if (observedAt < liabilityClock) {
          throw new Error("RFQ finalization restart clock moved backward behind its durable liability ledger");
        }
      }

      const malformed = this.#db.prepare(`
        SELECT
          COUNT(CASE WHEN requests.state <> 'ACTIVE' THEN 1 END) AS inactive_requests,
          COUNT(CASE WHEN offers.expires_at > requests.expires_at THEN 1 END) AS invalid_expiry,
          COUNT(CASE WHEN
            (offers.private_request_digest IS NULL) <> (offers.executable_offer_digest IS NULL)
            OR (offers.private_request_digest IS NULL) <> (offers.execution_binding_digest IS NULL)
            OR (offers.private_request_digest IS NULL) <> (offers.finalized_at IS NULL)
            OR (offers.execution_authorization_digest IS NULL) <>
              (offers.execution_authorization_expires_at IS NULL)
            OR (offers.execution_authorization_digest IS NULL) <> (offers.authorized_at IS NULL)
            OR (offers.execution_authorization_digest IS NOT NULL AND offers.execution_binding_digest IS NULL)
          THEN 1 END) AS invalid_binding
        FROM firm_offer_commitments AS offers
        INNER JOIN rfq_requests AS requests ON requests.request_id = offers.request_id
        WHERE offers.state = 'ACTIVE'
      `).get();
      if (Number(malformed.inactive_requests) !== 0
          || Number(malformed.invalid_expiry) !== 0
          || Number(malformed.invalid_binding) !== 0) {
        throw new Error("RFQ finalization restart found inconsistent active liability state");
      }
      const unsafeSettlementOwnership = Number(this.#db.prepare(`
        SELECT COUNT(*) AS count
        FROM firm_offer_commitments AS offers
        INNER JOIN settlements ON settlements.selected_offer_id = offers.offer_id
        INNER JOIN rfq_requests AS requests ON requests.request_id = offers.request_id
        WHERE offers.state = 'ACTIVE'
          AND (
            offers.execution_binding_digest IS NULL
            OR offers.execution_authorization_digest IS NULL
            OR settlements.pricing_id <> offers.request_id
            OR settlements.direction <> offers.direction
            OR settlements.amount_sats <> offers.lightning_amount_sats
            OR settlements.capacity_epoch <> offers.capacity_epoch
            OR settlements.created_at < offers.authorized_at
            OR settlements.created_at >= offers.execution_authorization_expires_at
            OR settlements.created_at >= offers.expires_at
            OR requests.state <> 'ACTIVE'
            OR requests.direction <> settlements.direction
            OR requests.notional_sats <> settlements.amount_sats
          )
      `).get().count);
      if (unsafeSettlementOwnership !== 0) {
        throw new Error("settlement-owned firm offer lacks complete user authorization");
      }
      const duplicateSettlementOwnership = Number(this.#db.prepare(`
      ...
      `).get().count);
      if (duplicateSettlementOwnership !== 0) {
        throw new Error("RFQ finalization restart found multiple settlements for one RFQ");
      }

      this.#db.prepare(`
        INSERT INTO coordinator_meta(key, value) VALUES (?, ?)
        ON CONFLICT(key) DO UPDATE SET value = excluded.value
      `).run(RFQ_FINALIZATION_RESTART_CLOCK_KEY, String(observedAt));

      const due = this.#db.prepare(`
        SELECT offers.*
        FROM firm_offer_commitments AS offers
        LEFT JOIN settlements ON settlements.selected_offer_id = offers.offer_id
        WHERE offers.state = 'ACTIVE' AND offers.expires_at <= ? AND settlements.settlement_id IS NULL
        ORDER BY offers.expires_at, offers.offer_id
        LIMIT ?
      `).all(observedAt, limit);
      for (const offer of due) {
        this.#completeFirmOffer(
          ...
          "expired-unexercised",
          digest({
            code: "RFQ_FINALIZATION_RESTART_EXPIRED",
            offerId: offer.offer_id,
            observedAt,
      ...
      const expiredRequestsClosed = this.#closeExpiredEmptyRfqRequests(observedAt, limit);
      const phases = this.#db.prepare(`
        SELECT
          COUNT(CASE WHEN offers.execution_binding_digest IS NULL THEN 1 END) AS reserved,
          COUNT(CASE WHEN offers.execution_binding_digest IS NOT NULL
            AND offers.execution_authorization_digest IS NULL THEN 1 END) AS finalized,
          COUNT(CASE WHEN offers.execution_authorization_digest IS NOT NULL THEN 1 END) AS authorized,
          MIN(offers.expires_at) AS earliest_expiry
        FROM firm_offer_commitments AS offers
        LEFT JOIN settlements ON settlements.selected_offer_id = offers.offer_id
        WHERE offers.state = 'ACTIVE' AND settlements.settlement_id IS NULL
      `).get();
      const settlementOwned = this.#db.prepare(`
        SELECT
          COUNT(*) AS total,
          COUNT(CASE WHEN settlements.terminal_state IS NULL THEN 1 END) AS nonterminal,
          COUNT(CASE WHEN settlements.terminal_state IS NOT NULL THEN 1 END) AS terminal
        FROM firm_offer_commitments AS offers
        INNER JOIN settlements ON settlements.selected_offer_id = offers.offer_id
        WHERE offers.state = 'ACTIVE'
      `).get();
      const expiredReleaseBacklog = Number(this.#db.prepare(`
        SELECT COUNT(*) AS count
        FROM firm_offer_commitments AS offers
        LEFT JOIN settlements ON settlements.selected_offer_id = offers.offer_id
        WHERE offers.state = 'ACTIVE' AND offers.expires_at <= ? AND settlements.settlement_id IS NULL
      `).get(observedAt).count);
      const settlementOwnedCommitments = Number(settlementOwned.total);
      const pendingUnsettledCommitments = Number(phases.reserved)
        + Number(phases.finalized) + Number(phases.authorized);
      const core = Object.freeze({
        schema: RFQ_FINALIZATION_RESTART_SCHEMA,
        state: settlementOwnedCommitments !== 0
          ? "settlement-recovery-required"
          : expiredReleaseBacklog !== 0
            ? "cleanup-incomplete"
            : pendingUnsettledCommitments !== 0 ? "waiting-for-expiry" : "ready",
        observedAt,
        releasedExpiredCommitments: due.length,
        expiredRequestsClosed,
        pendingReservedCommitments: Number(phases.reserved),
        pendingFinalizedCommitments: Number(phases.finalized),
        pendingAuthorizedCommitments: Number(phases.authorized),
        earliestPendingExpiry: phases.earliest_expiry ?? null,
        expiredReleaseBacklog,
        settlementOwnedCommitments,
        nonterminalSettlementOwnedCommitments: Number(settlementOwned.nonterminal),
        terminalSettlementOwnedCommitments: Number(settlementOwned.terminal),
  ...
  reserveVerifiedFirmOffer(input) {
    ...
    return this.#transaction(() => {
      ...
      const preclaimedSettlement = this.#db.prepare(`
        SELECT settlement_id FROM settlements
        WHERE selected_offer_id = ? OR pricing_id = ? LIMIT 1
      `).get(value.offerId, value.requestId);
      if (preclaimedSettlement) {
        throw new Error("firm offer identifier was already claimed by a settlement");
      }
  ...
  #bindFirmOfferUserAuthorization(value) {
    const current = this.getFirmOffer(value.offerId);
    if (!current) throw new Error("firm offer does not exist");
  ...
  bindFirmOfferUserAuthorization(input) {
    ...
    return this.#transaction(() => this.#bindFirmOfferUserAuthorization(value));
  }
  ...
  recordFirmOfferOutcome(input) {
    ...
    return this.#transaction(() => {
      ...
      const selectedSettlement = this.#db.prepare(`
        SELECT settlement_id, terminal_state, terminal_proof_digest, terminal_at
        FROM settlements WHERE selected_offer_id = ? LIMIT 1
      `).get(offerId);
      const reconciledWithoutFill = selectedSettlement && input.outcome === "user-abandoned"
        && ["REFUNDED", "FAILED", "CANCELED"].includes(selectedSettlement.terminal_state)
        && selectedSettlement.terminal_proof_digest === evidenceDigest
        && selectedSettlement.terminal_at <= recordedAt;
      if (selectedSettlement && input.outcome !== "filled" && !reconciledWithoutFill) {
        throw new Error("settlement-owned firm offer requires exact terminal settlement reconciliation");
      }
      ...
      } else if (reconciledWithoutFill) {
        const request = this.getRfqRequest(completed.requestId);
        if (!request || request.state !== "ACTIVE") {
          throw new Error("terminally reconciled firm offer requires an active RFQ");
        }
        this.#releaseActiveFirmOffers(completed.requestId, "user-abandoned", recordedAt);
        this.#db.prepare(`
          UPDATE rfq_requests SET state = 'ABANDONED', resolution_digest = ?, resolved_at = ?
          WHERE request_id = ?
        `).run(evidenceDigest, recordedAt, completed.requestId);
      }
  ...
  #acceptSettlement(value) {
    const recordDigest = digest(value);
    const existing = this.getSettlement(value.settlementId);
    if (existing) {
      if (existing.recordDigest !== recordDigest) {
        throw new Error("settlement identifier was already bound to different terms");
      }
      return existing;
    }
    const offer = this.getFirmOffer(value.selectedOfferId);
    if (!offer && this.getRfqRequest(value.pricingId)) {
      throw new Error("settlement selected firm offer does not exist");
    }
    if (offer) {
      const request = this.getRfqRequest(offer.requestId);
      const existingRequestSettlement = this.#db.prepare(`
        ...
        LIMIT 1
      `).get(offer.requestId);
      if (existingRequestSettlement) {
        throw new Error("RFQ already has a settlement");
      }
      if (offer.state !== "ACTIVE" || offer.expiresAt <= value.createdAt
          || offer.selectionAuthorizationExpiresAt <= value.createdAt
          || offer.executionAuthorizationExpiresAt <= value.createdAt
          || !offer.privateRequestDigest || !offer.executableOfferDigest || !offer.executionBindingDigest
          || !offer.executionAuthorizationDigest
          || offer.reservedAt > value.createdAt || offer.finalizedAt > value.createdAt
          || offer.authorizedAt > value.createdAt) {
        throw new Error("settlement requires an active fully authorized firm offer");
      }
      if (!request || request.state !== "ACTIVE" || request.expiresAt <= value.createdAt
          || request.requestId !== value.pricingId || request.direction !== value.direction
          || request.notionalSats !== value.amountSats || offer.direction !== value.direction
          || (offer.direction === "lightning-to-bit" && offer.lightningAmountSats !== value.amountSats)
          || offer.capacityEpoch !== value.capacityEpoch) {
        throw new Error("settlement does not match its active RFQ, firm offer, amount, or capacity epoch");
      }
    }
    try {
      this.#db.prepare(`
  ...
  acceptSettlement(input) {
    ...
    return this.#transaction(() => this.#acceptSettlement(value));
  }

  acceptAuthorizedFirmOfferSettlement(input) {
    const value = normalizeAuthorizedFirmOfferSettlement(input);
    return this.#transaction(() => {
      const before = this.getFirmOffer(value.authorization.offerId);
      if (!before) throw new Error("firm offer does not exist");
      const request = this.getRfqRequest(before.requestId);
      if (value.settlement.selectedOfferId !== before.offerId
          || value.settlement.pricingId !== before.requestId
          || value.settlement.direction !== before.direction
          || !request || request.state !== "ACTIVE"
          || value.settlement.amountSats !== request.notionalSats
          || (before.direction === "lightning-to-bit"
            && value.settlement.amountSats !== before.lightningAmountSats)
          || value.settlement.capacityEpoch !== before.capacityEpoch
          || value.settlement.nonceAuthorityDigest !== before.selectionAuthorizationDigest
          || value.settlement.intentDigest !== value.authorization.executionAuthorizationDigest
          || value.settlement.selectedSetDigest !== before.offerDigest
          || value.settlement.createdAt !== value.authorization.authorizedAt) {
        throw new Error("authorized settlement does not match its exact firm-offer commitments");
      }
      const offer = this.#bindFirmOfferUserAuthorization(value.authorization);
      const settlement = this.#acceptSettlement(value.settlement);
      if (settlement.selectedOfferId !== offer.offerId
          || settlement.intentDigest !== offer.executionAuthorizationDigest) {
        throw new Error("authorized settlement handoff was not persisted atomically");
      }
      return Object.freeze({ offer, settlement });
    });
  }

  bindContractIntent(input) {
    exactKeys(input, ["binding", "boundAt"], "contract intent persistence");
    const binding = validateContractIntentBinding(input.binding);
    const boundAt = integer(input.boundAt, "contract intent boundAt");
    const contractIntentRecordDigest = digest(binding);
    return this.#transaction(() => {
      const settlement = this.getSettlement(binding.settlementId);
      if (!settlement) throw new Error("contract intent settlement does not exist");
      if (settlement.contractIntentDigest) {
        if (settlement.contractIntentRecordDigest !== contractIntentRecordDigest) {
          throw new Error("settlement is already bound to a different contract intent");
        }
        return settlement;
      }
      if (settlement.state !== "INTENT_ACCEPTED" || settlement.executionPolicyBindingDigest
          || settlement.reservationId || settlement.lastActionId || settlement.terminalState
          || settlement.haltCode || settlement.reconciliationRequired) {
        throw new Error("contract intent must bind before execution policy, reservation, actions, or closure");
      }
      if (binding.direction !== settlement.direction
          || binding.userAuthorizationDigest !== settlement.intentDigest
          || binding.selectedOfferId !== settlement.selectedOfferId
          || binding.paymentHash !== settlement.paymentHash
          || binding.invoiceDigest !== settlement.invoiceDigest
          || binding.lightningAmountSats !== settlement.amountSats) {
        throw new Error("contract intent does not match the durable authorized settlement");
      }
      if (binding.direction === "lightning-to-bit" && binding.nonce !== settlement.intentNonce) {
        throw new Error("Lightning-to-BIT contract intent changed the user nonce");
      }
      if (binding.authorizedAt < settlement.createdAt || boundAt < binding.authorizedAt
          || boundAt >= binding.expiresAt) {
        throw new Error("contract intent persistence time is outside its authorization window");
      }
      const offer = this.getFirmOffer(settlement.selectedOfferId);
      if (offer) {
        const mismatches = [];
        if (offer.state !== "ACTIVE") mismatches.push("inactive-offer");
        if (offer.solverId.toLowerCase() !== binding.solverAddress) mismatches.push("solver");
        if ((offer.direction === "lightning-to-bit" || offer.bitAmountWei !== "0")
            && offer.bitAmountWei !== binding.bitAmount) mismatches.push("bit-amount");
        if ((offer.direction === "lightning-to-bit"
              && offer.lightningAmountSats !== binding.lightningAmountSats)
            || (offer.direction === "bit-to-lightning"
              && BigInt(offer.lightningAmountSats) < BigInt(binding.lightningAmountSats))) {
          mismatches.push("lightning-amount");
        }
        if (offer.executionAuthorizationDigest !== settlement.intentDigest) mismatches.push("user-authorization");
        if (offer.authorizedAt > binding.authorizedAt) mismatches.push("authorization-time");
        if (offer.expiresAt < binding.expiresAt) mismatches.push("offer-expiry");
        if (offer.selectionAuthorizationExpiresAt < binding.expiresAt) mismatches.push("selection-expiry");
        if (offer.executionAuthorizationExpiresAt < binding.expiresAt) mismatches.push("execution-expiry");
        if (mismatches.length !== 0) {
          throw new Error(`contract intent does not match the selected durable firm offer: ${mismatches.join(",")}`);
        }
      }
      try {
        const update = this.#db.prepare(`
          UPDATE settlements SET
            contract_quote_id = ?, contract_intent_digest = ?, contract_intent_code_hash = ?,
            contract_chain_id = ?, contract_from_address = ?, contract_to_address = ?, contract_calldata = ?,
            contract_calldata_digest = ?, contract_transaction_value = ?, contract_user_address = ?,
            contract_solver_address = ?, contract_beneficiary_address = ?, contract_bit_amount = ?,
            contract_fee_bit_amount = ?, contract_nonce = ?, contract_quote_expires_at = ?,
            contract_last_safe_claim_at = ?, contract_refund_after = ?, contract_solver_signature = ?,
            contract_user_signature = ?, contract_authorized_at = ?, contract_intent_record_digest = ?,
            contract_intent_bound_at = ?, state = 'CONTRACT_INTENT_BOUND', version = version + 1, updated_at = ?
          WHERE settlement_id = ? AND state = 'INTENT_ACCEPTED' AND contract_intent_digest IS NULL
        `).run(
          binding.quoteId,
          binding.contractIntentDigest,
          binding.settlementContractCodeHash,
          binding.chainId,
          binding.fromAddress,
          binding.toAddress,
          binding.calldata,
          binding.calldataDigest,
          binding.transactionValue,
          binding.userAddress,
          binding.solverAddress,
          binding.beneficiaryAddress,
          binding.bitAmount,
          binding.feeBitAmount,
          binding.nonce,
          binding.quoteExpiresAt,
          binding.lastSafeClaimAt,
          binding.refundAfter,
          binding.solverSignature,
          binding.userSignature,
          binding.authorizedAt,
          contractIntentRecordDigest,
          boundAt,
          boundAt,
          settlement.settlementId,
        );
        if (update.changes !== 1) throw new Error("contract intent was not persisted atomically");
      } catch (error) {
        if (/constraint|unique/i.test(String(error?.message))) {
          throw new Error("contract intent conflicts with an existing quote or calldata commitment");
        }
        throw error;
      }
      this.#event({
        ...
        eventType: "SETTLEMENT",
        eventCode: "CONTRACT_INTENT_BOUND",
        dataDigest: contractIntentRecordDigest,
        occurredAt: boundAt,
      });
  ...
  bindSettlementExecutionPolicy(input) {
    ...
    return this.#transaction(() => {
      ...
      if (settlement.state !== "CONTRACT_INTENT_BOUND" || !settlement.contractIntentDigest
          || settlement.reservationId || settlement.lastActionId
          || settlement.terminalState || settlement.haltCode || settlement.reconciliationRequired) {
        throw new Error("settlement execution policy requires the contract intent before reservation, actions, or closure");
      }
      ...
      const update = this.#db.prepare(`
        ...
        SET release_record_digest = ?, risk_policy_digest = ?, evidence_policy_digest = ?, solver_capability_digest = ?,
          execution_policy_binding_digest = ?, execution_policy_bound_at = ?, state = 'EXECUTION_POLICY_BOUND',
          version = version + 1, updated_at = ?
        WHERE settlement_id = ? AND state = 'CONTRACT_INTENT_BOUND' AND release_record_digest IS NULL
      `).run(
  ...
  recordReservation(input) {
    ...
    return this.#transaction(() => {
      ...
      if (!settlement.contractIntentDigest) throw new Error("reservation requires the durable contract intent");
      if (settlement.contractIntentDigest !== value.reservationIntentDigest) {
        throw new Error("reservation contract intent digest does not match");
      }
      if (settlement.contractQuoteId !== value.reservationId) {
        throw new Error("reservation quote ID does not match the durable contract intent");
      }
      ...
      if (!["CONTRACT_INTENT_BOUND", "EXECUTION_POLICY_BOUND"].includes(settlement.state)) {
        throw new Error("reservation requires a contract-bound settlement state");
      }
      this.#db.prepare(`
  ...
  planAction(input) {
    ...
    return this.#transaction(() => {
      ...
      if (!settlement.contractIntentDigest) {
        throw new Error("action requires the durable contract intent");
      }
      ...
      if (
        settlement.contractIntentDigest !== value.intentDigest || settlement.paymentHash !== value.paymentHash
