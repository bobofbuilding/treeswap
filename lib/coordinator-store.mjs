...
function normalizeSolverSnapshot(input) {
  exactKeys(input, [
    "availableBitWei", "availableLightningSats", "capabilityDigest", "capabilityVerified", "capacityEpoch", "observedAt", "solverId",
  ...
  return Object.freeze({
    ...
    capabilityDigest,
    capacityEpoch: integer(input.capacityEpoch, "capacityEpoch"),
    capacityObservedAt: integer(input.observedAt, "observedAt"),
...
export class CoordinatorStore {
  ...
  static async open(path, { allowMemory = false } = {}) {
    ...
    database.prepare("INSERT OR IGNORE INTO coordinator_meta(key, value) VALUES ('schema', 'treeswap.coordinator.v3')").run();
    const schema = database.prepare("SELECT value FROM coordinator_meta WHERE key = 'schema'").get();
    if (["treeswap.coordinator.v1", "treeswap.coordinator.v2"].includes(schema?.value)) {
      database.prepare("UPDATE coordinator_meta SET value = 'treeswap.coordinator.v3' WHERE key = 'schema'").run();
    } else if (schema?.value !== "treeswap.coordinator.v3") {
      database.close();
  ...
  recordSolverCapacity(input) {
    ...
    return this.#transaction(() => {
      ...
      if (!existing) {
        this.#db.prepare(`
          INSERT INTO solver_capacity(
            solver_id, capability_digest, snapshot_digest, capacity_epoch, capacity_observed_at,
            available_bit_wei, available_lightning_sats, created_at, updated_at
          ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(
          value.solverId, value.capabilityDigest, snapshotDigest, value.capacityEpoch, value.capacityObservedAt,
      ...
      this.#db.prepare(`
        UPDATE solver_capacity SET capability_digest = ?, snapshot_digest = ?, capacity_epoch = ?,
      ...
      `).run(
        value.capabilityDigest, snapshotDigest, value.capacityEpoch, value.capacityObservedAt,
