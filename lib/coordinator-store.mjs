...
const COORDINATOR_SCHEMA = "treeswap.coordinator.v8";
...
function releaseLiabilitySnapshotForDatabase(database) {
  const rows = database.prepare(`
    SELECT settlement_id, direction, release_record_digest, evidence_policy_digest,
  ...
  for (const releaseRecordDigest of releaseDigests) {
    ...
    const policyKeys = [...new Set(releaseRows.map((row) => `${row.direction}:${row.evidencePolicyDigest}`))].sort();
    const executionPolicies = policyKeys.map((key) => {
      const split = key.indexOf(":");
      const direction = key.slice(0, split);
      const evidencePolicyDigest = key.slice(split + 1);
      const policyRows = releaseRows.filter((row) => (
        row.direction === direction && row.evidencePolicyDigest === evidencePolicyDigest
      ));
      return Object.freeze({
        direction,
...
function normalizeSettlementExecutionPolicy(input) {
  exactKeys(input, [
    "boundAt", "evidencePolicyDigest", "releaseRecordDigest", "settlementId", "solverCapabilityDigest",
  ...
  if ([releaseRecordDigest, evidencePolicyDigest, solverCapabilityDigest]
...
function normalizeFirmOfferReservation(input) {
  exactKeys(input, [
    ...
    "marketRiskDigest", "marketRiskValidUntil", "selectionAuthorizationDigest",
      ...
      try {
        this.#db.prepare(`
          INSERT INTO firm_offer_commitments(
            ...
            market_risk_digest, market_risk_valid_until,
        ...
        `).run(
          ...
          value.selectionAuthorizationExpiresAt, value.marketRiskDigest, value.marketRiskValidUntil,
    ...
    return this.#transaction(() => {
      ...
      if (settlement.executionPolicyBindingDigest) {
        ...
        const expected = digest({
          schema: "treeswap.settlement-execution-policy.v2",
      ...
      const update = this.#db.prepare(`
        UPDATE settlements
        SET release_record_digest = ?, evidence_policy_digest = ?, solver_capability_digest = ?,
