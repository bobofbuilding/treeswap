...
const COORDINATOR_SCHEMA = "treeswap.coordinator.v5";
...
function normalizeFirmOfferReservation(input) {
  ...
  exactKeys(input.offer, [
    "bitAmountWei", "capacityEpoch", "direction", "expiresAt", "lightningAmountSats",
  ...
  static async open(path, { allowMemory = false } = {}) {
    ...
    try {
      ...
      if (existingMeta) {
        ...
        if (![COORDINATOR_SCHEMA, "treeswap.coordinator.v1", "treeswap.coordinator.v2", "treeswap.coordinator.v3", "treeswap.coordinator.v4"].includes(existingSchemaValue)) {
      ...
      const schema = database.prepare("SELECT value FROM coordinator_meta WHERE key = 'schema'").get();
      if (["treeswap.coordinator.v1", "treeswap.coordinator.v2", "treeswap.coordinator.v3", "treeswap.coordinator.v4"].includes(schema?.value)) {
      ...
      try {
        this.#db.prepare(`
          INSERT INTO firm_offer_commitments(
            offer_id, offer_digest, request_id, solver_id, direction, amount, bit_amount_wei,
            lightning_amount_sats, capacity_epoch, expires_at, record_digest, state, reserved_at
          ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'ACTIVE', ?)
        `).run(
          value.offerId, value.offerDigest, value.requestId, value.solverId, value.offer.direction, String(amount),
          String(bitAmountWei), String(lightningAmountSats), value.offer.capacityEpoch, value.offer.expiresAt,
          recordDigest, value.now,
        );
      } catch (error) {
