...
export async function pinnedPublicHttpsRequest(urlValue, options, {
  ...
  if (requestUrl.origin !== url.origin || requestUrl.pathname !== "/v1/capability" || requestUrl.search || requestUrl.hash) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const request = httpsRequestImpl({
      ...
      response.on("data", (chunk) => {
        ...
        if (received > limit) {
          response.destroy();
          finish(reject, new Error("solver endpoint response exceeded its size limit"));
          return;
        }
        chunks.push(Buffer.from(chunk));
      });
      response.on("aborted", () => finish(reject, new Error("solver endpoint response was interrupted")));
      response.on("error", (error) => finish(reject, error));
      response.on("end", () => {
        const headers = response.headers;
        const bodyStream = Readable.toWeb(Readable.from([Buffer.concat(chunks)]));
        finish(resolve, {
...
async function boundedJson(response, maximumBytes) {
  ...
  if (!contentType.startsWith("application/json")) throw new Error("solver endpoint response content type is invalid");
  const declared = Number(response.headers?.get?.("content-length") ?? 0);
  if (Number.isFinite(declared) && declared > maximumBytes) {
    throw new Error("solver endpoint response exceeded its size limit");
  }
  if (!response.body) throw new Error("solver endpoint returned an empty response");
  ...
  try {
    ...
    throw new Error("solver endpoint response was not JSON");
  }
}
...
export async function queryVerifiedSolverCapability({
  ...
  let response;
  try {
    response = normalizeResponseShape(await boundedJson(rawResponse, responseLimit));
  } catch {
    throw new SolverEndpointError("solver endpoint returned an invalid response", { code: "INVALID_RESPONSE" });
  }
  ...
  const capability = await verifySolverCapability({
