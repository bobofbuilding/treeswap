...
export function validateContractIntentSigningPayload(input) {
  const source = exactRecord(
    input,
    ["direction", "domain", "message", "primaryType"],
    "contract intent signing payload",
  );
  ...
  const beneficiaryField = lightningToBit ? "beneficiary" : "solverBeneficiary";
  const nonceField = lightningToBit ? "nonce" : "solverNonce";
  const normalizedDomainValue = Object.freeze({
    name: domain.name,
    version: domain.version,
    chainId: domain.chainId.toString(),
    verifyingContract: domain.verifyingContract,
  });
  const normalizedMessageValue = Object.freeze({
    quoteId: message.quoteId,
    user: message.user,
    solver: message.solver,
    [beneficiaryField]: message[beneficiaryField],
