...
const OWNED_SERVICES = new WeakSet();
const SERVICE_LEASES = new WeakMap();
...
export function selectedSolverInvoiceMaterialServiceMode(value) {
  const context = SERVICES.get(value);
  if (!context) throw new TypeError("selected-solver invoice service provenance is invalid");
  return context.mode;
}

export function claimSelectedSolverInvoiceMaterialService(value, input) {
  const context = SERVICES.get(value);
  if (!context) throw new TypeError("selected-solver invoice service provenance is invalid");
  const source = exactDataRecord(
    input,
    ["paymentSecretKeyId", "signal"],
    "selected-solver invoice service ownership",
  );
  if (keyId(source.paymentSecretKeyId) !== context.paymentSecretKeyId) {
    throw new Error("selected-solver invoice service key version changed");
  }
  if (!(source.signal instanceof AbortSignal) || source.signal.aborted) {
    throw new TypeError("selected-solver invoice service ownership requires an active signal");
  }
  if (OWNED_SERVICES.has(value)) {
    throw new Error("selected-solver invoice service is already route-bound");
  }
  OWNED_SERVICES.add(value);
  const leaseContext = { service: value, signal: source.signal, state: "active" };
  const stop = () => { leaseContext.state = "stopped"; };
  source.signal.addEventListener("abort", stop, { once: true });
  const lease = Object.freeze({
    resolve(request, options) {
      if (this !== lease || SERVICE_LEASES.get(this) !== leaseContext
          || leaseContext.state !== "active" || leaseContext.signal.aborted) {
        throw new TypeError("selected-solver invoice service lease is inactive");
      }
      return resolveSelectedSolverInvoiceMaterial(value, request, options);
    },
    status() {
      if (this !== lease || SERVICE_LEASES.get(this) !== leaseContext) {
        throw new TypeError("selected-solver invoice service lease provenance is invalid");
      }
      return Object.freeze({
        schema: "treeswap.selected-solver-invoice-material-service-lease.v1",
        state: leaseContext.state,
        mode: context.mode,
        exposesPreimage: false,
        exposesLndCredential: false,
        fundingAuthorization: false,
  ...
  SERVICE_LEASES.set(lease, leaseContext);
  return lease;
}
