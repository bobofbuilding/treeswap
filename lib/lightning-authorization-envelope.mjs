...
function normalizeLightningAuthorizationEnvelope(envelope) {
  const source = exactDataRecord(envelope, ["payload", "signature"], "authorization envelope");
  const payload = normalizeLightningAuthorizationPayload(source.payload);
  if (typeof source.signature !== "string") throw new Error("authorization signature is invalid");
  const signature = Buffer.from(source.signature, "base64");
  if (signature.length !== 64 || signature.toString("base64") !== source.signature) {
    throw new Error("authorization signature is invalid");
  }
  return Object.freeze({ payload, signature: source.signature });
}

export function lightningAuthorizationEnvelopeDigest(envelope) {
  const normalized = normalizeLightningAuthorizationEnvelope(envelope);
  return `0x${createHash("sha256").update(canonicalize(normalized)).digest("hex")}`;
}
...
export function verifyLightningAuthorizationEnvelope(input) {
  ...
  const envelope = normalizeLightningAuthorizationEnvelope(source.envelope);
