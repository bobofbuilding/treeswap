...
async function inspectRelease(root, raw, index, usedPaths) {
  ...
  if (raw.runtime.coordinatorSchema !== "treeswap.coordinator.v10") {
  ...
  return Object.freeze({
    ...
    releaseRecordDigest,
    releasePolicyDigest,
    riskPolicyDigest: bytes32(
      record.evidenceDigests.riskPolicy,
      `releases[${index}].riskPolicyDigest`,
    ),
...
export async function inspectRetainedReleaseCustody({ manifestPath }) {
  ...
  if (manifest.schema !== RETAINED_RELEASE_CUSTODY_SCHEMA
      || manifest.coordinatorSchema !== "treeswap.coordinator.v10") {
  ...
  for (const liability of liabilitySnapshot.releases) {
    ...
    for (const executionPolicy of liability.executionPolicies) {
      if (executionPolicy.riskPolicyDigest !== release.riskPolicyDigest
          || !release.daemonEvidencePolicies.some((entry) => (
...
export function prepareRetainedReleaseRecoveryJobSet({
  ...
  for (const liabilityPolicy of readiness.liability.executionPolicies) {
    if (liabilityPolicy.riskPolicyDigest !== readiness.release.riskPolicyDigest
        || !policies.some((entry) => (
  ...
  const jobs = normalizeCoordinatorRecoveryJobs(settlements.map((settlement) => {
    if (!settlement.releaseRecordDigest || !settlement.riskPolicyDigest || !settlement.evidencePolicyDigest
    ...
    if (settlement.riskPolicyDigest !== readiness.release.riskPolicyDigest) {
      throw new Error("recovery settlement is bound to another risk policy");
    }
...
export function verifiedRetainedReleaseRecoveryJobs({
  ...
  return Object.freeze({
    jobs: jobSet.jobs,
    jobSetDigest: jobSet.jobSetDigest,
    lease,
    releaseId: jobSet.releaseId,
    releaseRecordDigest: jobSet.releaseRecordDigest,
  });
}
